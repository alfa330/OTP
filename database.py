...
class Database:
        ...
        with self._get_cursor() as cursor:
            ...
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS shift_auction_test_access (
                    ...
                    selected_schedule_plan_id INTEGER NULL REFERENCES resource_saved_schedule_plans(id) ON DELETE SET NULL,
                    published_to_work_schedules_at TIMESTAMP NULL,
                    published_to_work_schedules_by INTEGER NULL REFERENCES users(id) ON DELETE SET NULL,
                    updated_by INTEGER REFERENCES users(id) ON DELETE SET NULL,
                    updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
            ...
            cursor.execute("ALTER TABLE shift_auction_test_access ADD COLUMN IF NOT EXISTS paused_at TIMESTAMP NULL;")
            cursor.execute("ALTER TABLE shift_auction_test_access ADD COLUMN IF NOT EXISTS finished_at TIMESTAMP NULL;")
    ...
    def _get_shift_auction_test_status(self, enabled, starts_at=None, ends_at=None, paused_at=None, finished_at=None):
        if not enabled:
            return "disabled"
        if finished_at:
            return "closed"
    ...
    def _serialize_shift_auction_period_row(self, row):
        ...
        date_from = row[1]
        date_to = row[2]
        shift_count = int(row[5] or 0) if len(row) > 5 else 0
        can_restart = bool(row[6]) if len(row) > 6 else bool(date_to and date_to >= date.today())
        return {
            "id": int(row[0]),
    ...
    def _validate_shift_auction_period_tx(self, cursor, schedule_plan_id, require_restartable=False):
        ...
        if require_restartable and not bool(row[6]):
            raise ValueError("AUCTION_PERIOD_PAST")
        return row
    ...
    def _get_shift_auction_operator_blocked_dates_tx(self, cursor, operator_id, lot_dates=None):
        ...
        cursor.execute("""
            SELECT DISTINCT ON (d.shift_date)
                d.shift_date,
                p.id,
                p.status_code,
                p.start_date,
                ...
                p.is_blacklist,
                p.comment
            FROM UNNEST(%s::date[]) AS d(shift_date)
            JOIN operator_schedule_status_periods p
              ON p.operator_id = %s
             AND d.shift_date >= p.start_date
             AND d.shift_date <= COALESCE(p.end_date, %s::date)
            WHERE p.start_date <= %s::date
              AND COALESCE(p.end_date, %s::date) >= %s::date
            ORDER BY d.shift_date,
                     p.start_date DESC,
                     COALESCE(p.end_date, p.start_date) DESC,
                     p.id DESC
        """, (auction_dates, operator_id, period_end, period_end, period_end, period_start))

        results = []
        for shift_date, period_id, status_code, start_date, end_date, dismissal_reason, is_blacklist, comment in (cursor.fetchall() or []):
            if not shift_date:
                continue
    ...
    def get_shift_auction_test_access(self, current_user_id=None):
        ...
        return {
            ...
            "updated_by": settings_row[7] if settings_row else None,
            "updated_by_name": settings_row[8] or "",
            "updated_at": settings_row[9].isoformat() if settings_row and settings_row[9] else None,
        ...
        with self._get_cursor() as cursor:
            ...
            if action_norm == 'pause':
                if current_status != 'open':
                    raise ValueError("AUCTION_CANNOT_PAUSE")
                cursor.execute("""
                    UPDATE shift_auction_test_access
                    SET paused_at = %s,
                        updated_by = %s,
                ...
                pause_delta = now - paused_at
                next_ends_at = ends_at + pause_delta if ends_at else None
                cursor.execute("""
                    UPDATE shift_auction_test_access
                    SET paused_at = NULL,
                        ends_at = %s,
    ...
    def publish_shift_auction_test_to_work_schedules(self, updated_by=None):
        with self._get_cursor() as cursor:
            ...
            if not settings_row:
                raise ValueError("AUCTION_NOT_AVAILABLE")
            if self._get_shift_auction_test_status(*settings_row) != "closed":
            ...
            day_off_dates = {
                ...
                if operator_id is not None and day_off_date is not None
            }

            summary = {
                "operators_total": len(operator_ids),
                "days_total": len(lot_dates),
                "cleared_days": 0,
                "shifts_saved": 0,
                "days_off_saved": 0,
                "blocked_days": 0,
            ...
            for operator_id in operator_ids:
                ...
                for lot_date in lot_dates:
                    ...
                    if lot_date_key in blocked_date_map:
            ...
            cursor.execute("""
                UPDATE shift_auction_test_access
                SET published_to_work_schedules_at = CURRENT_TIMESTAMP,
    ...
    def restart_shift_auction_test(self, schedule_plan_id, updated_by=None):
        with self._get_cursor() as cursor:
            ...
            cursor.execute("""
                UPDATE shift_auction_test_access
                SET selected_schedule_plan_id = %s,
                    paused_at = NULL,
                    finished_at = NULL,
            ...
            event = self._insert_shift_auction_test_event(cursor, "auction_restarted", {
    ...
    @staticmethod
    def _is_survey_operator_dismissed_status(value):
        return str(value or '').strip().lower() in ('fired', 'dismissal', 'dismissed')

    def filter_active_survey_operator_ids(self, operator_ids):
        normalized_ids = []
        for op_id in operator_ids or []:
            try:
                parsed = int(op_id)
        ...
        with self._get_cursor() as cursor:
            cursor.execute("""
                ...
                WHERE id = ANY(%s)
                  AND NOT (
                      LOWER(TRIM(COALESCE(status, ''))) IN ('fired', 'dismissal', 'dismissed')
                  )
            """, (normalized_ids,))
    ...
    def update_survey(self, survey_id, requester_id, requester_role, title, description, assignment, questions, operator_ids, is_test=False):
        ...
        for idx, raw_question in enumerate(questions):
            question_id = None
            raw_question_id = (raw_question or {}).get('id')
            if raw_question_id not in (None, ''):
                try:
                    parsed_question_id = int(raw_question_id)
                    if parsed_question_id > 0:
        ...
        with self._get_cursor() as cursor:
            ...
            cursor.execute("SELECT id FROM survey_questions WHERE survey_id = %s", (survey_id,))
            existing_question_ids = {int(row[0]) for row in cursor.fetchall()}
            cursor.execute("UPDATE survey_questions SET position = -id WHERE survey_id = %s", (survey_id,))
            ...
            for question in normalized_questions:
                ...
                if question_id is not None and question_id in existing_question_ids:
                    ...
                    kept_question_ids.add(int(cursor.fetchone()[0]))

            removed_question_ids = list(existing_question_ids - kept_question_ids)
            if removed_question_ids:
                cursor.execute("""
                    DELETE FROM survey_questions
                    WHERE survey_id = %s AND id = ANY(%s)
                """, (survey_id, removed_question_ids))
            ...
            cursor.execute("SELECT operator_id FROM survey_assignments WHERE survey_id = %s", (survey_id,))
            existing_operator_ids = {int(row[0]) for row in cursor.fetchall()}
            target_operator_ids = set(operator_ids_norm)
            removed_operator_ids = list(existing_operator_ids - target_operator_ids)
            if removed_operator_ids:
                cursor.execute("""
                    DELETE FROM survey_responses
                    WHERE survey_id = %s AND operator_id = ANY(%s)
                """, (survey_id, removed_operator_ids))
                cursor.execute("""
                ...
                cursor.execute("""
                    ...
                    SET
                        ...
                        completed_at = CASE
                            ...
                            ELSE NULL
                        END
                """, (survey_id, operator_id, requester_id))

            return {'id': survey_id, 'updated_at': self._survey_dt_to_iso(datetime.now())}
