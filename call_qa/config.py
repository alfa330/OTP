...
# на длинных звонках, берём с запасом. Уменьшать нельзя: сверх лимита конкурентности
# Z.ai не отказывает, а ставит в очередь, и на 52 параллельных запросах самый долгий
# ответ пришёл ровно на 300-й секунде (замер 28.08.2026).
...
# локальным путём, что у Vertex.
#
# Лимит аккаунта — по КОНКУРЕНТНОСТИ, а не по токенам: 50 одновременных запросов на
# GLM-5.3-Flash (кабинет Z.ai, 28.08.2026). Замер настоящими телами оценки в тот же день:
#
#   потоков   успешно   медиана   максимум   оценок/час
#        16     16/16      84 с     116 с           495
#        32     32/32     126 с     181 с           637
#        52     52/52     166 с     300 с           624
#
# Два вывода, которых не было в документации. Первый: сверх лимита Z.ai не отказывает,
# а СТАВИТ В ОЧЕРЕДЬ — 52 запроса при лимите 50 прошли все до одного, просто медленнее.
# Значит защищаться надо не от ошибки 1302, а от роста задержки. Второй: пропускная
# способность упирается в ~630 оценок/час около 32 потоков и дальше не растёт вовсе,
# зато задержка продолжает удваиваться.
#
# Отсюда 16: это 495 оценок/час (ночной прогон 660 звонков за 1,3 часа вместо 4,2),
# запас до таймаута в 2,6 раза и 34 свободных слота аккаунта. Слоты нужны: карточка
# оценки у супервайзера ходит в тот же пул, и на 52 потоках она ждала бы пять минут.
ZAI_LOCAL_BATCH_WORKERS = int(env("ZAI_LOCAL_BATCH_WORKERS", "16"))
...
# Умолчание — `glm-5.3-flash`: модель нативно мультимодальна и читает то, чего
# Claude здесь не читал вовсе. Проверено запросами 28.08.2026 на живом API:
# картинка (чек Kaspi) — 5,5 с и 444 входных токена, PDF — 11 с, пятисекундное
# видео — 6 с и 1 581 токен, все пять кадров перечислены верно. Вернуться на
# Claude — одна переменная AI_QA_MODEL_VISION=claude-sonnet-5.
# Имя AI_QA_MODEL_VISION основное; CLAUDE_MODEL_VISION оставлено как совместимость.
CLAUDE_MODEL_VISION = env("AI_QA_MODEL_VISION") or env("CLAUDE_MODEL_VISION", "glm-5.3-flash")
# У GLM это уровень рассуждений (low/high/max). Для вложений хватает low: описание
# картинки — не рассуждение, а чтение, и high удвоил бы и время, и выход.
...
# --- Видео ---
# Claude видео не читал вовсе, поэтому оно молча выпадало из транскрипта эпизода:
# сообщение с роликом выглядело как пустое. GLM-5.3-Flash его читает.
# Вендорский предел — 200 МБ на ролик, но 20 МБ здесь НАШ и намеренно ниже: файл
# уезжает в тело запроса как base64, то есть плюс треть веса, и двухсотмегабайтный
# ролик означал бы запрос на четверть гигабайта. Цену всё равно задаёт не вес, а
# длительность: модель разбирает видео на 2 кадра в секунду (fps=2 в конфигурации
# видеопроцессора), и пятисекундный ролик весом 5,9 КБ стоил 1 581 входной токен.
# Длительность вендор не ограничивает нигде в документации — предел ставит контекст.
MEDIA_VIDEO_MAX_BYTES = int(env("WZ_MEDIA_VIDEO_MAX_BYTES", str(20 * 1024 * 1024)))
MEDIA_VIDEO_TYPES = ("video",)
# Ролик описывается тем же коротким JSON, что и картинка, но кадров в нём больше.
VIDEO_MAX_TOKENS = int(env("WZ_VIDEO_MAX_TOKENS", "3000"))
...
# Массовое описание вложений у провайдера БЕЗ пакетного режима (Z.ai): считаем сами
# в несколько потоков. Потоков меньше, чем у оценки звонков: вложение — это ещё и
# скачивание файла, и лимит конкурентности аккаунта общий с ночным прогоном оценок.
MEDIA_LOCAL_WORKERS = int(env("WZ_MEDIA_LOCAL_WORKERS", "8"))
# Видео читается дольше картинки, а качается ещё дольше.
MEDIA_LOCAL_TIMEOUT = float(env("WZ_MEDIA_LOCAL_TIMEOUT", "300"))
