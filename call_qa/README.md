...
Раздел оценивает три отдела, и отдел — вторая ось доступа рядом с направлениями.
Перечень живёт в `config.DEPARTMENT_CODES` (ключ — `departments.code`, а не id:
код стабилен и уже служит ключом в остальных местах портала).

| Отдел | Телефония | Переписка |
|---|---|---|
| `op` — Отдел продаж | записи загружают вручную (GCS) | Wazzup, эпизоды Верификаторов |
| `szov` — СЗоВ | Oktell | Chat2Desk, заявки чат-менеджеров |
| `tez` — Тез КЦ | Binotel | ChatApp, эпизоды |

Селектор отдела в разделе получает список из `GET /api/ai-qa/departments`:
супер-админ и глобальный админ видят все три отдела, глава и СВ — свой,
наблюдатель «Маркетинга» — разборы ОП (его периметр не расширялся вместе с
разделом). Отдел из запроса сверяется с доступными (`_ai_qa_requested_department`),
иначе подстановка `?department=` открывала бы чужой отдел. На ЗАПИСИ (классификация
критериев, разбор) отдел сверяется отдельно — `_ai_qa_direction_department_allowed`:
у главы отдела ось направлений не ограничена, и без этой проверки он правил бы
шкалу чужого отдела.

## Субъекты оценки

Субъект — это ТАБЛИЦА-ИСТОЧНИК, а не «звонок или чат»: id у `calls`,
`imported_calls`, `wazzup_episodes`, `c2d_chat_snapshots` и `chatapp_episodes` —
независимые последовательности, и они численно пересекаются.

| Субъект (`subject_kind`) | Что это | Источник текста |
|---|---|---|
| `call` | аудиозвонок, уже оценённый человеком (`calls`) | GCS → Soniox ASR (диаризация, per-token confidence) |
| `imported_call` | звонок из АТС БЕЗ оценки в журнале (`imported_calls`) | тот же путь, что у `call` |
| `wz_episode` | эпизод переписки WhatsApp у Верификаторов ОП (`wazzup_episodes`) | сообщения Wazzup + расшифровка вложений |
| `c2d_snapshot` | заявка Chat2Desk у СЗоВ (`c2d_chat_snapshots`, `source='chat2desk'`) | сообщения снапшота + расшифровка вложений |
| `ca_episode` | эпизод переписки у Тез КЦ (`chatapp_episodes`) | сообщения ChatApp (иначе снапшот) + расшифровка вложений |

Все три источника переписки приводятся к ОДНОЙ форме сообщения
(`subjects._fetch_chatapp_messages`, `subjects._c2d_snapshot_messages`), поэтому
план вложений, их расшифровка и сборка транскрипта общие: четвёртая копия
нормализатора неизбежно разошлась бы с остальными.

### Звонок без оценки в журнале

У СЗоВ и Тез КЦ звонок можно подтянуть из АТС, не оценивая его человеком, и
именно такие звонки раздел берёт первыми: `random_call` сначала ищет
`imported_calls` без связанной строки `calls` (пул портала), и лишь потом падает
на звонок, оценённый человеком (там видно расхождение ИИ и супервайзера).
`POST /api/ai-qa/pull-call` идёт за НОВЫМ звонком прямо в АТС — теми же функциями
`_oktell_random_call` / `_binotel_random_call`, что и кнопка «Случайный звонок»
журнала: вторая копия разошлась бы с первой (у Oktell там ловушка с путём к
записи, из-за которой исходящие молча терялись). Оценки в журнале при этом не
появляется.
...
### Переписка: когда оценка возможна

У эпизодных источников (Wazzup, ChatApp) мерка одна — доля ответов
доминирующего оператора. У Chat2Desk её не существует: единица там ЗАЯВКА,
закреплённая за одним оператором, и автора у отдельного сообщения нет вовсе.
Вместо доли отсекается то, что реально делает оценку нечестной, — РУЧНАЯ
ПЕРЕДАЧА чата другому сотруднику посреди заявки (системная строка «Чат
передан…»). Автоназначение в начале передачей не считается: это и есть выдача
заявки оператору. Замер на проде 07.09.2026: из 1972 неоценённых заявок с ≥2
ответами оператора реально поделены между людьми 131.
...
`python -m call_qa.batch_eval --month YYYY-MM --workdir <durable-dir>` использует те же fingerprints, snapshot и финализацию, что online-путь. Субъект выбирается флагом `--subject`, отдел — флагом `--department` (по умолчанию `op`).

Отдел здесь ЯВНЫЙ и по умолчанию отдел продаж: пакетный прогон — это платные вызовы модели на весь месяц, и включать в него СЗоВ и Тез КЦ вместе с открытием раздела нельзя. Субъект и отдел должны совпадать (`--department szov --subject ca_episode` отвергается): у отдела ровно один источник переписки. Отдел входит и в имя папки прогона — иначе перезапуск СЗоВ подхватил бы чекпоинты ОП.
...
| `WZ_MIN_OPERATOR_SHARE` | `0.9` | Минимальная доля ответов одного оператора в эпизоде Wazzup. |
| `WZ_MIN_OPERATOR_MESSAGES` | `2` | Минимум ответов оператора в эпизоде Wazzup. |
| `AI_QA_DEPARTMENT_CODES` | `op,szov,tez` | Отделы, которые оценивает раздел. |
| `AI_QA_CHAT_CRITERIA_MAP` | `83:84` | Откуда брать шкалу чата, если оператор числится на другом направлении (ТЭЗ: «ТП линия» → «ТП чат»). |
| `CA_MIN_OPERATOR_SHARE` | как у Wazzup | Порог атрибуции эпизода ChatApp (Тез КЦ). |
| `CA_MIN_OPERATOR_MESSAGES` | как у Wazzup | Минимум ответов оператора в эпизоде ChatApp. |
| `C2D_MIN_OPERATOR_MESSAGES` | `2` | Минимум ответов оператора в заявке Chat2Desk (доли ответов у источника нет). |
