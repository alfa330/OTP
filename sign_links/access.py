...
защищённый QR-доступом… журнал запросов с нужными фильтрами (для админов)».
На портал это ложится так:

    оператор СЗоВ / фронт-офиса     вводит ИИН и получает ссылку — после QR
    супервайзер этих отделов        то же; QR ему не задают (он его подтверждает)
    глава СЗоВ / глава фронт-офисов то же, плюс журнал СВОЕГО отдела
...
# Периметр раздела — два отдела. Списка id нет и не будет: отдел и есть периметр.
SECTION_DEPARTMENT_CODES = ('szov', 'front_office')
