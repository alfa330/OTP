...
-- ── ЖУРНАЛ ЭТАПОВ СДЕЛКИ ───────────────────────────────────────────────────
-- Единственное место, где у нас появляется ИСТОРИЯ сделки. В amoCRM её нет:
-- у сделки один статус, и вчерашний ответ на вопрос «на каком этапе была эта
-- заявка» сегодня уже другой (замерено 11.09.2026: дозвон 499 → 534, согласия
-- 257 → 267 на том же составе лидов).
--
-- Поэтому «Маркетинговый мониторинг» (ТЗ #317, ФТ-08 «Этап на момент
-- разговора») спрашивает не CRM, а этот журнал: какой этап стоял у сделки в
-- момент звонка. Строка пишется ТОЛЬКО когда этап или причина отказа
-- изменились — иначе выгрузка раз в 15 минут дала бы 96 одинаковых строк в
-- сутки на каждую из 16 тысяч сделок.
--
-- Задним числом журнал не наполняется: бэкфилл из amoCRM положил бы сюда
-- СЕГОДНЯШНИЙ этап с сегодняшней меткой времени, и это было бы враньё. До
-- первой записи по сделке «этап на момент разговора» честно пуст, и фильтр
-- показывает это словами, а не молча пустым списком.
CREATE TABLE IF NOT EXISTS op_funnel_lead_stages (
    source       VARCHAR(24)  NOT NULL,
    lead_key     VARCHAR(64)  NOT NULL,
    seen_at      TIMESTAMP    NOT NULL DEFAULT NOW(),
    stage_raw    TEXT         NOT NULL DEFAULT '',
    reason_raw   TEXT         NOT NULL DEFAULT '',
    PRIMARY KEY (source, lead_key, seen_at)
);

-- Поиск идёт всегда «последняя запись не позже момента звонка», поэтому индекс
-- по убыванию времени внутри сделки.
CREATE INDEX IF NOT EXISTS idx_op_funnel_lead_stages_at
    ON op_funnel_lead_stages (source, lead_key, seen_at DESC);
"""
