...
CONTEST = {
    ...
    "splits": [
        ...
        {
            "crm_operator_id": "146",
            "operator_name": "Серикбаева Асел Куралбеккызы",
            # 11.09.2026 в 10:34 «Чат менеджер» -> «Основа» (user_history) —
            # уже ПОСЛЕ последнего зачётного дня регистраций (07.09), поэтому
            # линейной части у неё нет вовсе: весь конкурс она отработала в
            # чатах. Дату ставим по тому же правилу (день перехода остаётся
            # прежней группе), а окно среза «до» прижмёт split_before_to.
            "switch_date": "2026-09-12",
            "before_group": "chat",
            "after_group": "line",
        },
        {
            "crm_operator_id": "600",
            "operator_name": "Толегенов Анет Серикулы",
            # У него граница внутри конкурса, и переход обратный: 11.08.2026 в
            # 23:05 «Основа» -> «Чат менеджер» (user_history), то есть первые
            # пять дней конкурса заработаны на линии, а с 12.08 — в чатах.
            # Возврат на «Основу» 11.09.2026 на конкурс уже не влияет:
            # регистрации после 07.09 в зачёт не идут. Решение владельца
            # 11.09.2026 — делить именно по этой границе.
            "switch_date": "2026-08-12",
            "before_group": "line",
            "after_group": "chat",
        },
    ],
...
def split_before_to(switch_date, registered_to=None):
    ...
    За окно конкурса граница не выходит. Переход после последнего зачётного дня
    (registered_to) — законный случай: регистраций «после» в конкурсе нет вовсе,
    весь счёт остаётся прежней группе. Без прижимания мы спросили бы у CRM окно
    ШИРЕ конкурсного, срез «до» вышел бы больше общего счёта, и apply_group_splits
    прижал бы его с предупреждением «часть обнулена» — шум ровно там, где всё
    посчитано верно."""
    before_to = (date.fromisoformat(switch_date) - timedelta(days=1)).isoformat()
    if registered_to and before_to > registered_to:
