...
# Потолок периода выгрузки. Тридцать суток — как у «Посылок»: столько же, сколько
# человек способен осмысленно просмотреть за раз, и ровно та рамка, о которой
# попросил владелец 08.09.2026 («макс выгрузка на 30 дней»). Границу держит
# СЕРВЕР (`_export_period` в routes.py) — кнопку в интерфейсе можно обойти,
# ручку зовут напрямую. Двойник во фронте — EXPORT_MAX_DAYS в journalMeta.js,
# их сверяет тест.
EXPORT_MAX_DAYS = 30
...
COLUMNS = (
    ...
    # «Обращение» заполнено только у передачи: это заявка, куда вендор реально
    # положил заметку. У просмотра его нет — с 04.09.2026 чат склеен по парку, и
    # обращений внутри несколько, так что записывать одно из них значило бы
    # соврать о том, что человек смотрел. Колонку не убираем: она нужна, чтобы
    # свести передачу с метриками чат-менеджера, и заполнена у прежних строк.
    ('request_id', 'Обращение (заметки)', 15),
    ('dialog_id', 'Диалог', 13),
...
def plural_days(count):
    """«31 день», «32 дня», «45 дней». Двойник `pluralDays` из journalMeta.js.

    Нужен именно он, а не «%d суток»: «сутки» — множественное без формы
    единственного, и «31 суток» — не по-русски (как и «31 сутки»). Та же
    функция стоит в «Посылках»; копия здесь, а не импорт, потому что пакеты
    разделов друг друга не импортируют.
    """
