...
* **Адрес чата — `client_id` + `channel_id`**, рядом справочно лежат
  `dialog_id` и `request_id`. Постановка требует знать, «в какой чат он был
  направлен», а у Chat2Desk это разные ключи: клиент (водитель), канал
  (таксопарк), диалог (переписка) и заявка (обращение внутри переписки). После
  склейки «один чат = один парк» (04.09.2026) адресом стал именно канал:
  у события «открыл переписку» одного обращения больше нет, их внутри
  несколько, поэтому `request_id` там остаётся пустым. У «передал» он есть, но
  приходит от вендора — это обращение, куда реально легла заметка, и по нему
  запись сходится с метриками чат-менеджера в c2d_requests.
...
* **Индекс по хвосту телефона стоит на ЧУЖОЙ таблице `c2d_requests`.** Раздел
  ищет водителя по последним девяти цифрам номера — иначе иностранный номер,
  названный без кода страны, не находится ничем. Индекс лежит здесь, рядом с
  тем единственным запросом, ради которого нужен, а не в общем блоке монолита.
...
DDL = (
    ...
    # Канал (таксопарк) добавлен 04.09.2026 вместе со склейкой чатов «один чат =
    # один парк»: именно он стал настоящим адресом чата, а `request_id` у
    # события «открыл переписку» смысл потерял — обращений внутри чата
    # несколько. Отдельным ALTER, а не колонкой в CREATE TABLE: таблица уже
    # живёт на проде, и `CREATE TABLE IF NOT EXISTS` её не тронет.
    "ALTER TABLE dch_events ADD COLUMN IF NOT EXISTS channel_id BIGINT",
    ...
    # ── Хвост номера в чужой таблице ─────────────────────────────────────────
    #
    # Индекс по ВЫРАЖЕНИЮ на c2d_requests — единственная строка схемы раздела,
    # которая трогает не свою таблицу. Так сделано осознанно: нужен он ровно
    # одному запросу — поиску водителя по последним девяти цифрам номера
    # (queries.local_client_by_tail), без которого иностранный номер, названный
    # без кода страны, не находится вовсе. Держать его здесь, рядом с этим
    # запросом, честнее, чем прятать в общий блок c2d_requests монолита, где
    # никто не свяжет его с разделом.
    #
    # Выражение обязано СЛОВО В СЛОВО совпадать с тем, что в запросе, иначе
    # планировщик индекс не возьмёт. Обе функции IMMUTABLE, поэтому индекс по
    # ним строится.
    #
    # Цена: 86 тыс. строк, один b-tree; выигрыш — 0,2 с полного прохода на
    # каждом поиске, который не нашёлся точным совпадением. Синк метрик пишет в
    # таблицу раз в сутки, так что замедление вставки здесь роли не играет.
    r"""
