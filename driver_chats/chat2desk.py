...
# Сколько записей номера пробуем у вендора, прежде чем сказать «не нашли».
# Первая почти всегда и срабатывает; остальные — про номер, введённый без кода
# страны или в чужой форме. Потолок стоит ради общей месячной квоты.
...
def find_client(phone):
    ...
    «поиск не нашёл». Фильтр точный, поэтому номер, записанный у вендора иначе
    (с «8», без кода страны), первым запросом не найдётся — перебираем
    правдоподобные записи, пока не совпадёт.
    """
    for candidate in phone_variants(phone)[:MAX_CLIENT_LOOKUPS]:
        payload = _request('GET', '/v1/clients', params={'phone': candidate, 'limit': 2})
        ...
        return {
            ...
            # Отдаём ту запись, под которой номер реально лежит у вендора: по
            # ней потом сходится журнал и повторный поиск.
