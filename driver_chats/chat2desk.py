...
# Границы «похоже на телефон». Десять цифр — короткий национальный номер без
# кода страны, пятнадцать — потолок E.164. Уже этого хватает, чтобы отсечь мусор
# вроде идентификаторов WhatsApp, которые вендор кладёт в то же поле (244 строки
# вида «[wa…» длиной 30-32 знака).
MIN_PHONE_DIGITS = 10
MAX_PHONE_DIGITS = 15

# Сколько последних цифр опознают номер однозначно. Девять — не на глаз: в
# c2d_requests 16 736 разных номеров, и на девятизначном хвосте у них НИ ОДНОГО
# совпадения между разными клиентами (замер 08.09.2026), а на восьмизначном уже
# две пары. Хвост нужен, потому что один и тот же водитель записан у вендора то
# с кодом страны, то без, то с «+», то с «00», и человек за телефоном диктует
# номер как привык — особенно иностранный.
PHONE_TAIL_DIGITS = 9


def normalize_phone(raw):
    """Любая запись номера -> одни цифры, в том виде, в каком его хранит вендор.

    НЕ только Казахстан. В c2d_requests 1148 записей с другим кодом страны:
    Россия (79…, 11 знаков), Узбекистан (998…), Киргизия (996…), Таджикистан
    (992…), Турция, Великобритания — все по 12 знаков. Водитель с российской
    симкой — обычное дело, и раздел обязан его находить, а не отвечать «введите
    казахстанский номер».

    Три поправки на привычный ввод, все безопасны: «00 998 …» -> «998 …»
    (международный префикс набора вместо «+»), «8 707 …» -> «7 707 …»
    (внутренняя форма Казахстана и России) и «707 …» -> «7 707 …»
    (казахстанский абонентский номер без кода страны).

    Иностранный номер, продиктованный БЕЗ кода страны, этой функцией не
    достраивается и достроен быть не может: кодов стран две сотни. Его ловит
    поиск по хвосту — см. phone_tail и queries.local_client_by_tail.

    Возвращает None, только если из ввода вообще не собирается номер: пусть
    лучше раздел скажет «непохоже на номер», чем уйдёт в вендора с мусором.
    """
    digits = _phone_digits(raw)
    if not digits or not MIN_PHONE_DIGITS <= len(digits) <= MAX_PHONE_DIGITS:
        return None
    return digits


def _phone_digits(raw):
    """Цифры номера с поправками на привычный ввод, БЕЗ проверки длины.

    Отдельно от normalize_phone, потому что длина у двух путей разная: точному
    совпадению нужен номер целиком (от десяти цифр — так его хранит вендор), а
    поиску по хвосту хватает девяти. Девять — это не «поблажка», а ровно
    национальный номер Узбекистана, Киргизии и Таджикистана: там абонентский
    номер девятизначный, и водитель диктует его без кода страны.
    """
    digits = re.sub(r'\D', '', str(raw or ''))
    if not digits:
        return None
    # 00 998 90 … -> 998 90 …: международный префикс НАБОРА, который пишут
    # вместо «+». Ни один код страны с нуля не начинается, так что снять его
    # безопасно; без этого узбекский номер, продиктованный «как набирать»,
    # уходил в поиск четырнадцатизначным и не находился нигде.
    if digits.startswith('00') and len(digits) - 2 >= PHONE_TAIL_DIGITS:
        digits = digits[2:]
    # 8 707 123 45 67 -> 7 707 123 45 67
    if len(digits) == 11 and digits.startswith('8'):
        digits = '7' + digits[1:]
    # 707 123 45 67 -> 7 707 … (казахстанский мобильный без кода страны)
    if len(digits) == 10 and digits[0] == '7':
        digits = '7' + digits
    return digits


def phone_tail(raw, size=PHONE_TAIL_DIGITS):
    """Последние `size` цифр номера — то, чем он опознаётся в ЛЮБОЙ записи.

    Точные варианты (phone_variants) перебирают то, как номер пишут У НАС:
    с «8», с «+», без кода страны. Для иностранного номера этого мало — кодов
    стран в мире две сотни, и угадать, что «531 729 83 61» это Турция, нельзя.
    Зато хвост у номера один и тот же во всех записях, и по нему водитель
    находится, как бы его номер ни лежал у вендора.

    Девяти цифр ДОСТАТОЧНО, даже если номер целиком мы бы забраковали: узбек,
    киргиз и таджик диктуют ровно девять — их национальный номер такой длины.
    Верхнюю границу оставляем ту же, что у номера: длиннее пятнадцати — это уже
    идентификатор мессенджера, а не телефон.

    Поправки на привычный ввод общие с normalize_phone: иначе «8 707…» дал бы
    хвост от восьмёрки, а в базе лежит семёрка, и промах выглядел бы как
    «водителя нет».
    """
    digits = _phone_digits(raw)
    if not digits or not size <= len(digits) <= MAX_PHONE_DIGITS:
        return None
    return digits[-size:]
...
def phone_variants(phone):
    ...
    Вендор кладёт телефон без приведения, поэтому рядом с 77071234567 может
    лежать 87071234567 или +77071234567. Ищем по всем правдоподобным записям
    сразу, а не заводим колонку-дубль ради одного экрана.
    ...
    if len(normalized) == 11 and normalized[0] == '7':
        # Казахстан и Россия: те же цифры записывают и с «8», и без кода страны.
        tail = normalized[1:]
        out += ['8' + tail, tail]
    elif len(normalized) == 10:
        # Ввели номер без кода страны, и это не казахстанский «7…»: пробуем
        # дописать код так, как его пишут у нас.
        out += ['7' + normalized, '8' + normalized, '+7' + normalized]
    seen = set()
...
# Сколько обращений к вендору делаем, прежде чем сказать «не нашли». Потолок
# стоит ради общей месячной квоты.
MAX_CLIENT_LOOKUPS = 3


def client_lookups(phone):
    """Порядок, в котором спрашиваем вендора о клиенте: [(фильтр, номер), …].

    НОМЕР У ВЕНДОРА ЛЕЖИТ В ОДНОМ ИЗ ДВУХ ПОЛЕЙ (проверено живьём 17.09.2026):

    * обычный клиент — `phone` = «77000000105», а `client_phone` бывает пустым:
      фильтр `client_phone` такого клиента НЕ находит;
    * клиент WhatsApp, пришедший идентификатором вместо номера, —
      `phone` = «[wa_gupshup] KZ.1000000000000001», а номер только в
      `client_phone`. Фильтр `phone` такого водителя не находит НИКОГДА.

    Второй вид появился 02.09.2026 сразу у всех парков, то есть это WhatsApp, а
    не настройка канала: у «Ноль такси» так приходит каждое шестое обращение
    (16 %), у iTaxi и Jana — каждое восьмое (13 %). Один такой номер три
    оператора искали 24 раза и получали «не найдено», а у вендора за те же
    сутки лежал 31 сообщение водителя в «Ноль такси».

    Порядок — по частоте попадания: номер целиком в `phone`, он же в
    `client_phone`, потом прочие записи. Записи с «+» и «8» идут последними и
    упираются в потолок: из 16 664 клиентов c2d_requests (45 суток) ни у одного
    номер так не записан.
    """
    variants = phone_variants(phone)
    if not variants:
        return []
    lookups = [('phone', variants[0]), ('client_phone', variants[0])]
    lookups += [('phone', candidate) for candidate in variants[1:]]
    return lookups[:MAX_CLIENT_LOOKUPS]


def _row_has_number(row, candidate):
    """Отдал ли вендор клиента именно с этим номером.

    Неизвестный фильтр вендор молча игнорирует и отдаёт всех клиентов компании
    подряд (так уже было с `search` и `query`). Возьми мы тогда первую строку —
    оператор увидел бы переписку чужого водителя. Поэтому совпадение номера
    проверяем сами, а не верим фильтру.
    """
    wanted = re.sub(r'\D', '', str(candidate or ''))
    if not wanted:
        return False
    return any(re.sub(r'\D', '', str(row.get(field) or '')) == wanted
               for field in ('phone', 'client_phone'))


def find_client(phone):
    """{'id', 'name', 'phone'} или None. От одного до трёх вызовов API.

    Фильтры `phone` и `client_phone` работают и оба точные; `search` и `query`
    вендор молча игнорирует и отдаёт всех 663 тыс. клиентов, что однажды уже
    приняли за «поиск не нашёл». В каком поле искать и в каком порядке — см.
    client_lookups.
    """
    for field, candidate in client_lookups(phone):
        payload = _request('GET', '/v1/clients', params={field: candidate, 'limit': 2})
        row = next((r for r in (payload.get('data') or [])
                    if isinstance(r, dict) and _row_has_number(r, candidate)), None)
        if not row:
            continue
        return {
            'id': row.get('id'),
            'name': clean_client_name(row.get('name')),
            # Отдаём номер цифрами, а не идентификатор мессенджера из `phone`:
            # под ним ложатся кеш и журнал, и повторный поиск того же водителя
            # находит его уже в своей памяти, без вызова вендора.
            'phone': candidate.lstrip('+'),
        }
    return None
