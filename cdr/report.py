...
COLUMNS = (
    ...
    ('queue', 'Очередь', 10),
    # Наш номер: входящему его набрал клиент, исходящему с него позвонили. Парк — по
    # очереди, куда станция ведёт входящие на этот номер (cdr/lines.py).
    ('line_number', 'Номер парка', 14),
...
def _fill_context(sheet, period_from, period_to, summary, generated_at,
    ...
    lines = [
        ...
        ('Не дошли до очереди (в счёт касаний не входят)', summary.get('before_queue', 0)),
        ...
        ('«Не дошёл до очереди» — клиент положил трубку на приветствии или в меню.',
         'Оператору такой звонок не поступал, поэтому он есть строкой на листе «Касания», '
         'но не входит ни в «Касаний», ни в листы «Операторы», «По дням» и «Сводка». '
         '«Приветствие/IVR, с» у него — сколько человек слушал до того, как положить трубку.'),
        ...
        ('«Номер парка» — наш номер в этом звонке.',
...
def _fill_summary(sheet, summary, by_type, by_result, operators):
    ...
    block('ГЛАВНОЕ', [
        ('Касаний', total),
        ('Не дошли до очереди (не входят в касания)', summary.get('before_queue', 0)),
