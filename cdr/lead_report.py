...
_TOUCHES_SHEET = (
    ('ID карточки', 12), ('Телефон', 13), ('Тип лида', 10), ('Статус / этап', 30),
    ('Дата карточки', 17), ('№ касания', 9), ('Дата и время', 17), ('Ответили в', 17),
...
def build_context(rows, *, source_info, period_from, period_to, touches_to, summary, blocks,
    ...
    subject_gen = source_info.get('subject_gen') or subject
    subject_dat = source_info.get('subject_dat') or subject
    ...
    binding = (
        ...
        'за %d минуты до появления %s в системе и раньше появления следующей карточки с тем же '
        'номером. Окна соседних карточек с одним номером стыкуются встык и не перекрываются, '
        ...
        'трубку, карточка появляется через несколько секунд (медиана опережения 10 с). Колонка '
        '«Дубль телефона» показывает, что карточек с этим номером несколько: «2 из 3» — вторая '
        'по времени из трёх.' % (subject_dat, grace_minutes, subject_gen, grace_minutes))
    if no_window:
        binding += (' В ЭТОМ файле окна по дате отключены для карточек с уникальным номером: к ним '
                    'отнесены все звонки по номеру за весь период звонков, включая сделанные до '
                    'появления карточки. У карточек с дублем номера окна сохранены.')
    traps = []
    if filters_note:
        traps.append('Файл собран с фильтрами: %s. Это не весь поток, а его срез.' % filters_note)
    if own_only:
        traps.append('В файле ТОЛЬКО звонки ответственного по каждой карточке. Звонки коллег по тем '
    ...
    traps.append(
        'Окно наблюдения у карточек разной длины: звонки взяты по %s включительно, поэтому у ранних '
        'карточек на дозвон было больше времени, чем у поздних. Сравнивать число касаний по дням '
    ...
    traps.append('Записи разговоров на портал не скачиваются — в файле только ссылки на сервер '
                 'записей, и открываются они из внутренней сети компании.')
    ...
    if missing_lead_days:
        traps.append('За %d суток периода %s в снимке нет (%s): либо их не было, '
                     'либо ночная выгрузка «Воронки ОП» ещё не прошла — она идёт в 05:20 за вчера.'
                     % (len(missing_lead_days), subjects,
    ...
    return {
        'прочти_первым': (
            'Это выгрузка %s отдела продаж («%s»), к каждой карточке приклеены все звонки отдела по '
            'её номеру телефона: кто звонил, когда, дозвонился ли, сколько говорили и где лежит '
            'запись разговора. Ниже — что это за данные, что означает каждая колонка и чему в '
            'цифрах верить нельзя. Лист «Лиды» широкий, по одной карточке в строке; лист «Касания» '
            '— по одному звонку в строке, для сводных и подсчётов.' % (subjects, source_info['label'])),
        'что_это_за_файл': (
            'Компания управляет сетью таксопарков в Казахстане и подключает водителей к работе на '
            'заказах агрегатора. Источник карточек — %s; момент появления карточки — %s. В файле %d %s за '
        ...
        'что_такое_касание': (
            'Касание — один звонок между отделом продаж и человеком из карточки. Это не строка '
            'телефонной станции: у звонка в очередь строк бывает десятки, они склеены по linkedid. '
            'Касания пронумерованы по времени: «Касание 1» — первый звонок в окне карточки.'),
        ...
        'колонки_сводки': {
            'Касаний всего': 'сколько звонков ОП было по карточке в её окне',
            'Исходящих / Входящих': 'те же касания по инициатору',
            'Разговоров': 'сколько касаний закончились разговором (длительность > 0)',
            'Разговор всего, с': 'суммарное время разговоров по карточке',
            'Первое касание / Последнее касание': 'крайние по времени касания',
            'Первый исходящий ОП': 'первый звонок именно со стороны отдела',
            'Реакция ОП, мин': 'минут от появления карточки до первого исходящего; отрицательное — '
                               'оператор набрал номер раньше, чем завёл карточку',
            'Кто работал с лидом': 'все операторы, касавшиеся карточки, в порядке появления',
            'Дубль телефона': '«2 из 3» — вторая по времени карточка с этим номером из трёх',
            'Касаний до заявки': 'звонки, начавшиеся до появления карточки (в пределах допуска)',
            'Ответственный звонил': 'да / нет — звонил ли закреплённый за карточкой сотрудник; пусто, '
        ...
        'на_что_файл_отвечает': [
            ...
            'сколько времени прошло от появления карточки до первого звонка и как это отличается по дням, паркам, источникам',
            'сколько попыток тратится на одну карточку и когда попытки прекращают',
            'какие карточки остались без звонка и чем они отличаются от остальных',
            'звонил ли закреплённый сотрудник сам или лидом занимались другие',
        ],
        'на_что_файл_НЕ_отвечает': [
            'о чём говорили — записи разговоров есть ссылками, расшифровок нет',
        ...
        'цифры': {
            'карточек': total, 'с_касаниями': with_touches,
...
def build_workbook(rows, *, source_info, period_from, period_to, touches_to, summary, blocks,
    ...
    _fill_summary(summary_sheet, summary, rows, source_info)
...
def _fill_context(sheet, context):
    ...
    title = WriteOnlyCell(sheet, value='%s: карточки отдела продаж и касания по ним'
                          % (context.get('источник') or 'Сделки'))
    ...
    for key, value in context['колонки_касания'].items():
        line(key, value)
    sheet.append([])
    section('КОЛОНКИ СВОДКИ ПО КАРТОЧКЕ')
...
def _fill_summary(sheet, summary, rows, source_info=None):
    ...
    total = summary.get('leads', 0) or 0
    subjects = ((source_info or {}).get('subjects') or 'карточек')
    head = subjects[0].upper() + subjects[1:]
    block('ГЛАВНОЕ', [
        ('%s в файле' % head, total),
        ('%s с касаниями ОП' % head, summary.get('with_touches', 0)),
        ('%s без единого звонка' % head, summary.get('without_touches', 0)),
        ('Доля %s с касанием, %%' % subjects, _percent(summary.get('with_touches', 0), total)),
        ...
        ('Касаний до появления карточки', summary.get('before_lead', 0)),
    ...
    block('ПО ТИПУ ЛИДА', [(k, v, t, _percent(t, v)) for k, v, t in summary.get('by_lead_type', [])],
          ('Тип лида', head, 'С касанием', 'Доля, %'))
    block('ПО СТАТУСАМ / ЭТАПАМ', [(k, v, t, _percent(t, v)) for k, v, t in summary.get('by_stage', [])],
          ('Статус', head, 'С касанием', 'Доля, %'))
    block('ПО ПАРКАМ', [(k, v, t, _percent(t, v)) for k, v, t in summary.get('by_park', [])],
          ('Парк', head, 'С касанием', 'Доля, %'))
    block('СКОЛЬКО КАСАНИЙ НА КАРТОЧКУ', [(k, v, _percent(v, total)) for k, v in summary.get('distribution', [])],
          ('Касаний', head, 'Доля, %'))
...
def build_json(rows, *, source_info, period_from, period_to, touches_to, summary, blocks,
    ...
    return {
        ...
        'метаданные': {
            ...
            'период_карточек': '%s - %s' % (_ru_date(period_from), _ru_date(period_to)),
        ...
        'карточки': [_lead_json(row) for row in rows],
    }
