...
--
-- Зачем. Фильтр списка кабинета врёт про часть людей: измерено 01.09.2026, у
-- водителя 273a5c22… карточка говорит «Sapar», а фильтр находит его только под
-- «Бумажным документооборотом», и держит там с 24.08.2026 во всех девяти
-- выгрузках. Единственный источник правды — карточка, а она стоит ОДИН запрос на
-- человека, пачкой кабинет её не отдаёт. На файле в 15 738 строк это 13 220
-- запросов и до 97 минут против нынешних 1 245 запросов и пяти минут.
--
-- Почему кеш решает задачу, а не просто её прячет. Люди в файлах повторяются
-- почти полностью: за 25 выгрузок набралось 17 582 уникальных водителя, а в
-- выгрузке №32 (8 912 строк) НОВЫХ относительно предыдущих двадцати четырёх — НИ
-- ОДНОГО. Значит цену подтверждения платит первый прогон, а следующие спрашивают
-- только новых и просроченных. Без этого раздел стал бы неработоспособен: он
-- запускается по нескольку раз в день (31.08.2026 — четыре выгрузки), а
-- has_running_job блокирует остальные на всё время прогона.
--
-- checked_at — не украшение, а срок годности: провайдера меняют, и за 12 дней
-- истории список зафиксировал 1 774 перехода с «бумажного» на реального. Строку
-- старше срока спрашиваем заново.
CREATE TABLE IF NOT EXISTS fleet_edm_card_providers (
...
CREATE INDEX IF NOT EXISTS fleet_edm_card_providers_checked_idx
    ON fleet_edm_card_providers (checked_at);
