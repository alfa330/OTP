...
def orphan_jobs(cursor, instance_id, silence_seconds=90, own_silence_seconds=600,
    ...
    * свой owner_instance + десять минут молчания — это уже наш собственный
    ...
    Про попытки. Каждый подхват увеличивает attempts, и порог подняли с двенадцати
    до сорока 01.09.2026 — потому что рассуждение под старым числом перестало быть
    верным. Оно звучало так: «двенадцать заведомо больше, чем деплоев успевает
    случиться за одну выгрузку, ведь самая долгая укладывается в четверть часа».
    Укладывалась — пока провайдера брали только из списка. С подтверждением по
    карточкам (см. engine.VERIFY_BY_CARD) выгрузка на 25 126 строк идёт час-два:
    карточка стоит один запрос на человека, а кабинет под нагрузкой отдаёт их
    десятками в минуту. За это время деплоев набегает столько же, сколько раньше
    за всю жизнь выгрузки: 01.09.2026 у живой выгрузки №36 набралось четыре
    попытки за полчаса, и все четыре — чужие деплои, а не сбои.

    Порог всё равно нужен: он ловит выгрузку, которая падает всегда в одном и том
    же месте (такой клинч случился в тот же день — см. card_task в engine.py).
    Но ловить он должен настоящее залипание, а не обычный рабочий день с деплоями.
    Сорок — это примерно шесть часов при нынешнем темпе перезапусков.
    """
    cursor.execute(
        """
        UPDATE fleet_edm_jobs
