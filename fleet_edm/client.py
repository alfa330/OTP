...
# вверх — по чуть-чуть).
#
# Было двести — и это оказалось верно только для коротких прогонов. На длинном
# проходе карточками (01.09.2026, выгрузка №36 на 25 126 строк) кабинет отбивал
# примерно каждый второй запрос, то есть 429 приходил каждые несколько секунд, и
# двухсот чистых ответов ПОДРЯД не набиралось НИКОГДА. Пул навсегда оседал на
# полу в два потока: темп 20 карточек в минуту вместо возможных пятидесяти, и
# выгрузка переставала укладываться в промежуток между деплоями.
#
# Сорок — это по-прежнему больше минуты спокойной работы, но уже достижимо между
# эпизодами отказов. Вниз клиент всё так же уезжает мгновенно, так что кабинет от
# этого не страдает: изменилась только скорость возвращения, а не агрессивность.
THROTTLE_RECOVERY_AFTER = 40
    ...
    def _request(self, method, path, *, park_id, body=None, attempts=7):
        ...
        for attempt in range(1, attempts + 1):
            ...
            try:
                ...
                # Вместо JSON пришло что-то другое — и это ДВА РАЗНЫХ случая,
                # которые нельзя путать. Протухшая сессия выглядит строго так:
                # код 200 и HTML страницы входа Яндекс ID (измерено). А вот
                # неверно собранный запрос отвечает 400 с ПУСТЫМ телом
                # (проверено 01.09.2026: driver_ids вместо driver_id →
                # 400, content-type application/octet-stream, length 0).
                # Раньше оба случая говорили «сессия протухла, нужен новый
                # вход» — то есть человека отправляли логиниться заново из-за
                # нашей же ошибки в теле запроса.
                ...
                raise FleetError(
                    'Fleet вернул {} без JSON (тело {} байт, content-type {}) — '
                    'похоже, запрос собран неверно, а не сессия протухла'.format(
                        response.status_code, len(response.content or b''),
                        response.headers.get('content-type') or 'не указан'))
