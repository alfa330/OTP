...
# Колонки, появившиеся после первого выката: CREATE TABLE IF NOT EXISTS
# существующую таблицу не трогает. Выполняются между таблицами и индексами —
# порядок задан в init_trainer_schema и объяснён там же.
_MIGRATIONS: list[str] = [
    # Что это за реплика собеседника: answer / clarify / no_answer. Без неё
    # нельзя ни объяснить в журнале, почему наставник переспросил, ни запретить
    # ему переспрашивать дважды подряд — правило вики смотрит на вид ПРЕДЫДУЩЕЙ
    # реплики, а до 22.08.2026 вид нигде не сохранялся.
    "ALTER TABLE trainer_turns ADD COLUMN IF NOT EXISTS kind TEXT",
    # Сколько браузер ждал продолжения фразы, прежде чем отправить её. Отдельно
