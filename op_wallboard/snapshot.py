...
def touch_group(touch, ext_group, queue_owners):
    ...
    Потерянный с номером идёт по номеру, а НЕ к хозяину очереди — и это намеренно (владелец,
    23.09.2026). Разница «Все» против суммы групп — сигнал: кто-то без группы сидит во входящих
    очередях. В тот день так нашлась Перизат Нуржан (вн. 6661): очередь звонила ей 1943 раза и
    ни разу не дозвонилась, шесть потерянных «Основы» ушли к ней. Отнесение к хозяину очереди
    прятало такие случаи и было откачено тем же днём."""
    ext = str(touch.get('ext') or '')
    if ext:
