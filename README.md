...
ФИО, город, число опозданий и минуты, минуты раннего ухода и подозрительные
отметки. Считается по той же истории отбивок (`glb_events`), поэтому цифры сходятся
с «Обзором»; сотрудники, у которых за период были только неявки и отсутствие
отметки об уходе, в таблицу не попадают и считаются отдельной строкой под ней.
Город берётся из кадровой карточки (`users.city`) по ФИО — связи между сотрудником
Workpace и нашим `users` нет, поэтому написание складывается (казахские буквы к
русским, отчество необязательно), а тёзкам с разными городами город не проставляется.
Колонка «Город» появляется только у отделов, где он заполнен, — сейчас это «Регионы»
(у нас — «Фронт офисы»).
