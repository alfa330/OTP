...
2. ЗНАК ЗАНИМАЕТ 56 % ПЛИТКИ. Иконка объявлена maskable: Android обрезает её
   под форму своей темы (круг, «капля», квадрат со скруглением) и гарантирует
   неприкосновенным только центральный круг диаметром 80 %. При 56 % сам знак
   (он круглый) укладывается в этот круг с запасом — та же картинка честно
   работает и как maskable, и как обычная, поэтому файлов три, а не шесть.
...
MARK_SCALE = 0.56                 # доля плитки под знак (см. пункт 2 в шапке)
