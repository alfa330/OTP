...
- Нельзя завести запрос там, где спрашивать не у кого (своя инициатива:
  `created_by == assigned_to` и поручителя нет) — `NO_ONE_TO_ASK`.
...
- Смена исполнителя (`PATCH /api/tasks/<id>`) снимает открытый запрос: вопрос от
  человека, которого на задаче уже нет, закрыть некому.
