...
test('вечерняя статья не уезжает на следующий день', () => {
  /* Главная ловушка этой даты. В базе лежит время Алматы без пояса, а сервер
     подписывает его как GMT — значит браузер в Алматы прибавит пять часов.
     Статья 1021 боевой базы заведена 11.09 в 19:54, и как момент времени она
     читается уже двенадцатым числом. */
  assert.equal(addedOn('Fri, 11 Sep 2026 19:54:25 GMT'), 'добавлена 11.09.26');
  assert.equal(addedOn('2026-09-11T23:30:00Z'), 'добавлена 11.09.26');
});

test('строку без пояса разбираем как местную — её так разбирает и сам Date', () => {
  assert.equal(addedOn('2026-09-11T19:54:25'), 'добавлена 11.09.26');
});
