...
class TezOpHoursFrontendTests(unittest.TestCase):
    ...
    def test_success_rows_show_count_with_completion_and_footer_keeps_all_totals(self):
        ...
        # Задача #329 (2026-09-16): их развели по трём колонкам — «Успешки»,
        # «Выполнение», «Успешки в час», — а подписи ушли в шапку.
        ...
        self.assertIn("Успешки", header)
        self.assertIn("Выполнение", header)
        self.assertIn("Успешки в час", header)
        self.assertEqual(header.count("hoursSummaryColClass"), 3)
        ...
        self.assertEqual(rows.count("hoursSummaryColClass"), 3)
        ...
        # Колонок в итоге ровно столько же, сколько в шапке и в строке.
        self.assertEqual(footer.count("hoursSummaryColClass"), 3)
