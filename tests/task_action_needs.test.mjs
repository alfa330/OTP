...
test('спрашивавшему его же вопрос не показывают', () => {
  /* Отсекается именно АВТОР вопроса. Раньше здесь проверялось «я исполнитель», и
     фикстура была сама себе противоречива: исполнитель ME, а автор вопроса —
     WORKER, которого в задаче нет. Спросить может только исполнитель, поэтому в
     честном сценарии автор — тот же ME. */
  const selfTask = ownedTask({
    assignee: { id: ME, name: 'Я' },
    info_request: { id: 501, author_id: ME, author_name: 'Я', body: 'нет доступа' },
  });
  assert.notEqual(taskActionNeed(selfTask, ME, NOW)?.kind, 'info');

  /* А вот случай, ради которого правило и переписано: постановщик вписал себя
     соисполнителем, спросил КОЛЛЕГА — вопрос обязан дойти. Прежнее «я
     исполнитель» прятало его, и отвечать было некому. */
  const shared = ownedTask({
    assignee: { id: ME, name: 'Я' },
    assignees: [{ id: ME, name: 'Я' }, { id: WORKER, name: 'Исполнитель' }],
  });
  assert.equal(taskActionNeed(shared, ME, NOW)?.kind, 'info');
});
