...
class ComposeTest(unittest.TestCase):
    ...
    def test_refusal_in_caveat_words_counts_only_when_nothing_else_is_said(self):
        """«В показанных фрагментах этого нет» — оговорка частичного ответа по
        правилу 4 промпта, но теми же словами модель отказывает и целиком.

        Формулировки — с прода (замер 15.09.2026). Нераспознанный отказ не уходит
        супервайзеру («Вопросы операторов»), а ложный отказ отнимает у полезного
        ответа источники и зря будит супервайзера — поэтому обе стороны здесь.
        """
        refusals = (
            'В представленных фрагментах нет сведений о том, какой приоритет '
            'начисляется за год эксплуатации машины.',
            'В показанных мне фрагментах информации о выдаче детского автокресла парком нет.',
            'В предоставленных фрагментах нет информации о том, кто является автором '
            'вопроса. Статья содержит только структуру компании.',
            'Қолжетімді мақала үзінділерінде Kaspi аударымдары туралы ақпарат жоқ. '
            'Бұл сұрақ бойынша қаржы бөліміне жүгініңіз.',
        )
        answers = (
            # Оговорка + смежные сведения списком — это ответ.
            'В предоставленных фрагментах статей нет информации о кондиционере в «Эконом». '
            'Для тарифа **«Комфорт»**:\n* Водитель обязан реагировать на просьбы пассажира.',
            'Как поменять провайдера, в показанных фрагментах нет — только упоминание '
            'даты смены. Что есть по смежной теме:\n- При ошибке проверьте дату.',
            # Оговорка не в начале — ответ уже дан.
            'В городе Балхаш можно работать по тарифу «Межгород». Офисов в Балхаше '
            'в показанных фрагментах нет.',
            'Қолжетімді мақала үзінділерінде бұл туралы ақпарат жоқ. Көрсетілген '
            'материалдарда тек мынадай мәліметтер бар:\n* Күндізгі тексеріс',
            # Число внутри той же фразы — частичный ответ.
            'В показанных фрагментах нет срока, но залог — 5000 тенге.',
        )
        for text in refusals:
            self.assertTrue(ai_answer.is_refusal(text), text[:60])
        for text in answers:
            self.assertFalse(ai_answer.is_refusal(text), text[:60])
