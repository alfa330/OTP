...
class BackGestureTests(unittest.TestCase):
    ...
    def test_entries_never_outgrow_the_open_layers(self):
        """ЗАПИСЕЙ РОВНО СТОЛЬКО, СКОЛЬКО ОТКРЫТЫХ СЛОЁВ.

        Публикация на Pages подменяет сборку разом, у открытого приложения
        остаётся прежняя, и портал лечится перезагрузкой; записи, положенные ДО
        неё, принадлежат ПРЕЖНЕМУ документу, и шаг назад в них грузит приложение
        заново. Но это про записи НИЖЕ дна — а дно кладётся сразу после
        перезагрузки, и ниже него жест не пускает. Наши записи всегда выше дна и
        всегда принадлежат живому документу.

        Расти им тоже некуда: сверка кладёт запись только на слой, которого ещё
    ...
    def test_one_entry_per_layer_because_of_the_ios_snapshot(self):
        """У КАЖДОГО СЛОЯ СВОЯ ЗАПИСЬ ИСТОРИИ, И РЕШАЕТ ЭТО НЕ УДОБСТВО, А iOS.

        Во время маха пальцем от края система показывает не живую страницу, а
        СНИМОК той записи, куда человек возвращается. Снимок делается в момент
        ухода с записи и больше не обновляется.

        Пока запись была одна на все слои, снимок у неё был один — сделанный
        тогда, когда открылся ПЕРВЫЙ слой, то есть с разделом. Владелец
        11.09.2026 прислал видео: экран «Пароль» уезжает вправо, из-под него
        выезжает раздел «Супервайзеры», и лишь через треть секунды появляется
        живой профиль. Со стороны — «свайп перекидывает в другой раздел и
        обратно в профиль». В замерах состояния этого не видно ВООБЩЕ: DOM и
        адрес всё это время правильные, потому что показанное системой к DOM
        отношения не имеет.
    ...
    def test_counter_decides_not_the_state_of_the_entry(self):
        ...
        11.09.2026: после захода в «Курсы» первый шаг назад приходил «на чужую
        запись», мы возвращали дно и не закрывали ничего — «назад» из Курсов
        переставал работать с первого раза. Прод в тот же момент возвращал в
        «Задачи» с первого нажатия, то есть это был бы регресс.

        Счёт живёт в переменной модуля: его не сотрёт никто."""
        pop = self.STACK[self.STACK.index('const handlePop'):self.STACK.index('const listen')]
        self.assertIn('if (!guards.length) {', pop)
        self.assertIn('guards.pop();', pop)
        self.assertNotIn('otpSeq ===', pop)

    def test_floor_survives_a_two_step_swipe(self):
        """НИЖЕ ДНА ЖЕСТ НЕ ПУСКАЕТ.

        Портал — одно приложение на одной странице, и шаг ниже его корня выводит
        из ДОКУМЕНТА: в установленном портале нет ни адресной строки, ни кнопки
        «вперёд» — только закрыть приложение и открыть заново. Владелец
        11.09.2026: «при входе на разделы и назад сайт просто зависает».

        Признак «мы ниже дна» — свои записи кончились."""
        pop = self.STACK[self.STACK.index('const handlePop'):self.STACK.index('const listen')]
        self.assertIn('if (!guards.length) {', pop)
        self.assertIn('armRoot();', pop)
        # И дно под записями обязано быть заведено до первой из них.
        self.assertIn('if (!guards.length && !standingOnFloor()) armRoot();', self.STACK)

    def test_pop_never_lets_a_foreign_address_through(self):
        """АДРЕС В НАШИХ ЗАПИСЯХ ЗАМЕРЗАЕТ, И ЭТО ОН «ПЕРЕКИДЫВАЛ В ДРУГОЙ РАЗДЕЛ».

        Наши записи держат тот адрес, который был в строке в момент их
        появления: сменив раздел, портал правит адрес только у ТЕКУЩЕЙ записи,
        до лежащих ниже ему не дотянуться. Плёнка history с самого входа
        (замер 11.09.2026): дно получает `?view=wiki` в ту секунду, когда
        человек первый раз уходит из стартового раздела, и держит его весь
        сеанс; дальше любой шаг назад приземляется на дно, браузер
        восстанавливает его адрес, роутер читает и уводит портал в Вики."""
        self.assertIn('const restoreOwnUrl = () => {', self.STACK)
        pop = self.STACK[self.STACK.index('const handlePop'):self.STACK.index('const listen')]
        self.assertIn('restoreOwnUrl();', pop)
        # ПЕРВОЙ строкой обработчика: следующим адрес читает роутер.
        self.assertLess(pop.index('restoreOwnUrl();'), pop.index('if (!guards.length)'))
    ...
    def test_window_never_swaps_the_section_under_itself(self):
        ...
        Страховка общая и на всё, что напишут дальше."""
