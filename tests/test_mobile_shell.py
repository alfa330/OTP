...
class BackGestureTests(unittest.TestCase):
    ...
    def test_pop_never_lets_a_foreign_address_through(self):
        ...
        Дно и сторожевая держат тот адрес, который был в строке в момент их
        ...
        восстанавливает его адрес, роутер читает и уводит портал в Вики — хотя
        жест закрывал экран поверх профиля.

        Замер до правки: стоим в «Задачах» → открыли профиль → мах → оказались
        в «Вики» без профиля. После: остаёмся в «Задачах», мах закрывает ровно
        один слой. Владелец 11.09.2026: «свайпы всё ещё перекидывает в другой
        раздел, а не профиль» — это была третья его жалоба об одном и том же.

        Отсюда же росли и «пролаги»: подмена раздела тянет полную перерисовку
        чужого раздела на каждый жест."""
