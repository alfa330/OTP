...
test('салон виден только через открытую дверь и только вблизи', () => {
    const closed = readFrame({ azimuth: 250, distance: 2.4 }, {});
    assert.equal(closed.view, 'left', 'дверь закрыта — это просто левый борт');

    const opened = readFrame({ azimuth: 250, distance: 2.4 }, { doorFrontLeft: true });
    assert.equal(opened.view, 'inside_front');

    const faraway = readFrame({ azimuth: 250, distance: 6 }, { doorFrontLeft: true });
    assert.equal(faraway.view, 'left', 'с шести метров в проём не снять — это борт');
});

test('открытая дверь перебивает борт, а не наоборот', () => {
    // Азимут 250° попадает и в сектор левого борта, и в сектор салона.
    // Человек, подошедший вплотную к раскрытой двери, снимает салон.
    const frame = readFrame({ azimuth: 250, distance: 2.0 }, { doorFrontLeft: true });
    assert.equal(frame.view, 'inside_front');
});

test('водительская дверь ближе к носу, задняя — к корме', () => {
    /* Нос машины на 180°, корма на 0°. Значит у левого борта 250° — это
       водительская дверь, а 294° — задняя. Перепутанные местами, эти секторы
       засчитывали «передний ряд» человеку, стоящему у задней двери. */
    assert.equal(targetAzimuth('inside_front'), 250);
    assert.equal(targetAzimuth('inside_rear'), 294);
    assert.ok(azimuthGap(targetAzimuth('inside_front'), 180)
        < azimuthGap(targetAzimuth('inside_rear'), 180),
        'передняя дверь обязана быть ближе к носу, чем задняя');
});

test('задняя дверь и багажник имеют свои сектора', () => {
    assert.equal(readFrame({ azimuth: 294, distance: 2.4 }, { doorRearLeft: true }).view,
        'inside_rear');
...
test('вплотную к салону — тоже брак', () => {
    const frame = readFrame({ azimuth: 250, distance: INSIDE_RANGE.near - 0.2 },
        { doorFrontLeft: true });
    assert.equal(frame.view, 'inside_front');
