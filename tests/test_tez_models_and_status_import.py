...
class TezBinotelRandomCallTests(unittest.TestCase):
    ...
    def test_normalize_call_exposes_employee_and_recording(self):
        raw = {
            ...
            "internalNumber": "907", "externalNumber": "77000000111",
            "disposition": "answer", "recordingStatus": "uploaded",
            "employeeData": {"name": "Тестбаева Жасмин Тесткызы",
                             "email": "operator94@example.com"},
        }
        ...
        self.assertEqual(c["employee_name"], "Тестбаева Жасмин Тесткызы")
        self.assertEqual(c["employee_email"], "operator94@example.com")
    ...
    def test_shared_sip_is_disambiguated_by_name(self):
        ...
        ops = [
            {"id": 365, "name": "Тестбаева Акерке Тесткызы"},   # sip 925
            {"id": 367, "name": "Сынакова Аружан Тесткызы"},  # sip 925 (тот же!)
            {"id": 337, "name": "Тестбаева Жасмин Тесткызы"},  # sip 907
        ]
        lookup = _build_binotel_operator_lookup(self.ns, ops)
        resolve = self.ns["_status_import_resolve_operator_matches"]
        self.assertEqual([o["id"] for o in resolve("Тестбаева Акерке Тесткызы", lookup)], [365])
        self.assertEqual([o["id"] for o in resolve("Сынакова Аружан Тесткызы", lookup)], [367])
        self.assertEqual([o["id"] for o in resolve("Тестбаева Жасмин Тесткызы", lookup)], [337])
