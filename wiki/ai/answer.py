...
# Отказ, сказанный словами ОГОВОРКИ. Промпт сам велит модели говорить «в
# показанных мне фрагментах этого нет» про ЧАСТЬ вопроса внутри ответа (правило
# 4), и ровно этими словами она иногда отказывает целиком. С «Вопросами
# операторов» это перестало быть косметикой: нераспознанный отказ не уходит
# супервайзеру (wiki/questions.py), и оператор остаётся без ответа молча.
#
# Поэтому признак строже, чем у «доступн…»: фраза стоит В НАЧАЛЕ, а во всём
# тексте нет ни числа, ни списка, ни таблицы. Замер на проде 15.09.2026, 488
# ответов за 60 дней: тот же широкий признак, что у «доступн…», перевёл бы в
# отказы 7 ответов, из них 4 полезных (таблица по Балхашу, смежные правила
# списком); строгий — ровно 3 настоящих отказа. Казахский — тем же правилом:
# из 4 ответов с «жоқ» отказом был один, три других — ответы со списком.
_CAVEAT_REFUSAL = re.compile(
    r'^\s*[^.!?\n]*(?:показанн|представленн|предоставленн|переданн|приведённ|приведенн'
    r'|қолжетімді|көрсетілген|берілген)[^.!?\n]*\b(?:нет|жоқ)\b[^.!?\n]*[.!?]?', re.I)
_CONTENT_MARKUP = re.compile(r'(?m)^\s*(?:[-*•]|\d+[.)])\s|\||\*\*')

# Ответ, который НАЧИНАЕТСЯ с признания «нет информации о …», а дальше даёт смежное
# (правило 4 промпта: «скажи, чего именно нет, и сразу дай то, что есть»). Это не
# отказ — источники и смежные данные остаются, — но и не ответ на заданный вопрос,
# поэтому такой вопрос тоже уходит супервайзеру (wiki/questions.py: should_escalate;
# решение по задаче #321 от 15.09.2026 — оператор спросил комиссию Яндекса, получил
# таблицу комиссий парков, а к супервайзеру вопрос не ушёл).
#
# Признание ищется только в ПЕРВОЙ фразе и только в паре «где искали» + «сведений
# нет»: одно слово «нет» бывает и ответом («штрафа за это нет»). Замер на проде
# 15.09.2026 за 60 дней: у операторов и стажёров 68 полных отказов и ещё 63 таких
# ответов — почти все про настоящие дыры в базе знаний (комиссии Яндекса и парков,
# модели авто по тарифам, адреса офисов).
_WHERE_LOOKED = (r'(?:доступн|показанн|представленн|предоставленн|переданн|приведённ'
                 r'|приведенн|фрагмент|стать|материал)')
_ADMITS_MISSING = re.compile(
    r'^\s*[^.!?\n]*' + _WHERE_LOOKED + r'[^.!?\n]*(?:'
    r'\b(?:нет|отсутству\w*)\b[^.!?\n]*(?:информац|сведени|данн)'
    r'|(?:информац|сведени|данн)[^.!?\n]*\b(?:нет|отсутству\w*)\b)'
    r'|^\s*[^.!?\n]*(?:қолжетімді|берілген|көрсетілген|үзінді)[^.!?\n]*ақпарат[^.!?\n]*\bжоқ\b',
    re.I)


def admits_missing(body):
    """Начинается ли ответ с признания, что спрошенного в статьях нет.

    Отказом это не делает — см. is_refusal: смежные данные с числами остаются
    ответом со своими источниками. Признак отвечает на другой вопрос: ушёл ли
    заданный вопрос без ответа и нужен ли по нему супервайзер.
    """
    return bool(_ADMITS_MISSING.search(str(body or '')))
...
def is_refusal(body):
    ...
    Второй признак — отказ словами оговорки («в показанных фрагментах нет…»),
    строже первого; почему — у _CAVEAT_REFUSAL.
    """
    text = str(body or '').strip()
    if not text:
        return True
    caveat = _CAVEAT_REFUSAL.match(text)
    if (caveat and not _HAS_NUMBER.search(text)
            and not _CONTENT_MARKUP.search(text[caveat.end():])):
        return True
