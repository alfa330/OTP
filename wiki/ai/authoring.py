...
# ПОЧЕМУ «ПЕРЕНОСИШЬ», А НЕ «ПРИВОДИШЬ К ВИДУ СТАТЬИ» (замеры 02.09.2026).
# Здесь стояло «даёшь разделам заголовки, убираешь служебный мусор документа,
# выстраиваешь порядок изложения», и модель выполняла ИМЕННО ЭТО. Запрет
# «НЕ сокращай» ниже не помогал: он отрицание, а указание убрать мусор —
# утвердительное и конкретное. Выглядело это как «ИИ съедает половину текста»,
# и два дня лечилось сменой моделей, пока владелец не спросил про промпт.
#
# Замер на двух настоящих .docx, по два прогона, vertex:gemini-3-flash-preview.
# Доля переноса = знаки текста статьи / знаки текста документа:
#     прежняя формулировка            0,540  0,604  0,627  0,631
#     эта                             0,902  0,945  1,012  1,014
# На gemini-3.5-flash та же формулировка ничего не испортила: 0,986-1,010.
#
# Служебная шапка в тело НЕ полезла ни в одном из восьми прогонов, хотя указание
# её убирать мы отсюда и убрали: «СПРАВКА ПО РАСХОДАМ» и подпись отдела с датой
# модель отсеивает сама — название она обязана положить в поле НАЗВАНИЕ, а
# подпись сама по себе никуда не встаёт. То есть та фраза была не нужна, а вредна.
#
# Плата: оформление стало скромнее — приёмов из девяти было 6, стало 4-6, обычно
# пропадают карточки. Размен принят владельцем: полный текст важнее карточек, и
# статья дешевеет вчетверо, потому что снова годится самая дешёвая модель.
#
# ВАРИАНТ, КОТОРЫЙ ОТВЕРГНУТ: то же плюс количественное правило «статья не может
# быть короче документа, в нём N знаков». Перенос чуть выше (0,979 против 0,962),
# но оформление садится до 2 видов блоков — правило про объём вытесняет разметку.
SYSTEM_PROMPT = """Ты — редактор корпоративной вики таксопарка. Ты превращаешь загруженный документ в статью вики.

ЧТО ТЫ ДЕЛАЕШЬ
Ты ПЕРЕНОСИШЬ документ в статью целиком и добавляешь разметку. Каждый абзац
документа обязан стать абзацем статьи — в том же порядке и с тем же текстом. Ты
НЕ пересказываешь, НЕ сокращаешь, НЕ объединяешь абзацы и НЕ выбрасываешь «воду»:
решать, что здесь лишнее, не твоя работа. Ничего не добавляй от себя.
