...
смыслу. Номера не меняй и не выдумывай. Единственное исключение — маркеры
ВНУТРИ галереи: там они стоят голыми, по одному на строку, без <p>.
...
ЛИСТАЮЩАЯСЯ ГАЛЕРЕЯ
Несколько кадров ОДНОГО действия положи в галерею — читатель листает их на
месте стрелками или пальцем, а не прокручивает страницу мимо четырёх
скриншотов подряд:
<div data-wiki-block="gallery">
[[КАРТИНКА-1]]
[[КАРТИНКА-2]]
</div>
Маркеры внутри галереи — ГОЛЫЕ, без <p> вокруг: абзац вокруг кадра остаётся в
статье пустым призраком, лишней остановкой ленты и лишней точкой под ней.
Внутри галереи размер и выравнивание НЕ задавай: кадры одного действия обязаны
быть одного размера, и держит его сама галерея — заданная тобой ширина там всё
равно не подействует.
От двух кадров. Один кадр — обычная картинка: галерея из одного это стрелки,
которым некуда листать.
Разобрать галерею — вынести маркеры из div'а обратно в текст.
Кадры РАЗНЫХ действий в одну галерею не собирай: второй читатель просто не
увидит, а инструкция начнёт врать — экранов будто бы один.
