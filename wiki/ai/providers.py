...
#
# Замеры 01.09.2026 на двух настоящих документах, по два прогона на модель.
# Мерились ДВЕ вещи, и модели разошлись по ним в разные стороны.
#
# ПОЛНОТА. Доля переноса = знаки текста статьи / знаки текста документа:
#   «Стоимость ИИ-оценки звонков» (8 328 знаков, 3 таблицы)
#     vertex:gemini-3-flash-preview   0,466  0,474  0,590  0,591
#     zai:glm-5.3-flash               0,991  0,991  0,992  1,009
#   «Смета расшифровки звонков Основа ОП» (7 693 знака, 6 таблиц)
#     vertex:gemini-3-flash-preview   0,616  0,649  0,651
#     zai:glm-5.3-flash               0,992  0,999  1,037
...
# authoring.MAX_OUTPUT_TOKENS). Это не лимит и не наша обработка — пошаговый
# разбор конвейера показал, что после модели теряются только строки
# «НАЗВАНИЕ:/КРАТКО:», которые и должны уйти. Модель пересказывает, хотя
# SYSTEM_PROMPT это прямо запрещает.
#
# ОФОРМЛЕНИЕ. Сколько ВИДОВ блоков разметки модель пускает в дело (в гиде их
# девять, markup.MARKUP_GUIDE), на том же документе и том же промпте:
#     vertex  6 видов — lead, stats, stat, cards, card, note;
#             списки checks, crosses, chips, steps; показатели все числом;
#     zai     2 вида  — lead, note; список только steps.
# GLM повторяется до символа: в обоих прогонах ровно тот же бедный набор, и
# блоки, добавленные в гид в тот же день (stats, crosses, gallery), он не
# трогает вовсе. markup.warnings() этого НЕ ловит: она видит перебор плашек и
# потерю блоков, но не бедность оформления.
#
# Решение владельца 01.09.2026, после того как он посмотрел статьи глазами:
# «glm реально хуже делает стили, надо вернуть gemini». Размен принят сознательно
# и с обеими цифрами на руках: полнота сдана ради оформления.
#
# Про сжатие человек не остаётся в неведении: authoring.structure_warnings при
# доле ниже 0,6 пишет в редактор «Текста в статье заметно меньше, чем в
# документе», и на Vertex это предупреждение срабатывает регулярно.
#
# GLM оставлен ВТОРЫМ: полная статья с бедным оформлением — заготовка, которую
# редактор дооформит, а отказ оставил бы его ни с чем.
#
# ЧТО НЕ ПРОВЕРЕНО и куда копать, если сжатие надоест: варианты усиленного
# промпта мерились ТОЛЬКО на GLM (четыре штуки, доля 0,98-1,02 у всех — там
# поднимать нечего). На Vertex тот же прогон не состоялся, он почти весь ушёл в
# 429. Не исключено, что правило переноса, сформулированное количественно,
# подтянет и его — тогда обе цели сойдутся на одной модели.
_EDITOR_CHAIN = (
    ('vertex', 'gemini-3-flash-preview'),
    ('zai', 'glm-5.3-flash'),
)
