...
generate): помощник отвечает на вопрос своими словами. У РЕДАКТОРА СТАТЕЙ своя —
editor_chain, generate_article: он переносит документ в статью, и цепочка там
короче, потому что бесплатные звенья сборку статьи не тянут (Groq отдаёт 413 на
9 000 токенах вывода, Cloudflare отваливается по таймауту). Чем модели
различаются на этой задаче и почему выбрана именно та — у _EDITOR_CHAIN, с
замерами.
...
ИСКЛЮЧЕНЫ:
  ...
  * gemini-3.6-flash ЧЕРЕЗ КЛЮЧ AI STUDIO (провайдер 'gemini') — мышление не
    гасится (672-1381 токен несмотря на thinkingBudget=0) и 429 «exceeded your
    current quota» на 4-м запросе. Через Vertex та же модель ведёт себя иначе:
    мышление гасится, токенов на него в usage нет вовсе, и она стоит вторым
    звеном в цепочке РЕДАКТОРА (см. _EDITOR_CHAIN). Запрет здесь — про ключ, а
    не про модель.
...
# на вопрос своими словами: там ценится точность формулировки. Редактор ПЕРЕНОСИТ
# документ в статью и оформляет его: там ценятся полнота и разметка, и модели
# расходятся по этим двум меркам сильнее, чем похожи между собой.
#
# МЕРКИ:
#   ПЕРЕНОС    — доля = знаки текста статьи / знаки текста документа. Ниже 0,6
#                authoring.structure_warnings пишет редактору «текста заметно
#                меньше»; около 1,0 значит документ перенесён целиком;
#   ОФОРМЛЕНИЕ — сколько ВИДОВ блоков разметки из девяти (markup.MARKUP_GUIDE)
#                модель пустила в дело.
#
# 16 прогонов 01.09.2026: два настоящих документа («Стоимость ИИ-оценки звонков»
# 8 328 знаков / 3 таблицы и «Смета расшифровки звонков Основа ОП» 7 693 / 6), по
# два прогона, один и тот же SYSTEM_PROMPT:
#
...
# Выдуманных чисел и потерянных таблиц нет ни у кого.
#
# ПОЧЕМУ 3.5-flash: он единственный берёт обе цели сразу — переносит документ
# целиком и при этом дважды из четырёх прогонов дотянулся до тех же ШЕСТИ видов
# блоков, что и gemini-3-flash-preview, включая cards. Заодно самый быстрый: 10 с
# против 15, а сборка держит соединение к БД всё это время (шапка
# wiki/routes_import.py).
#
# Что пробовали до него и почему не подошло:
#   * gemini-3-flash-preview оформляет богато, но ПЕРЕСКАЗЫВАЕТ: останавливается
#     сам на ~1 400-1 500 выходных токенах при потолке 9 000 и finish='STOP'. Это
#     не обрыв и не наша обработка — пошаговый разбор конвейера показал, что после
#     модели теряются только строки «НАЗВАНИЕ:/КРАТКО:», которые и должны уйти.
#     Модель пересказывает, хотя SYSTEM_PROMPT это прямо запрещает;
#   * glm-5.3-flash переносит целиком, но берёт ДВА вида блоков из девяти (lead и
#     note) и повторяется до символа; блоки, добавленные в гид в тот же день
#     (stats, crosses, gallery), не трогает вовсе. Владелец посмотрел статьи
#     глазами: «glm реально хуже делает стили». markup.warnings() этого НЕ ловит —
#     она видит перебор плашек и потерю блоков, но не бедность оформления. То же у
#     glm-5.3 и glm-5.2, и у openai/gpt-5.6-luna через OpenRouter (там один lead);
#   * gemini-3.6-flash ровнее по переносу и на 12 % дешевле, но потолок оформления
#     ниже: всегда 4 вида. Стоит ВТОРЫМ — ближайшая замена при отказе. Прежний
#     запрет на него в списке ИСКЛЮЧЕНЫ относится к ключу AI Studio (мышление не
#     гасилось, 429 на 4-м запросе); через Vertex мышление гасится, токенов на
#     него в usage нет вовсе;
#   * Anthropic и OpenRouter замерить не вышло: на обоих кончились деньги
#     («credit balance is too low» и HTTP 402). Поле открыто, если понадобится.
#
# ЦЕНА. Тариф — каталог биллинга Google (cloudbilling.googleapis.com/v1/services →
# C7E2-9256-1C43/skus, регион Global, наш), $/1М токенов вход/выход: 3.5-flash
# 1,50/9,00; 3.6-flash 1,50/7,50; 3-flash-preview 0,50/3,00; glm-5.3-flash
# 0,075/0,25. Статья дорожает с $0,0067 до $0,0291 — вчетверо, но это $2 против
# $9 в месяц при десяти статьях в день. Дорожает вдвойне: и ставка выше, и
# токенов больше — потому что текст ДЕЙСТВИТЕЛЬНО переносится, а не пересказывается.
#
# GLM оставлен ТРЕТЬИМ, за двумя Gemini: он единственный от другого поставщика, и
# при недоступности Vertex целиком статья всё же соберётся, пусть и бедно.
# ВРЕМЕННО ПЕРВЫМ СТОИТ 3-flash-preview — по указанию владельца 01.09.2026, пока
# идёт замер порога («пока что переключи ИИ на preview обратно»). Числа выше при
# этом не отменяются: на документах в ~8 тыс. знаков он переносит половину, и
# редактор увидит предупреждение «Текста в статье заметно меньше».
#
# Открытый вопрос, ради которого это временно: СЖИМАЕТ ЛИ ОН МАЛЕНЬКИЕ ДОКУМЕНТЫ.
# На обоих замеренных документах (7 693 и 8 328 знаков) он выдал 4 247-5 116
# знаков статьи — почти одинаково, независимо от входа, при 1 350-1 676 выходных
# токенах. Похоже на ПОСТОЯННЫЙ бюджет вывода около 4,5 тыс. знаков: тогда доля
# падает механически как 4500/размер, никакого «порога» у модели нет, а есть
# деление, и до ~4 500 знаков он не сжимает ничего. Если это подтвердится,
# развилка «маленькие — preview, большие — 3.5-flash» имеет смысл; если он
# ужимает и документ в 1 000 знаков — не имеет.
# Известно и обратное ограничение: 3.5-flash держит перенос до 12 тыс. знаков
# (0,977 и 1,005), а на 20 тыс. проседает до 0,889 и 0,844, и оформление там же
# скатывается к двум видам блоков. Потолок есть у обеих моделей, просто разный.
# ЛЕСТНИЦА РАЗМЕРОВ 02.09.2026 объяснила, ЧТО именно ломалось, и ломалась НЕ
# модель. gemini-3-flash-preview держал перенос до двух тысяч знаков и срывался
# дальше (0,58 на восьми тысячах, 0,43 на двенадцати) — выглядело как свойство
# модели, и под это готовилась развилка по размеру документа. Настоящая причина
# была в промпте: он велел «убирать служебный мусор» (разбор — над
# authoring.SYSTEM_PROMPT). С исправленным промптом preview держит 0,902-1,014 на
# тех же восьмитысячных документах, развилка стала не нужна и УБРАНА: двух ветвей
# ради обхода симптома в коде быть не должно.
#
# Порядок ниже — по замерам с ИСПРАВЛЕННЫМ промптом:
#   vertex:gemini-3-flash-preview  перенос 0,968, блоков 4-6, 15 с, $0,0067
#   vertex:gemini-3.5-flash        перенос 1,002, блоков 2-4, 10 с, $0,0291
#   zai:glm-5.3-flash              перенос 0,991, блоков 2,   33 с, $0,0009
# Первым самый дешёвый: он же и оформляет богаче всех. Вторым 3.5-flash — он ровнее
# по переносу, если preview просядет. Третьим GLM: единственный не от Google,
# страхует недоступность Vertex целиком (02.09.2026 Vertex отдавал 403 по биллингу
# проекта, и вика жила именно на этом звене).
_EDITOR_CHAIN = (
    ('vertex', 'gemini-3-flash-preview'),
