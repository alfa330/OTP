...
# ВЛОЖЕНИЯ СТАТЬИ
#
# Файл, приложенный к статье целиком (бланк, форма, презентация), — это не
# картинка внутри текста: его не показывают, а скачивают. Отличать одно от
# другого приходится явно, потому что лежат они в ОДНОЙ таблице: без признака
# список приложений к инструкции собрал бы туда же все иллюстрации из её текста,
# а «убрать приложение» отвязало бы картинку и оставило в статье битый <img>.
#
# Отдельную таблицу заводить не за что: хранилище, права и прокси
# /api/wiki/file/<id> у приложения и картинки одни и те же, а разъехались бы
# ровно те места, которые обязаны работать одинаково.
#
# Порядок задаёт редактор (sort_order), а не время загрузки: приложения к
# регламенту нумерованные, и «Приложение 2», уехавшее выше первого только
# потому, что его перезалили, — это ошибка документа.
# ─────────────────────────────────────────────────────────────────────────────
_FILE_STATEMENTS = [
    "ALTER TABLE wiki_files ADD COLUMN IF NOT EXISTS "
    "is_attachment BOOLEAN NOT NULL DEFAULT FALSE;",
    "ALTER TABLE wiki_files ADD COLUMN IF NOT EXISTS "
    "sort_order INTEGER NOT NULL DEFAULT 0;",
    # Частичный: выборка всегда одна и та же — приложения одной статьи по порядку.
    "CREATE INDEX IF NOT EXISTS idx_wiki_files_attachments "
    "ON wiki_files (article_id, sort_order) WHERE is_attachment;",
]
...
def init_wiki_schema(cursor):
    ...
    # Приложения к статьям — ALTER по wiki_files из блока базовых таблиц.
    for statement in _FILE_STATEMENTS:
        cursor.execute(statement)
