...
SPACE_FEATURES = (
    ...
    # Вкладка «Новости». Новость ПРИНАДЛЕЖИТ пространству, в котором её
    # написали (news_posts.space_id, решение владельца 18.09.2026: «чтобы по
    # пространствам новости Таксопарков и Тез не смешивались»): список
    # редактора, лента и справочник адресата сужены им, а окно за пределами
    # вики сужает сама новость — до людей отделов своего пространства.
    # Тумблер отвечает на другой вопрос — «есть ли эта дверь в ЭТОМ
    # пространстве»: клиентской вике объявления могут быть не нужны вовсе,
    # как не нужен там и «Помощник».
    'news',               # вкладка «Новости»
...
_STATEMENTS = [
    ...
    CREATE TABLE IF NOT EXISTS wiki_section_access_rules (
        ...
        -- Право строить ДЕРЕВО внутри этой ветки: заводить подразделы и править
        -- уже заведённые. Здесь, а не в способностях, потому что способность
        -- глобальна: она открыла бы пространство целиком (см. шапку
        -- wiki/access.py: capabilities_from_grants). Дефолт FALSE — правило,
        -- выписанное до появления колонки, дерева не касалось.
        manage_subsections BOOLEAN NOT NULL DEFAULT FALSE,
...
_OFFICE_STATEMENTS = [
    ...
    # Telegram-ники офиса. Нужны «Обращениям»: бот отмечает ответственных города
    # в группе «iTaxi Вопросы/ответы» (задача #280). Живут здесь, а не в коде
    # раздела, по той же причине, что и телефон офиса, — ник меняет тот, кто его
    # знает, и смена ника не должна стоить выката. Хранятся БЕЗ «@».
    #
    # Ников у офиса бывает несколько (решение владельца 07.09.2026: у Астаны в
    # ТЗ их два), поэтому строка со списком через запятую, а не одна колонка на
    # ник и не отдельная таблица. Запятая однозначна: в нике Telegram её быть не
    # может — только латиница, цифры и «_». Таблица здесь дала бы третью
    # сущность ради двух коротких слов без единого собственного свойства: у ника
    # нет ни записки, ни порядка, ни срока — в отличие от телефонов парка, у
    # которых всё это есть, и потому они и живут таблицей.
    "ALTER TABLE wiki_offices ADD COLUMN IF NOT EXISTS telegram_usernames TEXT;",
...
def _scope_audit_to_space(cursor):
    ...
    Разбор ПО ОБЪЕКТУ идёт на каждом запуске, и это правка от 25.08.2026.
    Раньше он был разовым — из опасения, что деплой начнёт доразмечать записи,
    «ничьи» намеренно. Опасение не оправдалось: разобрать можно только запись,
    объект которой СУЩЕСТВУЕТ, а намеренно ничья — это запись об объекте,
    которого нет (удалён) или не было вовсе (черновик из документа). Такая не
    разберётся никогда, сколько раз ни запусти.

    Зато без повтора на боевой базе залипли записи, у которых пространство
    появилось ПОЗЖЕ: статья создавалась без раздела, раздел ей давали через
    минуту, и запись о создании навсегда оставалась ничьей — то есть висела в
    журнале ОБОИХ пространств. Отсюда и «журналы Теза и Таксопарков
    перемешались». Сам источник закрыт в роутах (пространство теперь называют
    явно), а это — починка того, что уже записано.

    Догадка ПО ДАТЕ — та осталась разовой: она приписывает записи единственному
    пространству, существовавшему в тот момент, и повторять её на каждом
    деплое значило бы раздавать хозяев тому, что осталось без разбора честно.
    """
    cursor.execute(
        """
...
_ORG_STATEMENTS = [
    ...
    #
    # До него дерево разделов правил только носитель способности
    # can_manage_structure, то есть директор и назначенный руками администратор
    # вики. Коммерческому директору выписали личное правило на его ветку со
    # всеми шестью правами и «вместе с подразделами» — и он всё равно не мог
    # завести подраздел: шесть прав правила описывают СОДЕРЖИМОЕ, а
    # capabilities_from_grants намеренно не превращает их в can_manage_*
    # (wiki/access.py). Молчаливый отказ ровно того класса, от которого этот
    # раздел лечили дважды: право выписано, в «Структуре» видно, а кнопки нет.
    #
    # Почему колонкой у ПРАВИЛА, а не способностью должности. Способность
    # глобальна: включив её роли 'admin', мы открыли бы на запись всё
    # пространство «Таксопарки» целиком — ветки чужих отделов, «Старую вики»,
    # корень пространства, — то есть отменили бы решение владельца 21.08.2026
    # («к другому отделу он не может притронуться»). Правило же адресовано
    # разделу, и вместе с ним право уходит ровно в ту ветку, которую выдали.
    #
    # Дефолт FALSE обязателен: правила в проде уже написаны, и миграция не
    # смеет раздать по ним право строить дерево (та же логика, что у
    # copy_protected ниже).
    "ALTER TABLE wiki_section_access_rules ADD COLUMN IF NOT EXISTS "
    "manage_subsections BOOLEAN NOT NULL DEFAULT FALSE;",
...
_SEARCH_LOG_STATEMENTS = [
    ...
    # Запрос был сужен фильтрами (тип документа, создатель, «только в
    # названиях»). Нужен ОТЧЁТУ, а не поиску: главный отчёт вкладки «Аналитика»
    # — «искали и не нашли» — стоит на results_count = 0 и объявлен там
    # единственным источником правды о дырах в базе знаний. С появлением
    # фильтров у нуля возникла третья причина: «статья есть, но её отсёк
    # фильтр». Без этого флага такие строки попадали бы в отчёт как ненаписанные
    # статьи, и он превратился бы в мусор ровно в той мере, в какой людям
    # понравятся фильтры.
    "ALTER TABLE wiki_search_log ADD COLUMN IF NOT EXISTS "
