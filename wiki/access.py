...
def has_write_capability(capabilities):
    ...
    видов: должностные и итоговые — вторые включают права, выписанные ПРАВИЛАМИ
    разделов (queries.load_capabilities). Справочники общекомпанейские, поэтому
    оба вызывающих (routes_parks, routes_offices) подают ctx['role_capabilities']:
    правило на один раздел вики не должно открывать телефоны всех парков.
    Любой новый общекомпанейский гейт обязан сделать тот же выбор осознанно.
    """
