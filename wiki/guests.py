...
── Срок ─────────────────────────────────────────────────────────────────────
По умолчанию — КОНЕЦ ДНЯ по Алматы, а не «момент плюс N часов». Человек читает
...
ЧАС МОЖНО НАЗВАТЬ ЯВНО (решение владельца 25.08.2026: «до 18:00»). Тогда конец
дня заменяется названным временем — «сегодня до 18:00» это ровно то, зачем
гостевой доступ и зовут: показать коллеге раздел на время созвона, а не на
сутки. Час не отдельный вид выдачи, а уточнение того же дня: сначала решается,
КАКОЙ это день (пресет или дата), потом — до какого часа внутри него. Поэтому
потолок остаётся дневным и час его не обходит.

Названный час обязан быть В БУДУЩЕМ, и проверка эта не формальность: «до 18:00»,
набранное в 18:30, иначе завело бы выдачу, истёкшую в момент создания, — строка
в списке есть, доступа нет.
...
def resolve_expiry(now, *, days=None, until=None, at_time=None):
    """Момент истечения выдачи по Алматы.

    days    — пресет «на N дней»: последним будет день now.date() + N.
              Ноль означает «сегодня».
    ...
    at_time — час в формате ЧЧ:ММ, необязательный. Без него выдача живёт до
    конца выбранного дня; с ним — до названного времени этого дня.
    ...
    if has_days:
        ...
        # Ноль — «сегодня», и это законный срок, а не пустая выдача: с ним
        # собирается «сегодня до 18:00», ради чего час и появился. Без нуля
        # ближайший пресет — завтрашний день, и однодневную выдачу пришлось бы
        # набирать датой.
        if count < 0:
            return _fail('Срок гостевого доступа не бывает отрицательным')
    ...
    # Час не назвали — конец дня, а не полночь следующего: полночь следующего
    # дня в подписи «до 06.09» читалась бы как «шестого уже нельзя», хотя
    # доступ ещё жив.
    moment = datetime.combine(last_day, _parse_time(at_time) or time(23, 59, 59))

    # Названный час обязан быть впереди. Пресет «1 день» с часом 09:00, набранный
    # в 10:00, даёт завтрашние 09:00 — это будущее и это законно; а вот «сегодня
    # до 09:00» в 10:00 — выдача, истёкшая в момент создания.
    if moment <= now:
        return _fail('Это время уже прошло — выберите час позже или другой день')
    return moment


def _parse_time(raw):
    """ЧЧ:ММ → time. Пусто — None («до конца дня»). Мусор — ошибка для человека."""
    ...
    try:
        ...
        return _fail('Время окончания — в формате ЧЧ:ММ, например 18:00')
    # Секунды нулевые намеренно: «до 18:00» означает ровно 18:00, и лишние
    # секунды в списке выдач читались бы как чужая точность.
    return parsed
