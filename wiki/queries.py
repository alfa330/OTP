...
#   * ровно столько, сколько выдали: сам раздел и его подразделы, если
#     выдававший это отметил, — либо всё пространство, если выдали пространство
#     целиком (третий объект выдачи, решение владельца 11.09.2026; выдаёт его
#     только супер-админ, access.may_grant_guest_space). Соседние ветки чужого
#     пространства выдачей на РАЗДЕЛ по-прежнему не открываются.
# Всё остальное — публичные разделы чужого пространства, собственные разделы,
# правила — границу по-прежнему не проходит.
#
# Выдача на пространство — это по-прежнему ЧТЕНИЕ, и шире оно не становится:
# прав на объекте она не даёт (resolve_article_permissions поднимает только
# can_read, и то через guest_allows_read), а справочники, аналитику, журнал и
# обзор у такого пространства закрывает признак guest_only — он считается
# разницей spaces_for_user с гостевой прибавкой и без неё.
_SPACE_GATE_SQL = """
SELECT p.id
  FROM picked p
...
# запроса, оно однажды разойдётся: в исходной вики два вычислителя доступа
...
# ─────────────────────────────────────────────────────────────────────────────
#
# ВЫДАЧА НА ПРОСТРАНСТВО ЦЕЛИКОМ (владелец, 11.09.2026) втекает в тот же
# guest_seed отдельной веткой, а не третьим CTE. Так она получает всё, что уже
# устроено вокруг гостевой выдачи, — щель в границе пространства
# (_SPACE_GATE_SQL смотрит ровно на guest_seed/guest_tree), отсечение отозванной
# и истёкшей, попадание в оба режима периметра. Отдельный CTE пришлось бы
# вписать в каждое из этих мест руками, и первое же забытое место открывало бы
# гостю дерево без статей или наоборот.
#
# Раскрытия на подразделы у этой ветки нет и не нужно: она перечисляет ВСЕ
# активные разделы пространства сразу, а не корень поддерева. Поэтому deep
# = FALSE — раскрывать уже нечего, и рекурсия по такому seed'у только гоняла бы
# по дереву второй раз.
#
# Архивные разделы не попадают (status = 'active'): пространство выдают, чтобы
# читать его вику, а архив из оборота убран. Выданный поимённо раздел при этом
# по-прежнему берётся как есть — это разные обещания.
_GUEST_SECTIONS_CTE = """
guest_seed AS (
    SELECT g.section_id AS id, g.include_subsections AS deep
     ...
     WHERE g.user_id = %(user_id)s
       ...
       AND g.expires_at > (CURRENT_TIMESTAMP AT TIME ZONE 'Asia/Almaty')
    UNION
    SELECT s.id AS id, FALSE AS deep
      FROM wiki_guest_access g
      JOIN wiki_sections s ON s.space_id = g.space_id AND s.status = 'active'
     WHERE g.user_id = %(user_id)s
...
def spaces_for_user(cursor, ctx, include_guest=True):
    ...
    ГОСТЬ. Пространство, в котором человеку выдали раздел или статью — или
    которое выдали ему целиком (решение владельца 11.09.2026), — попадает
    ...
    а прийти к ним некуда — переключатель пуст, и вики открывается пустой.
         ...
         WHERE sp.status = 'active'
           AND (NOT EXISTS (SELECT 1 FROM wiki_space_departments sd
                ...
                OR EXISTS (SELECT 1 FROM wiki_space_departments sd
                            WHERE sd.space_id = sp.id
                              AND sd.department_id = ANY(%(departments)s))
                -- Действующая именная выдача в этом пространстве. Пространство
                -- выдачи ищем тремя способами сразу: оно названо прямо
                -- (g.space_id — выдача пространства целиком), взято у выданного
                -- раздела или у разделов выданной статьи. Выдать можно любое из
                -- трёх, а отвечают они на один вопрос — «куда человека позвали».
                OR (%(with_guest)s AND EXISTS (
                    SELECT 1
                     ...
                     WHERE g.user_id = %(user_id)s
                       AND g.revoked_at IS NULL
                       AND g.expires_at > (CURRENT_TIMESTAMP AT TIME ZONE 'Asia/Almaty')
                       AND COALESCE(g.space_id, gs.space_id, gass.space_id) = sp.id)))
