...
# Код источника переноса. Первый: старая корпоративная вика на Wiki.js 2 во
# внутренней сети (http://192.168.88.186:3000, заголовок «Яндекс GO»).
SOURCE_OLD_WIKI = 'wikijs'
# Второй: база знаний Яндекс Про (pro.yandex.com). В отличие от первого, этот
# источник ЖИВОЙ — страницы там правят, и статья у нас держится с ним на связи
# (wiki/yandex_pro.py, wiki/yandex_sync.py). Для очереди модерации разницы нет:
# приехавшее из обоих источников одинаково ждёт решения человека.
SOURCE_YANDEX_PRO = 'yandex_pro'

# Подписи источников для интерфейса. Ключи — те же коды; фронт берёт подпись
# отсюда через /api/wiki/migration, чтобы название источника не оказалось
# зашитым строкой в двух местах сразу.
SOURCE_LABELS = {
    SOURCE_OLD_WIKI: 'старая вики',
    SOURCE_YANDEX_PRO: 'Яндекс Про',
}
