...
#
# Правила (постановка Алчинбаевой Анель при возврате #292 на доработку 18.09.2026:
# «AR должен считаться с момента поступления звонка в очередь. В нашем проекте этот
# порог начинается с 22 секунды. Все что до, не должно входить»):
#   * берём ТОЛЬКО входящие (callType 0) на линии ТП;
#   * принято — disposition ANSWER, сколько бы человек ни ждал;
#   * потеряно — не ANSWER И звонок ДОШЁЛ ДО ОЧЕРЕДИ;
#   * вошло = принято + потеряно. Брошенного на автоинформаторе в нём нет ВОВСЕ:
#     очереди он не видел, и в её показателе ему не место.
#
# Почему отбор идёт по СТАДИИ, а не по времени ожидания. Проверено на журнале за
# 08–22.09.2026 (873 принятых, 95 неотвеченных на линии ТП): `waitsec` считается уже
# ОТ ВХОДА В ОЧЕРЕДЬ, приветствие в него не входит. Доказательство — звонок с двумя
# плечами: очередь 91 с + оператор 2 с, а в шапке ровно 93; и потолок очереди ровно
# 120 с, а не 141. То есть «22-я секунда» владельца — это конец приветствия в
# настройках Binotel, а не 22 секунды ожидания в очереди: время в очереди мы и так
# считаем с нуля, и вычитать из него ещё 21 секунду значило бы вычесть приветствие
# дважды. За тот же период двойной вычет прятал 14 потерянных звонков из 60 и занижал
# AR с 6,4 % до 5,0 %.
#
# Стадия лежит в поле внутреннего номера неотвеченного звонка и бывает трёх видов:
# «Очередь», «Приветствие в рабочее время New» и номер оператора (очередь уже отдала
# звонок человеку, а он не снял). Незнакомую стадию считаем дошедшей до очереди —
# см. stage_reached_queue: переименование вендором должно быть видно как рост AR и
# новая строка в диагностике, а не как молча обнулённая метрика.
...
def stage_reached_queue(stage):
    """Дошёл ли звонок до очереди — по стадии, на которой он оборвался.

    Отвечаем «да» на всё, кроме явно узнанного автоинформатора, а не «да» только на
    слово «Очередь». Разница в том, чем оборачивается переименование у вендора:
    белый список молча обнулил бы потери (звонков в «очереди» больше нет), а чёрный
    в худшем случае завысит AR — и это видно и в цифре, и в диагностике стадий.
    """
    text = " ".join(str(stage or "").strip().lower().split())
    return not any(marker in text for marker in GREETING_STAGE_MARKERS)
...
def line_day_totals(calls, line_number, abandon_from_seconds=0):
    """Входящие одной линии из журнала → те же поля дня, что даёт страница очереди.

    abandon_from_seconds — с какой секунды ОЖИДАНИЯ В ОЧЕРЕДИ сброс считается потерей.
    Ноль (наш случай) значит «любой»: приветствие в это время не входит, и отсекать
    тут уже нечего. Параметр оставлен ради настройки без деплоя, если отдел захочет
    вернуть короткий сброс (short abandon) — правило тогда меняется одной переменной,
    а не правкой кода.
    ...
    for call in calls or []:
        ...
        bucket = stages.setdefault(stage, {"long": 0, "short": 0, "before_queue": 0})
        if not stage_reached_queue(stage):
            dropped_before_queue += 1
            bucket["before_queue"] += 1
    ...
    return {
        ...
        # Два числа вычеркнутых, а не одно: без них нельзя ответить на вопрос «почему
        # входящих меньше, чем в кабинете», а причины у них разные — один бросил трубку
