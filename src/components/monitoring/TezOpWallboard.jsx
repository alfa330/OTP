...
export default function TezOpWallboardBody({ snapshot, scale = 1 }) {
    ...
    return (
        // Раскладка та же, что у ТП: плитки и перерывы сверху, поимённый список во всю ширину
        // под ними. Одна и та же форма на обоих направлениях — глазу не нужно переучиваться.
        <div className="space-y-4" style={{ fontFamily: APPLE_FONT }}>
            ...
            {/* Поимённо: у продавца дневная величина — исходящие, поэтому в строке стоят
                набрано и дозвонились, а не «принято». Колонки задаёт сама таблица по
                направлению — здесь только говорится, какое оно. */}
            <TezOperatorsTable rows={snapshot?.roster} direction="op" scale={scale} />
        </div>
    );
}
