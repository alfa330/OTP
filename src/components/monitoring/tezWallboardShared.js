...
// Запасное значение правила потери: настоящее приходит в снимке (`abandon_from_seconds`),
// само правило целиком живёт на сервере. Здесь оно нужно только чтобы подпись не осталась
// пустой, если снимок пришёл из старого кэша. Ноль — «потеряны все, кто дошёл до очереди
// и не дождался»; приветствие в AR не входит независимо от этого числа.
export const TEZ_ABANDON_FROM_SECONDS = 0;

/*
 * Подпись правила потери под плитками дня. Словами, а не числом: «Потеряно» у нас и
 * «Непринятых» в кабинете Binotel считаются по-разному, и разницу надо назвать ровно один
 * раз на экране, а не оставлять руководителю догадываться.
 */
export const tezAbandonRuleHint = (seconds) => {
    const value = Number.isFinite(Number(seconds)) ? Math.max(0, Number(seconds)) : 0;
    return value > 0
        ? `Потеря — ожидание в очереди от ${value} с; на приветствии не считаем`
        : 'Потеря — сброс в очереди; на приветствии не считаем';
};
...
// Журнал перерывов вне графика. Ручка своя, а не общая с СЗоВ, потому что и права свои:
// журнал отдела смотрит его руководитель и его СВ, а не глава чужого отдела.
export const TEZ_BREAK_VIOLATIONS_PATH = '/api/tez_wallboard/break_violations';
