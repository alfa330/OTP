 ...
 * палитры. «Не в системе» серый: это отсутствие, а не состояние работы.
 *
 * Подписи — из справочника самого Chat2Desk (GET /v1/operators/statuses), а не наши слова:
 * чатник переключает статус у себя и должен узнавать его на стене. Так `holiday` — это
 * «Закрытие чатов» (человек в системе, дорабатывает открытые, новых не берёт), а вовсе не
 * отпуск, как он был подписан раньше. Подпись едет и в снимке (сервер), и здесь; сервер —
 * источник, здесь — запасной вариант для ключа без подписи.
 */
export const CHAT_STATUS_STYLE = {
    ...
    holiday: { label: 'Закрытие чатов', chip: 'bg-amber-100 text-amber-700' },
...
/*
 * Русское склонение по числу: 1 чат, 2 чата, 5 чатов. Нужно там, где число стоит РЯДОМ со
 * словом, а не в подписи плитки: «12 чатов в работе» со стены читается как работа с чатами,
 * а «12 в работе» — как что угодно (запрос владельца 20.08.2026).
 */
 ...
 *
 * Направлений табло два («Линия» и «Чат»), у каждого свой источник и свой темп опроса,
...
export const WALLBOARD_DIRECTIONS = {
    // Ключ 'osnova' — историческое имя «Линии». Подпись сменили, а ключ трогать нельзя: по
    // нему лежат выбор направления и набор показателей виджета в localStorage у каждого
    // пользователя, строки получателей отбивки в БД и параметр `direction` в запросах.
    osnova: {
        key: 'osnova',
        label: 'Линия',
