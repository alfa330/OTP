...
const SessionUserModal = ({
    ...
    // Право на выдачу целиком считает сервер и присылает одним флагом. Внутри
    // него два условия, и ни одно на клиенте не посчитать: гейт СПРАШИВАЕТ ли
    // подтверждение у этого человека (супервайзеру и админу разделы открыты и
    // без QR — «открывать» им нечего) и мой ли это периметр (в карточке нет
    // возглавляемых отделов ни у него, ни у смотрящего). Пока карточка не
    // загрузилась, кнопки нет: обещать действие по строке списка нельзя —
    // сервер откажет молча.
