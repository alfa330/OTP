 ...
 *     ИИН / Таксопарк / Период        → данные водителя, подписями
 *     подпись: ответ                  → суть обращения
 *     🔍 Проверено оператором + ✅/❌  → что проверили и что вышло
 *     ✅ Выполнено / ❗ Не выполнено   → что оператор сделал руками
 *
 * Старые обращения собраны прежним форматом (строка «парк · город · период»
 * метками и «✔️ Чек-лист выполнен: 3 из 3»), и он тоже разбирается: текст
 * лежит в базе, задним числом его никто не пересобирает.
 *
...
const MARKERS = [
    { mark: '✅', tone: 'green' },
    // «Нет» у проверенного пункта: ровно противоположный смысл соседней
    // галочки, и одинаковыми их показывать нельзя.
    { mark: '❌', tone: 'rose' },
    { mark: '❔', tone: 'slate' },
    { mark: '❗', tone: 'red' },
    { mark: '✔️', tone: 'blue' },
    // Заголовок блока проверок — не оценка, поэтому нейтральный.
    { mark: '🔍', tone: 'slate' },
 ...
 * нужен. Это первый блок перечня — у нынешних обращений там данные водителя
 * (ИИН, парк, город, период), у старых была строка контекста метками.
 *
 * Берутся ОТВЕТЫ без подписей: в строке шириной с экран «ИИН: … Таксопарк: …»
 * до второго значения уже не доходит, а сами значения узнаются и без подписи. */
export const bodyDigest = (body) => {
    ...
    if (!list) return '';
    const values = list.rows.filter((row) => row.label).map((row) => row.value);
    if (values.length > 1) return values.join(ITEM_SEP);
