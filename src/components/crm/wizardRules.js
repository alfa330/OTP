...
/* Куда уйдёт тема, если это НЕ группа её тематики. null — уйдёт «к себе», и
 * писать это в строке не нужно: адрес по умолчанию уже стоит заголовком
 * раздела, а повторённый в каждой строке заголовок читать перестают.
 *
...
export const routeNote = (item) => (
    item && item.routed && item.queue_title ? item.queue_title : null);
 ...
 * У уведённой группа как раз есть — её выключили или отвязали от неё чат, — и
...
export const blockedLabel = (item) => (
    item && item.routed ? 'Группа недоступна' : 'Нет группы');
