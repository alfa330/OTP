...
// ── Пикер диапазона дат в стиле сайта ────────────────────────────────────
// Свой календарь вместо <input type="date">: нативный рендерится по-разному в
// каждом браузере и выбивается из iOS-оформления. Диапазон — двумя кликами,
// с подсветкой при наведении; неделя с понедельника (ru).
...
const fmtShortDate = (iso, withYear) => {
...
function IosDateRangePicker({ from, to, max, onChange }) {
            ...
            {open && (
                <div className="absolute left-0 top-full z-50 mt-2 w-[268px] rounded-2xl bg-white p-3 shadow-xl ring-1 ring-slate-200/70">
                    ...
                    <div className="mt-2.5 flex items-center gap-2 border-t border-slate-100 pt-2.5">
                        <button type="button" onClick={() => setPreset({ from: today, to: today })}
        ...
        <div className="space-y-3">
            <div className="flex flex-wrap items-center justify-between gap-2">
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    <IosDateRangePicker from={from} to={to} max={isoDate(new Date())}
                                        onChange={({ from: f, to: t }) => {
                                            setFrom(f); setTo(t); load({ from: f, to: t });
                                        }} />
                </div>
