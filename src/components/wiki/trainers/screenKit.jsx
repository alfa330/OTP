...
export const ResultScreen = ({ scenario, onRestart }) => (
    <div className="wt-screen wt-result">
        ...
        <h2>Готово</h2>
        {/* Итог у каждого сценария свой: подписание заканчивается подписью,
            смена провайдера — новым провайдером, обращение — сохранённой
            карточкой. Общая фраза про подписанные документы врала на трёх
            тренажёрах из пяти, поэтому текст приходит из сценария. */}
        <p>{scenario.resultNote || 'Задание выполнено.'} Коротко повторим путь:</p>
