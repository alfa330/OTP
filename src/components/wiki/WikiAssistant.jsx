 ...
 *
 * ОТВЕТ БЕЗ ИСТОЧНИКОВ НЕВОЗМОЖЕН. Сервер не отдаёт ответ, под которым нет ни
 * одной дословно подтверждённой цитаты: вместо него приходит kind='no_answer'.
 * Поэтому отсутствие чипов источников под ответом — это не «забыли показать», а
 * признак отказа, и рисуется он иначе (серым пузырём).
 *
 ...
 * оператору важно видеть, что это вопрос к нему, а не неудачный ответ.
 *
 * ЦИТАТА ПОКАЗЫВАЕТСЯ, А НЕ ПРЯЧЕТСЯ. Она подтверждает, что ответ не выдуман, и
 * позволяет проверить за пять секунд, не открывая статью. Клик по чипу открывает
 * статью с подсветкой — плумбинг тот же, что у поиска раздела.
 *
...
const SourceChip = ({ source, onOpen }) => {
    ...
    const weak = source.quote_ok === false;
    return (
        <button
            type="button"
        ...
        >
            <div className="flex items-center gap-1.5">
                ...
                {weak && !unavailable && (
                    <IosBadge tone="amber" className="shrink-0">цитата из заголовка</IosBadge>
                )}
            </div>
            {source.heading_path && (
