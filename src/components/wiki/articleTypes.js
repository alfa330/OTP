...
/* Статус статьи — тоже один список на весь раздел.
 *
 * Жил тремя копиями: в шапке статьи, в каталоге и (после этой правки был бы) в
 * истории версий. Копия в каталоге даже несла комментарий «подписи совпадают с
 * шапкой статьи» — то есть совпадение держалось на честном слове и на памяти
 * того, кто правит. Третий экран, которому те же подписи нужны, — повод свести
 * их в одно место, а не размножить ещё раз.
 */
...
/** Подпись статуса; незнакомое значение показываем как есть, а не прячем. */
