...
/* Слаг статьи задаёт сервер: латиница, цифры и дефис, не длиннее 200 символов
   (_slugify в wiki/routes_structure.py; подчёркивание встречается в слагах,
   пришедших миграцией из старой вики). Всё, что не подходит под это описание,
   в адрес не пускаем и из адреса не читаем: значение уходит в путь запроса
   /api/wiki/articles/<slug>. */
export const normalizeArticleSlug = (value) => {
    const slug = String(value || '').trim();
    return /^[A-Za-z0-9_-]{1,200}$/.test(slug) ? slug : '';
};
