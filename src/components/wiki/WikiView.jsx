...
export default function WikiView({ apiBaseUrl, withAccessTokenHeader, showToast, user,
    ...
    /* Право выдавать ГОСТЕВОЙ доступ — у супервайзера и выше. Считает сервер и
       присылает готовым признаком: право живёт в ДОЛЖНОСТИ, отдельной лестницей
       (access.GUEST_GRANT_CEILING), и в словаре способностей его нет вовсе.
       Вывести его здесь по роли было бы вторым источником истины. */
    ...
    const tabs = useMemo(() => ([
        ...
        { key: 'catalog', label: 'Статьи', icon: BookOpen,
          /* canGrantGuest — четвёртый вход, и он не лишний: у супервайзера нет
             ни способностей can_manage_*, ни, бывает, права правки, а половину
             «Гостевой доступ» он обязан видеть. Без этого входа вкладка не
    ...
    const catalogModes = useMemo(() => [
        ...
        /* Гостевой доступ — супервайзеру и выше, тому, кто РАЗДАЁТ, а не тому,
           кто пишет: редактор без права выдавать ничего бы здесь не сделал.
           Право живёт в должности, поэтому его считает сервер и присылает
           признаком can_grant_guest — вывести его из способностей нельзя.
           Администратора доступов пропускаем отдельно: у него мастер-ключ, а
           должность бывает любая. */
