...
/*
 * Фирменный знак портала — тот же контур, что в сайдбаре
 * (sidebar-logo-mark.svg) и на иконке домашнего экрана, но НЕ картинкой, а
 * разметкой.
 *
 * Зачем инлайн, если рядом лежит .svg. Картинку через <img> нельзя
 * перекрасить: файл в сайдбаре залит белым — он рисуется на синем. На белой
 * карточке входа нужен цветной знак, и раньше ради этого на экране входа
 * висела картинка с ПОСТОРОННЕГО ХОСТИНГА (iili.io): без интернета и при его
 * блокировке вход встречал битым значком. Инлайн решает и то, и другое: заливка
 * задаётся здесь, файл никуда не ходит.
 *
 * По умолчанию знак залит фирменным градиентом (синий → фиолетовый, снят
 * пипеткой с favicon). `color="current"` заливает его currentColor — так знак
 * подхватывает цвет текста родителя.
 *
 * gradientId уникален на каждый экземпляр: два знака на одной странице с общим
 * id ссылались бы на одно определение, и при удалении первого второй потерял бы
 * заливку.
 */
const IcoreMark = ({ className = '', color = 'gradient', title }) => {
    const gradientId = useId();
    const fill = color === 'gradient' ? `url(#${gradientId})` : 'currentColor';
    return (
        <svg
