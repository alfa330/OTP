...
алиасы в `qa_marketing_dict`; новое написание заводится само при связывании
(`linker.learn_dictionary`) и сразу работает как отдельное значение, склеить его
с брендом — правка алиаса без релиза. Корзина «Не определено» (`NONE_BUCKET`) —
не пустой фильтр: у 60% сделок парк пуст, и без корзины сумма по паркам не
сходилась бы с общим числом разборов.
...
| Кто обрабатывал | `handler_ids[]`, `handler_group_ids[]`, `handler_mode` | «Говорил» — оператор раздела; «Ответственный» — `op_funnel_operator_map`, смена режима сбрасывает выбор |
| Этап сделки | `stages[]`, `stage_mode` | сырые имена этапов; `at_call` — из журнала |
| Причина отказа | `reasons[]` | только при этапе «Закрыто-нереализовано» — гасится на клиенте И отвергается сервером (`is_lost_stage`, маркеры совпадают буквально) |
