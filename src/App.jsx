...
const withAccessTokenHeader = (headers = {}) => {
    ...
    return nextHeaders;
        ...
        const RateCircle = ({ rate, len=1 }) => {
            const percentage = rate * 100 / len;
            const color = '#16a34a';
                ...
                <div className="relative w-10 h-10 mx-auto">
                    <svg className="w-full h-full" viewBox="0 0 36 36">
                        ...
                        <path
                            ...
                            stroke={color}
                            strokeWidth="3"
                            strokeDasharray={`${percentage.toFixed(2)}, 100`}
                        />
                    </svg>
        ...
        const SemiCircleProgress = ({ percentage, label, width = 150, height = 80 }) => {
            ...
            useEffect(() => {
                ...
                const startAngle = Math.PI;     // слева (180°)
                const endAngle = 2 * Math.PI;   // справа (360°)
                ...
                if (percentage <= 100) {
                    // обычный прогресс
                    const progressAngle = startAngle + (percentage / 100) * Math.PI;
                    ctx.beginPath();
                    ctx.arc(centerX, centerY, radius, startAngle, progressAngle);
                    ctx.strokeStyle =
                        percentage >= 90 ? "#16a34a" : percentage >= 50 ? "#eab308" : "#dc2626";
                    ctx.lineWidth = 8;
                    ctx.stroke();
                } else {
                    // зелёная дуга до 100%
                    ...
                    // синяя дуга сверху — начинается СЛЕВА и идёт вправо
                    const extraPercent = Math.min(percentage - 100, 50); // максимум 50%
                    if (extraPercent > 0) {
                        const extraAngle = startAngle + (extraPercent / 100) * Math.PI;
                        ctx.beginPath();
                        ctx.arc(centerX, centerY, radius, startAngle, extraAngle);
                ...
                ctx.fillText(`${percentage.toFixed(1)}%`, centerX, centerY - radius - 15);
        ...
        // ==================== Hours Accounting View ====================

        const TABS = [
        { key: "work_time", label: "Отработанные часы", unit: "ч" },
        { key: "break_time", label: "Перерыв", unit: "ч" },
        ...
        const [previewOperators, setPreviewOperators] = useState([]);
        const [previewSheetName, setPreviewSheetName] = useState(null);
        ...
        const monthLabel = useMemo(() => {
            ...
            return names[monthParts.monthNum - 1] || '';
        }, [monthParts]);

        const daysInMonth = useMemo(() => new Date(monthParts.year, monthParts.monthNum, 0).getDate(), [monthParts]);
        const daysArray = useMemo(() => Array.from({ length: daysInMonth }, (_, i) => i + 1), [daysInMonth]);

        // compute "today" relative to user's environment
        ...
        async function fetchDailyHoursAndTrainings() {
            ...
            for (const t of trainingsList) {
                ...
                let dayNum;
                try {
                dayNum = new Date(t.date + 'T00:00:00').getDate();
                } catch (e) {
                dayNum = parseInt(t.date.slice(-2), 10);
                }
                if (!tmap[op]) tmap[op] = {};
                if (!tmap[op][dayNum]) tmap[op][dayNum] = [];
        ...
        function formatHeaderLabel(day) {
        ...
        async function handlePreviewUpload(e) {
            e.preventDefault();
            if (!selectedDayUpload || !uploadFile) {
            ...
            if (resp.data && resp.data.status === 'success') {
                const normalized = (resp.data.operators || []).map((r, idx) => ({
                _rowIndex: idx,
                ...
                fallbackToast(resp.data?.error || 'Ошибка парсинга файла', 'error');
            }
            } catch (err) {
            console.error('preview error', err);
            fallbackToast(err.response?.data?.error || err.message || 'Ошибка предпросмотра', 'error');
            } finally {
        ...
        async function handleConfirmSave() {
            ...
            if (resp.data && resp.data.status === 'success') {
                ...
                setSelectedDayUpload(null);
                setPreviewOperators([]);
            } else {
                fallbackToast(resp.data?.error || 'Ошибка сохранения', 'error');
            }
            } catch (err) {
            console.error('save error', err);
            fallbackToast(err.response?.data?.error || err.message || 'Ошибка сохранения', 'error');
            } finally {
        ...
        function openCellDetail(operator, day) {
            ...
            work_time: Number(dayData.work_time || 0),
            break_time: Number(dayData.break_time || 0),
            talk_time: Number(dayData.talk_time || 0),
            calls: Number(dayData.calls || 0),
            efficiency: Number(dayData.efficiency || 0),
            // support multiple fines per day: prefer dayData.fines (array), fallback to single fine fields
            fines: Array.isArray(dayData.fines) && dayData.fines.length > 0 ? dayData.fines.map(f => {
                const amt = Number(f.amount || f.fine_amount || 0);
                const reason = f.reason || f.fine_reason || '';
                return {
                amount: amt,
                minutes: String(reason) === 'Опоздание' ? Math.round(amt / 50) : null,
                reason,
                comment: f.comment || f.fine_comment || ''
                };
            }) : (
                (dayData.fine_amount || dayData.fine_reason || dayData.fine_comment) ? (function(){
                const amt = Number(dayData.fine_amount || 0);
                const reason = dayData.fine_reason || '';
        ...
        function updateCellField(field, value) {
            ...
            if (!prev) return prev;
            const next = { ...prev };
            if (['work_time', 'break_time', 'talk_time', 'efficiency', 'fine_amount'].includes(field)) next[field] = value === '' ? '' : Number(value);
        ...
        // Fines: support multiple fines per day in the UI
        function addFine() {
            setCellModel(prev => {
            if (!prev) return prev;
            const next = { ...prev };
            next.fines = Array.isArray(next.fines) ? [...next.fines, { amount: 0, minutes: 0, reason: '', comment: '' }] : [{ amount: 0, minutes: 0, reason: '', comment: '' }];
            return next;
            });
        }

        function updateFineField(idx, field, value) {
            setCellModel(prev => {
            if (!prev) return prev;
            const next = { ...prev };
            next.fines = Array.isArray(next.fines) ? next.fines.map((f, i) => {
                if (i !== idx) return f;
                const updated = { ...f };
                if (field === 'amount') {
                updated.amount = (value === '' ? '' : Number(value));
                // if reason is Опоздание keep minutes in sync
                if (String(updated.reason) === 'Опоздание') {
                    updated.minutes = updated.amount ? Math.round(Number(updated.amount) / 50) : 0;
                }
                } else if (field === 'minutes') {
                // minutes -> compute amount (backend wants amount = minutes * 50)
                const mins = value === '' ? 0 : Number(value);
                updated.minutes = mins;
                updated.amount = Number(mins) * 50;
                } else if (field === 'reason') {
                updated.reason = value;
                const rv = String(value);
                if (rv === 'Опоздание') {
                    // derive minutes from existing amount if present
                    const amt = Number(updated.amount || 0);
                    updated.minutes = amt ? Math.round(amt / 50) : 0;
                    updated.amount = Number(updated.minutes) * 50;
                } else if (rv === 'Не выход') {
                    // fixed fine
                    updated.minutes = null;
                    updated.amount = 10000;
                } else if (rv === 'Прокси карта') {
                    updated.minutes = null;
                    updated.amount = 5000;
                } else {
                    // clear minutes when not Опоздание and keep amount editable
                    updated.minutes = null;
                }
                } else {
                updated[field] = value;
        ...
        function removeFine(idx) {
            setCellModel(prev => {
            if (!prev) return prev;
            const next = { ...prev };
            next.fines = Array.isArray(next.fines) ? next.fines.filter((_, i) => i !== idx) : [];
            return next;
            });
        }
                ...
                function clearCellFields() {
                    ...
                    next.calls = '';
                    next.efficiency = '';
                    // backward-compatible single fine fields
                    next.fine_amount = 0;
                    next.fine_reason = '';
                    next.fine_comment = '';
        ...
        async function saveCell() {
            ...
            const payload = {
                ...
                name: cellModel.name,
                work_time: Number(cellModel.work_time) || 0,
                break_time: Number(cellModel.break_time) || 0,
                talk_time: Number(cellModel.talk_time) || 0,
                calls: parseInt(cellModel.calls, 10) || 0,
                efficiency: Number(cellModel.efficiency) || 0,
                // include fines array (new) for backend that supports multiple fines
                fines: Array.isArray(cellModel.fines) ? cellModel.fines.map(f => ({
                // if reason is Опоздание, backend expects amount = minutes * 50 (minutes are NOT sent)
                amount: String(f.reason) === 'Опоздание' ? Number((Number(f.minutes || 0) * 50)) : Number(f.amount || 0),
                reason: f.reason || null,
                comment: f.comment || null
                })) : [],
                // backward-compatible single fields: total fine amount, first reason, concatenated comments
            ...
            fallbackToast(err.response?.data?.error || err.message || 'Ошибка при сохранении', 'error');
            } finally {
        ...
        async function handleNormChange(operatorId, value) {
            ...
            await axios.post(`${API_BASE_URL}/api/sv/update_norm_hours`, {
            ...
            fallbackToast('Ошибка обновления нормы часов', 'error');
            }
        }

        // ====== Render helpers & coloring logic ======
        const maxWork = useMemo(() => {
            let max = 8;
            for (const op of operators) {
            if (!op.daily) continue;
            for (const k of Object.keys(op.daily)) {
                const v = Number(op.daily[k]?.work_time || 0);
                if (v > max) max = v;
            }
            }
            return max;
        ...
        const maxTrainingsHours = useMemo(() => {
            let max = 0;
        ...
        function toggleDirectionSelection(dirKey) {
            ...
            const key = String(dirKey);
            if (s.includes('all')) {
            if (key === 'all') return setSelectedDirections(['all']);
            setSelectedDirections([key]);
            return;
            }
            if (s.includes(key)) {
            const idx = s.indexOf(key); s.splice(idx,1);
            if (s.length === 0) setSelectedDirections(['all']); else setSelectedDirections(s);
            } else {
            setSelectedDirections(prev => prev.filter(Boolean).concat(key));
            }
        }

        function monthRelationToToday() {
            if (monthParts.year < todayY) return -1;
            if (monthParts.year > todayY) return 1;
            if (monthParts.monthNum < todayM) return -1;
            if (monthParts.monthNum > todayM) return 1;
            return 0;
        }
        const monthRel = monthRelationToToday();

        function productionColorClass(diff, norm) {
            if (norm <= 0) return diff < 0 ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-700';
            const pct = (diff / norm) * 100;
        ...
        function getFineIcon(reason) {
            const r = String(reason || '').toLowerCase();
            if (r.includes('опоз') || r.includes('опоздание')) return 'fas fa-clock';
            if (r.includes('не выход') || r.includes('невыход') || r.includes('не выш')) return 'fas fa-user-times';
            if (r.includes('корп такси')) return 'fas fa-taxi';
            if (r.includes('прокси') || r.includes('прокси карта')) return 'fas fa-id-card';
        ...
        const { filteredOperators, activeCount, firedCount } = useMemo(() => {
            ...
            let list = operatorsViewTab === 'active' ? activeList.slice() : firedList.slice();

            // apply directions multi-filter when set (exclude 'all')
            if (selectedDirections && selectedDirections.length > 0 && !selectedDirections.includes('all')) {
            const dirSet = new Set(selectedDirections.map(s => String(s)));
            list = list.filter(op => {
                const key = op.direction || op.direction_name || (op.direction_id ? String(op.direction_id) : 'Без направления');
                return dirSet.has(String(key));
            });
            }

            return {
            filteredOperators: list,
        ...
        const footerTotals = useMemo(() => {
            ...
            // compute trainings per operator
            ...
            if (op.daily) {
                ...
                if (Array.isArray(dd.fines)) {
                    for (const f of dd.fines) {
                    sumFines += Number(f.amount || f.fine_amount || 0) || 0;
                    }
            ...
            if (regular > 0) occValues.push((effTotal / regular) * 100);
            }
        ...
        function renderCellByMetricWithStyleAndMarker(op, day, metricKey) {
            const dayKey = String(day);
            const d = op.daily && op.daily[dayKey];
            const isPast = (() => {
            if (monthRel < 0) return true;
            if (monthRel > 0) return false;
            return day < todayD;
            })();
            const isFuture = (() => {
            if (monthRel > 0) return true;
            ...
            const ratio = maxWork > 0 ? Math.max(0, Math.min(1, work / maxWork)) : 0;
            const alpha = 0.18 + 0.77 * ratio;
            const bg = `rgba(16,185,129,${alpha})`;
            const textClass = ratio > 0.5 ? 'text-white' : 'text-gray-900';
            ...
            const parts = [];
            if (countedHours > 0) {
                const ratio = Math.min(1, countedHours / maxTrainingsHours);
                const alpha = 0.18 + 0.77 * ratio;
                parts.push(
                <div key="c" className="inline-flex items-center justify-center px-2 py-1 rounded-md text-sm font-medium" style={{ backgroundColor: `rgba(16,185,129,${alpha})` }}>
                    <span className={ratio > 0.5 ? 'text-white' : 'text-gray-900'}>{countedHours.toFixed(2)}</span>
                </div>
                );
            }
            if (notCountedHours > 0) {
                const ratio = Math.min(1, notCountedHours / maxTrainingsHours);
                const alpha = 0.18 + 0.77 * ratio;
                parts.push(
                <div key="n" className="inline-flex items-center justify-center px-2 py-1 rounded-md text-sm font-medium ml-1" style={{ backgroundColor: `rgba(250,204,21,${alpha})` }}>
                    <span className={ratio > 0.5 ? 'text-white' : 'text-gray-900'}>{notCountedHours.toFixed(2)}</span>
                </div>
                );
            }
            ...
            if (selectedTab === 'fines') {
                ...
                const maxFines = 10000;

                // Фон по аналогии с work_time/trainings
                const ratio = Math.min(1, total / maxFines);
                const alpha = 0.18 + 0.77 * ratio;
                const bg = `rgba(250,204,21, ${alpha})`; // yellow-400 цвет
        ...
        function getAllTrainingsForDay(day) {
            const all = [];
            for (const op of filteredOperators) {
            const arr = getTrainingsFor(op.operator_id, day);
            for (const t of arr) all.push({ ...t, operator_name: op.name, operator_id: op.operator_id });
            }
        ...
        // ====== Trainings inside day modal ======
        function openTrainingModalForDay(opId, day, training = null) {
            if (!opId || !day) return;
            setTrainingModalState({ open: true, operatorId: opId, date: dayToDateStr(day), training: training ? { ...training } : null });
        }

        function closeTrainingModal() {
            setTrainingModalState({ open: false, operatorId: null, date: null, training: null });
        }

        async function handleTrainingSaveFromModal(data) {
        ...
        async function handleTrainingDeleteFromModal(trainingId) {
            ...
            console.error('handleTrainingDeleteFromModal error', err);
            fallbackToast('Ошибка при удалении тренинга', 'error');
            } finally {
            setIsTrainingActionLoading(false);
            }
        }
        ...
        async function downloadMonthlyReport() {
            ...
            if (!month) {
            fallbackToast('Выберите месяц', 'error');
            return;
            }
            ...
            const url = `${API_BASE_URL}/api/report/monthly_hours?${params.toString()}`;
            ...
            if (!resp.ok) {
                ...
                fallbackToast(`Не удалось получить файл: ${errText}`, 'error');
                return;
            }

            const blob = await resp.blob();
            const cd = resp.headers.get('Content-Disposition') || resp.headers.get('content-disposition') || '';
            let filename = `report_${month}.xlsx`;
            if (cd) {
            ...
            fallbackToast('Отчет загружен', 'success');
            } catch (err) {
            console.error('downloadMonthlyReport error', err);
            fallbackToast(err.message || 'Ошибка при скачивании отчёта', 'error');
            } finally {
        ...
        return (
                    ...
                    Object.keys(groupedByDirection).map(dirKey => (
                        ...
                        {groupedByDirection[dirKey].map(op => {
                            ...
                            for (const dKey of Object.keys(op.daily)) {
                                ...
                                if (Array.isArray(dd.fines)) {
                                for (const f of dd.fines) {
                                    finesTotal += Number(f.amount || f.fine_amount || 0) || 0;
                                }
                                } else if (dd.fine_amount) {
            ...
            {/* Upload modal (file selector -> preview) */}
            {selectedDayUpload && previewOperators.length === 0 && (
                ...
                <form onSubmit={handlePreviewUpload} className="bg-white p-6 rounded-lg shadow-lg w-full max-w-md" onClick={e => e.stopPropagation()}>
                    ...
                    <button type="submit" className="px-4 py-2 bg-blue-600 text-white rounded-md">{isPreviewing ? `Загрузка ${uploadProgress}%` : 'Предпросмотр'}</button>
                    </div>
                </form>
                </div>
            )}
            ...
            {selectedDayUpload && previewOperators.length > 0 && (
                ...
                <div className="bg-white p-4 rounded-lg shadow-lg w-[95%] max-w-6xl">
                    ...
                    <div className="flex gap-2">
                        ...
                        {isSavingPreview ? 'Сохранение...' : 'Сохранить'}
                        </button>
                    </div>
                    </div>
            ...
            {selectedCell && cellModel && (
                ...
                >
                <div
                    className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl border border-gray-100 overflow-y-auto max-h-[90vh]"
                ...
                >
                    ...
                    <div>
                        ...
                        <div className="text-xs text-gray-500 mt-1">Оператор: {selectedCell.operator?.name}</div>
                    </div>
                    ...
                    {/* Body */}
                    <div className="p-6">
                    {/* поля и тренинги — без изменений от вашего варианта */}
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <input
                            ...
                            type="number"
                            step="0.01"
                            value={cellModel.work_time}
                            onChange={e => updateCellField('work_time', e.target.value)}
                        />
                        </label>
                        ...
                        <input
                            ...
                            type="number"
                            step="0.01"
                            value={cellModel.break_time}
                            onChange={e => updateCellField('break_time', e.target.value)}
                        />
                        </label>
                    ...
                    {(selectedTab === 'work_time' || selectedTab === 'trainings') && (
                        ...
                        <div className="flex items-center justify-between mb-3">
                            ...
                            <button
                                type="button"
                                onClick={() => openTrainingModalForDay(selectedCell.operator.operator_id, selectedCell.day)}
                        ...
                        {getTrainingsFor(selectedCell.operator.operator_id, selectedCell.day).length === 0 ? (
                            <div className="rounded-md border border-dashed p-4 text-sm text-gray-500">Тренингов нет.</div>
                        ) : (
                            ...
                            {getTrainingsFor(selectedCell.operator.operator_id, selectedCell.day).map(t => {
                                ...
                                <div
                                    ...
                                    <div className="flex items-center gap-2">
                                        <div className="text-xs text-gray-500">by {t.created_by_name}</div>
                                        <button
                                        type="button"
                                    ...
                                    <div className="text-sm text-gray-700">{t.reason || '—'}</div>
                                    {t.comment && <div className="text-xs text-gray-600">{t.comment}</div>}
                    ...
                    {/* Empty state */}
                    {(!cellModel.fines || cellModel.fines.length === 0) ? (
                        <div className="rounded-md border border-dashed p-4 text-sm text-gray-500">Штрафов нет.</div>
                    ) : (
                        <div className="space-y-3">
                        {(cellModel.fines || []).map((f, idx) => {
                            const reason = String(f.reason ?? '');
                            const isAutoAmount = ['Опоздание', 'Не выход', 'Прокси карта'].includes(reason);
                            const minutes = Number(f.minutes ?? 0);
                            const computedAmount = reason === 'Опоздание' ? (minutes * 50) : reason === 'Не выход' ? 10000 : reason === 'Прокси карта' ? 5000 : f.amount;
                            ...
                            <div
                                ...
                                role="group"
                                aria-label={`Штраф ${idx + 1}`}
                            >
                                {/* top row: amount | reason | actions */}
                                <div className="grid grid-cols-1 md:grid-cols-6 gap-3 items-end" style={{ alignItems: 'start' }}>
                                {/* Amount (read-only for auto types, shows computed value) */}
                                <div className="col-span-2">
                                    <label className="text-xs text-gray-700 mb-1 block">Сумма</label>
                                    <input
                                    type="text"
                                    readOnly={isAutoAmount}
                                    className={`w-full p-2 rounded-md border ${isAutoAmount ? 'bg-gray-50 text-gray-700 cursor-not-allowed' : 'bg-white'}`}
                                    value={Number(computedAmount).toLocaleString('ru-RU')}
                                    onChange={e => updateFineField(idx, 'amount', e.target.value)}
                                    aria-label="amount"
                                    />
                                    {/* helper text for fixed/auto */}
                                    {reason === 'Опоздание' && (
                                    <div className="text-xs text-gray-500 mt-1">Вычисляется: минуты × 50. Изменять нельзя.</div>
                                    )}
                                    {reason === 'Не выход' && (
                                    <div className="text-xs text-gray-500 mt-1">Фиксированная сумма: 10 000.</div>
                                    )}
                                    {reason === 'Прокси карта' && (
                                    <div className="text-xs text-gray-500 mt-1">Фиксированная сумма: 5 000.</div>
                                    )}
                                </div>

                                {/* Reason select */}
                                <div className="col-span-2 md:col-span-2">
                                    <label className="text-xs text-gray-700 mb-1 block">Причина</label>
                                    <select
                                    className="w-full p-2 rounded-md border bg-white"
                                    value={f.reason ?? ''}
                                    onChange={e => updateFineField(idx, 'reason', e.target.value)}
                                    aria-label="reason"
                                    >
                                    <option value="">—</option>
                                    <option>Корп такси</option>
                                    <option>Опоздание</option>
                                    <option>Прокси карта</option>
                                    <option>Не выход</option>
                                    <option>Другое</option>
                                    </select>
                                </div>

                                {/* Computed minutes/amount preview (compact) */}
                                <div className="col-span-1 md:col-span-1">
                                    <label className="text-xs text-gray-700 mb-1 block">Превью</label>
                                    <div className="p-2 rounded-md border bg-gray-50 text-gray-700">
                                    {reason === 'Опоздание' ? `${minutes} мин → ${Number(minutes * 50).toLocaleString('ru-RU')}` :
                                        reason === 'Не выход' ? '10 000' :
                                        reason === 'Прокси карта' ? '5 000' :
                                        f.amount ?? '-'}
                                    </div>
                                </div>

                                {/* Actions: delete (aligned right) */}
                                <div className="col-span-1 flex items-center justify-end md:justify-end">
                                    <button
                                    type="button"
                                    onClick={() => removeFine(idx)}
                                    className="inline-flex items-center gap-2 text-sm text-red-600 px-2 py-1 rounded hover:bg-red-50 focus:outline-none"
                                    aria-label={`Удалить штраф ${idx + 1}`}
                                    >
                                    {/* trash icon */}
                                    <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
                                ...
                                {/* Minutes input + Comment */}
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                                {/* Minutes (only for Опоздание) */}
                                {reason === 'Опоздание' && (
                                    <div>
                                    <label className="text-xs text-gray-700 mb-1 block">Минут опоздания</label>
                                    <input
                                        type="number"
                                        min="0"
                                        className="w-full p-2 rounded-md border"
                                        value={f.minutes ?? 0}
                                        onChange={e => updateFineField(idx, 'minutes', e.target.value)}
                                        aria-label="minutes"
                                    />
                                    <div className="text-xs text-gray-500 mt-1">Минуты сохраняются — сумма формируется автоматически.</div>
                                    </div>
                                )}

                                {/* Comment — occupies remaining space */}
                                <div className={reason === 'Опоздание' ? 'md:col-span-2' : 'md:col-span-3'}>
                    ...
                    <button
                        onClick={closeCellModal}
                        className="px-4 py-2 rounded-md border text-sm text-gray-600 hover:bg-gray-50"
                    >
                        Отмена
                    </button>

                    <button
                        onClick={clearCellFields}
                        className="px-4 py-2 rounded-md border text-sm text-gray-600 hover:bg-gray-50"
                    >
                        Очистить
                    </button>
                    ...
                    <button
                        ...
                        className={`inline-flex items-center gap-2 px-5 py-2 rounded-md text-sm text-white ${isSavingCell ? 'bg-green-400 cursor-wait' : 'bg-green-600 hover:bg-green-700'}`}
                    >
                        {isSavingCell ? (
                        <>
                            <i className="fas fa-spinner fa-spin" aria-hidden="true" /> Сохранение...
            ...
            {TrainingModalComponent && trainingModalState?.open && (
        ...
        function AudioPlayer({ audioSrc, uniqueKey }) {
            ...
            const rewind10 = ()=> jumpToTime(Math.max(0, (audioRef.current?.currentTime||0) - 10));
            const forward10 = ()=> jumpToTime(Math.min(duration, (audioRef.current?.currentTime||0) + 10));
            const handleSeek = (e)=>{
            ...
            if(!audioSrc) return (
                ...
                <div className="text-sm text-red-500">Аудио отсутствует</div>
                </div>
            );

            return (
                <div key={uniqueKey || audioSrc} className="p-3 bg-white rounded-lg border border-gray-200 shadow-sm">
                <div className="player-bar">
                    <div className="controls-left">
                    <button className="icon-btn" onClick={togglePlayPause} title={isPlaying ? 'Пауза' : 'Воспроизвести'}>
                        {isPlaying ? (
                    ...
                    <button className="icon-btn" onClick={rewind10} title="Назад 10с">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-5 h-5">
                    ...
                    <button className="icon-btn" onClick={forward10} title="Вперёд 10с">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-5 h-5">
                ...
                <audio ref={audioRef} className="player-hidden" src={audioSrc}></audio>
                </div>
            );
            }

            // AdminCallsUploadView — visual / ergonomic refinement (with Shuffle / Распределить)
            // NOTE: preserves original logic, added: "Распределить" button + preview modal + POST /api/admin/shuffle

            function pad(n) { return String(n).padStart(2, '0'); }
            function secondsToHms(sec) {
            if (sec == null || isNaN(Number(sec))) return '00:00:00';
            ...
            if (s.includes(':')) {
                ...
                const secPart = (parts[parts.length - 1] || '0').replace(',', '.');
                const sec = parseFloat(secPart) || 0;
                const min = parseInt(parts[parts.length - 2] || '0', 10) || 0;
                const hr = parts.length >= 3 ? (parseInt(parts[parts.length - 3] || '0', 10) || 0) : 0;
            ...
            function DurationInput({ valueSec, onChange, ariaLabel }) {
            const secs = valueSec == null ? 0 : Number(valueSec) || 0;
            const hh = Math.floor(secs / 3600);
            const mm = Math.floor((secs % 3600) / 60);
            const ss = Math.floor(secs % 60);

            function handlePartChange(part, v) {
                if (v === '' || v == null) {
                ...
                let newSec = secs;
                if (part === 'h') newSec = hmsToSeconds(v, mm, ss);
                if (part === 'm') newSec = hmsToSeconds(hh, v, ss);
                if (part === 's') newSec = hmsToSeconds(hh, mm, v);
                if (typeof onChange === 'function') onChange(newSec);
            }

            const inputClass = 'w-14 p-1 border rounded text-sm text-center bg-white';
            return (
                <div className="flex items-center gap-2">
                <input aria-label={ariaLabel ? ariaLabel + ' hours' : 'hours'} className={inputClass} type="number" min="0" value={hh > 0 ? hh : ''} placeholder="hh" onChange={e=>handlePartChange('h', e.target.value)} />
                <div className="text-sm">:</div>
                <input aria-label={ariaLabel ? ariaLabel + ' minutes' : 'minutes'} className={inputClass} type="number" min="0" max="59" value={mm > 0 ? mm : ''} placeholder="mm" onChange={e=>handlePartChange('m', e.target.value)} />
                <div className="text-sm">:</div>
                <input aria-label={ariaLabel ? ariaLabel + ' seconds' : 'seconds'} className={inputClass} type="number" min="0" max="59" value={ss > 0 ? ss : ''} placeholder="ss" onChange={e=>handlePartChange('s', e.target.value)} />
                </div>
            );
            }

        function AdminCallsUploadView({ onAssignSelected, user }) {
            // -- state kept as in original component --
            const [files, setFiles] = useState([]);
            const [minDurationSec, setMinDurationSec] = useState(null);
            const [maxDurationSec, setMaxDurationSec] = useState(null);
            const [variationSec, setVariationSec] = useState(0);
            ...
            const [editingValues, setEditingValues] = useState({ min: null, max: null, variation: 0, sampleCount: 0 });
            const fileInputRef = useRef(null);

            const [pinnedOperator, setPinnedOperator] = useState(false);
            ...
            const [onlyWithCsv, setOnlyWithCsv] = useState(false);
            const [selectedDirections, setSelectedDirections] = useState(['all']);
            const [selectedHireMonths, setSelectedHireMonths] = useState(['all']);
            const [expandedHireYears, setExpandedHireYears] = useState({});

            // New state: operatorView holds all CSV calls related to a viewed operator, split by matching vs non-matching
            const [operatorView, setOperatorView] = useState({ operatorKey: null, matching: [], nonMatching: [] });

            // --- Shuffle modal state ---
            const [showShuffleModal, setShowShuffleModal] = useState(false);
            const [shuffleSummary, setShuffleSummary] = useState(null);
            const [shuffling, setShuffling] = useState(false);
            const [shuffleResult, setShuffleResult] = useState(null);
            const [distributionTab, setDistributionTab] = useState('general');
            ...
            // unchanged. If you need the unabridged logic present here for runtime, simply copy your
            // original function bodies into the indicated placeholders.

            // --------- BEGIN original logic placeholders (replace with your full functions) ----------
            ...
            function parseDateDMY(str) {
                ...
                const iso = `${yyyy}-${String(mm).padStart(2,'0')}-${String(dd).padStart(2,'0')}T${timePart || '00:00:00'}`;
            ...
            function parseHireDateToDate(rawValue) {
            ...
            function getHireMonthKey(rawValue) {
                const dt = parseHireDateToDate(rawValue);
                if (!dt) return null;
                return `${dt.getFullYear()}-${String(dt.getMonth() + 1).padStart(2, '0')}`;
            }

            function parseCSVText(text) {
                if (window && window.Papa && window.Papa.parse) {
                const r = window.Papa.parse(text, { skipEmptyLines: true });
                return r.data;
                }
            ...
            function normalizeRow(row) {
                const r = row.map(cell => (cell == null ? '' : String(cell).trim()));
                if (r.length < 7) while (r.length < 7) r.push('');
                const [id, dtRaw, typeRaw, phone, project, operator, durationRaw] = r;
                const dt = parseDateDMY(dtRaw);
                const duration = parseDurationString(durationRaw);
                const type = (String(typeRaw || '')).toLowerCase().includes('вход') || String(typeRaw || '').toLowerCase().includes('in') ? 'in' : 'out';
                ...
                phone: phone || '',
                project: project || '',
                operator: operator || 'Неизвестный',
                durationSec: duration,
                };
            }

            async function handleFilesSelected(e) { const fl = Array.from(e.target.files || []); setFiles(fl); }
            ...
            async function handleParseFiles() {
                ...
                for (const f of files) {
                    ...
                    let start = 0;
                    if (parsed.length > 0) {
                    const first = parsed[0].map(c => String(c || '').toLowerCase()).join(';');
                    if (first.includes('id') || first.includes('дата') || first.includes('date')) start = 1;
                    }
                ...
                setCalls(allRows);
                const minDVal = minDurationSec === null ? null : Number(minDurationSec);
                const maxDVal = maxDurationSec === null ? null : Number(maxDurationSec);
                const varVal = variationSec === '' ? 0 : Number(variationSec);
            ...
            function recomputeOperatorMap(allRows, globalMin, globalMax, globalSampleCnt, globalVar, params = {}, keepSelection = false) {
                const raw = {};
                for (const c of allRows) {
                const op = c.operator || 'Неизвестный';
                if (!raw[op]) raw[op] = [];
                raw[op].push(c);
                }
                ...
                let list = raw[op].slice();
                list.sort((a,b) => (b.datetime?.getTime() || 0) - (a.datetime?.getTime() || 0));
                const opParams = params[op] || null;
                const minD = opParams && opParams.min != null ? opParams.min : globalMin;
                const maxD = opParams && opParams.max != null ? opParams.max : globalMax;
                const varD = opParams && opParams.variation != null ? opParams.variation : globalVar;
                const sampleCnt = opParams && (opParams.sampleCount != null) ? opParams.sampleCount : globalSampleCnt;

                if (minD != null || maxD != null) {
                    const baseMin = minD != null ? Number(minD) : 0;
                    const baseMax = maxD != null ? Number(maxD) : (minD != null ? Number(minD) : Infinity);
                    const computedMin = Math.max(0, (Number(baseMin) || 0) - (Number(varD) || 0));
                    const computedMax = baseMax === Infinity ? Infinity : (Number(baseMax) || 0) + (Number(varD) || 0);
                    list = list.filter(c => {
                    if (c.durationSec == null || isNaN(Number(c.durationSec))) return false;
                    const dur = Number(c.durationSec);
                    if (dur < computedMin) return false;
                    if (computedMax !== Infinity && dur > computedMax) return false;
                    return true;
                    });
                }
                ...
                summary[op] = {
                    count: list.length,
                    samples: sampleCnt > 0 ? list.slice(0, sampleCnt) : [],
                    all: list,
                    custom: !!opParams,
                };
                });

                setOperatorMap(summary);
                if (!keepSelection) {
                ...
                const baseMin = globalMin != null ? Number(globalMin) : 0;
                const baseMax = globalMax != null ? Number(globalMax) : (globalMin != null ? Number(globalMin) : Infinity);
                const computedMin = Math.max(0, (Number(baseMin) || 0) - (Number(globalVar) || 0));
                const computedMax = baseMax === Infinity ? null : (Number(baseMax) || 0) + (Number(globalVar) || 0);
                setActiveRange({ min: computedMin, max: computedMax });
                } else {
                setActiveRange(null);
                }
            }

            function countAnyCallsForDisplayName(displayName) {
                if (!displayName || !calls || calls.length === 0) return 0;
                const norm = String(displayName || '').trim().toLowerCase();
                let cnt = 0;
                for (const c of calls) {
                const co = String(c.operator || '').trim().toLowerCase();
                if (!co) continue;
                if (co === norm) { cnt++; continue; }
                if (co.includes(norm) || norm.includes(co)) { cnt++; continue; }
                }
                return cnt;
            }

            function findOperatorMapKeyByName(name) {
                if (!name) return null;
                const norm = String(name).trim().toLowerCase();
                if (operatorMap[norm] && operatorMap[norm].count != null) return norm;
                for (const k of Object.keys(operatorMap)) {
                if (String(k).trim().toLowerCase() === norm) return k;
                }
                for (const k of Object.keys(operatorMap)) {
                if (String(k).trim().toLowerCase().includes(norm) || norm.includes(String(k).trim().toLowerCase())) return k;
                }
                return null;
            }

            function getAllCallsForDisplayName(displayName) {
                // returns all parsed calls from CSV that seem to belong to this displayName (loose match)
                const res = [];
                if (!displayName || !calls) return res;
                const norm = String(displayName).trim().toLowerCase();
                for (const c of calls) {
                const co = String(c.operator || '').trim().toLowerCase();
                if (!co) continue;
                if (co === norm || co.includes(norm) || norm.includes(co)) res.push(c);
                }
                return res;
            }

            function handleShowOperator(opKey) {
                const foundKey = findOperatorMapKeyByName(opKey);
                // prefer map key if exists, else use opKey as name
                const keyToUse = foundKey || opKey;
                if (!keyToUse) return;

                // get matching calls (those considered "подходящие" according to operatorMap filtered list)
                const matching = (operatorMap[keyToUse] && operatorMap[keyToUse].all) ? operatorMap[keyToUse].all.slice() : [];

                // get all CSV calls that relate to this operator by name
                const allRelated = getAllCallsForDisplayName(opKey);

                // nonMatching = allRelated - matching (by id)
                const matchingIds = new Set(matching.map(m => m.id));
                const nonMatching = allRelated.filter(c => !matchingIds.has(c.id));

                setSelectedOperator(keyToUse);
                setPinnedOperator(true);

                // select matching calls by default
                const s = new Set(selectedCalls);
                matching.forEach(c => s.add(c.id));
                setSelectedCalls(s);

                // store view split
                setOperatorView({ operatorKey: keyToUse, matching, nonMatching });

                // scroll to panel
                setTimeout(() => { window.scrollTo({ top: document.body.scrollHeight, behavior: 'smooth' }); }, 50);
            }

            function openEditOperator(opKey) {
                const key = findOperatorMapKeyByName(opKey) || opKey;
                setEditingOperator(key);
                const existing = operatorParams[key] || { min: null, max: null, variation: 0, sampleCount: 0 };
                setEditingValues({ min: existing.min ?? null, max: existing.max ?? null, variation: existing.variation ?? 0, sampleCount: existing.sampleCount ?? 0 });
            }

            function saveOperatorParams() {
                const op = editingOperator;
                const cleaned = { ...editingValues };
                cleaned.sampleCount = Number(cleaned.sampleCount) || 0;
                setOperatorParams(prev => {
                const updated = { ...prev, [op]: cleaned };
                recomputeOperatorMap(calls, minDurationSec, maxDurationSec, Number(sampleCount) || 0, Number(variationSec) || 0, updated, true);
                return updated;
                });
                setEditingOperator(null);
            }

            function resetOperatorParams(op) {
                setOperatorParams(prev => {
                const clone = { ...prev };
                delete clone[op];
                return clone;
                });
                if (editingOperator === op) setEditingOperator(null);
            }
            ...
            async function fetchDirections() {
                ...
                if (data && data.status === 'success' && Array.isArray(data.directions)) {
                    setDirections(data.directions.filter(d => d.isActive !== false));
                } else {
                    setDirections([]);
                    if (typeof showToast === 'function') showToast?.(data?.error || 'Не удалось получить направления', 'error');
                }
                } catch (err) {
                console.error('Fetch directions error:', err);
                setDirections([]);
                if (typeof showToast === 'function') showToast?.(err.response?.data?.error || 'Ошибка при получении направлений', 'error');
                }
            }

            async function fetchSvOperators(month = selectedMonth) {
                try {
                if (typeof axios === 'undefined') return;
                ...
                const data = resp.data;
                if (data && data.status === 'success' && Array.isArray(data.operators)) {
                    const ops = data.operators.map(op => ({
                    id: op.id,
                    name: op.name,
                    ...
                    const dirsMap = {};
                    ops.forEach(o => {
                    const did = o.direction_id != null ? String(o.direction_id) : 'null';
                    const dname = (o.direction_name || o.direction || 'Без направления').trim();
                    if (!dirsMap[did]) dirsMap[did] = { id: did, name: dname };
                    });
                    const builtDirs = Object.values(dirsMap).sort((a,b) => String((a.name||'')).localeCompare(String((b.name||''))));
                    setDirections(builtDirs);

                } else {
                    // fallback handling as in original
                    if (data && Array.isArray(data.operators)) setSvOperators(data.operators);
                    else if (data && Array.isArray(data.users)) setSvOperators(data.users);
                    else if (data && Array.isArray(data.data)) setSvOperators(data.data);
                    else {
                    const collected = [];
                    if (data && Array.isArray(data.sv_list)) {
                        data.sv_list.forEach(sv => { (sv.operators || []).forEach(op => collected.push(op)); });
                    }
                    setSvOperators(collected);

                    const dirsMap = {};
                    collected.forEach(o => {
                        const did = (o.direction_id != null ? String(o.direction_id) : (o.direction_id === 0 ? '0' : 'null'));
                        const dname = (o.direction_name || o.direction || 'Без направления').trim();
                        if (!dirsMap[did]) dirsMap[did] = { id: did, name: dname };
                    });
                    const builtDirs = Object.values(dirsMap).sort((a,b) => String((a.name||'')).localeCompare(String((b.name||''))));
                    if (builtDirs.length > 0) setDirections(builtDirs);
                    }
                }
                } catch (err) {
                console.error('Fetch sv data error:', err);
                setSvOperators([]);
                if (typeof showToast === 'function') showToast?.(err.response?.data?.error || 'Ошибка при получении данных операторов', 'error');
            ...
            const hireMonthTree = React.useMemo(() => {
                const monthNames = ['Январь', 'Февраль', 'Март', 'Апрель', 'Май', 'Июнь', 'Июль', 'Август', 'Сентябрь', 'Октябрь', 'Ноябрь', 'Декабрь'];
                const years = {};

                (svOperators || []).forEach(op => {
                const monthKey = getHireMonthKey(op?.hire_date || op?.hireDate);
                if (!monthKey) return;
                ...
                .map(year => ({
                    year,
                    months: Array.from(years[year])
                    .sort((a, b) => Number(a) - Number(b))
                    .map(month => ({
                        value: `${year}-${month}`,
                        label: monthNames[Number(month) - 1] || month
                    }))
                }));
            }, [svOperators]);

            const selectedHireMonthLabels = React.useMemo(() => {
                if (!selectedHireMonths || selectedHireMonths.length === 0 || selectedHireMonths.includes('all')) return '';
                const selectedSet = new Set(selectedHireMonths.map(String));
                const labels = [];
                for (const yearItem of hireMonthTree) {
                for (const monthItem of yearItem.months) {
                    if (selectedSet.has(monthItem.value)) labels.push(`${monthItem.label} ${yearItem.year}`);
                }
                }
                return labels.join(', ');
            }, [hireMonthTree, selectedHireMonths]);

            function toggleHireYear(year) {
                setExpandedHireYears(prev => ({ ...prev, [year]: !prev[year] }));
            }

            function toggleHireMonthSelection(monthValue) {
                const value = String(monthValue);
                const current = (selectedHireMonths || []).slice();

                if (current.includes('all')) {
                if (value === 'all') {
                    setSelectedHireMonths(['all']);
                } else {
                    setSelectedHireMonths([value]);
                }
                return;
                }
                ...
                setSelectedHireMonths(next.length > 0 ? next : ['all']);
                return;
                }

                setSelectedHireMonths(current.concat(value));
            }

            useEffect(() => {
                if (!selectedHireMonths || selectedHireMonths.length === 0 || selectedHireMonths.includes('all')) return;
                const available = new Set(hireMonthTree.flatMap(y => y.months.map(m => m.value)));
                const filtered = selectedHireMonths.filter(v => available.has(String(v)));
                const hasChanges = filtered.length !== selectedHireMonths.length
                || filtered.some((v, idx) => v !== selectedHireMonths[idx]);
                if (hasChanges) setSelectedHireMonths(filtered.length > 0 ? filtered : ['all']);
            }, [hireMonthTree, selectedHireMonths]);
            ...
            function toggleStatusSelection(status) {
                ...
                if (s.includes(lower)) {
                const idx = s.indexOf(lower); s.splice(idx,1);
                if (s.length === 0) setSelectedStatuses(['all']); else setSelectedStatuses(s);
                } else {
                setSelectedStatuses(prev => prev.filter(Boolean).concat(lower));
                }
            }

            const directionOptions = React.useMemo(() => {
                return (directions || []).map(d => ({ id: d.id, name: d.name || d.title || d.displayName || d.direction_name || d.direction }));
            }, [directions]);

            function toggleDirectionSelection(dirId) {
                const s = (selectedDirections || []).slice();
                const lower = String(dirId);
                if (s.includes('all')) {
                if (lower === 'all') return setSelectedDirections(['all']);
                setSelectedDirections([lower]);
                return;
                }
                if (s.includes(lower)) {
                const idx = s.indexOf(lower); s.splice(idx,1);
                ...
                setSelectedDirections(prev => prev.filter(Boolean).concat(lower));
                }
            }

            const filteredSvOperators = React.useMemo(() => {
                let list = svOperators.slice();
                ...
                const dirSet = new Set(selectedDirections.map(s=>String(s)));
                list = list.filter(op => dirSet.has(String(op.direction_id)));
                }
                if (selectedHireMonths && selectedHireMonths.length > 0 && !selectedHireMonths.includes('all')) {
                const monthSet = new Set(selectedHireMonths.map(String));
                list = list.filter(op => monthSet.has(getHireMonthKey(op?.hire_date || op?.hireDate)));
                }
                return list;
            }, [svOperators, selectedStatuses, selectedDirections, selectedHireMonths]);

            function getDesiredSampleCountForOperator(displayName, matchedKey) {
                if (matchedKey && operatorParams[matchedKey] && operatorParams[matchedKey].sampleCount != null) {
                return Number(operatorParams[matchedKey].sampleCount) || 0;
                }
                if (displayName && operatorParams[displayName] && operatorParams[displayName].sampleCount != null) {
                return Number(operatorParams[displayName].sampleCount) || 0;
                }
                return Number(sampleCount) || 0;
            }

            const insufficientOperators = React.useMemo(() => {
                const res = [];
                const apiNameSet = new Set((svOperators || []).map(o => String(o.name || o.login || o.operator_name || o.id || '').trim().toLowerCase()));
                for (const op of filteredSvOperators) {
                const displayName = op.name || op.login || op.operator_name || String(op.id || '—');
                const matchedKey = findOperatorMapKeyByName(displayName);
                const info = matchedKey ? operatorMap[matchedKey] : null;
                const csvMatchedCount = info ? info.count : 0;
                const anyCsvCount = countAnyCallsForDisplayName(displayName);
                if (onlyWithCsv && anyCsvCount === 0) continue;
                const desired = getDesiredSampleCountForOperator(displayName, matchedKey);
                const isInsufficient = (Number(desired) || 0) > 0 && (Number(csvMatchedCount) || 0) < Number(desired);
                if (isInsufficient) res.push({ op, csvCount: csvMatchedCount, desired, matchedKey });
                }
                if (selectedDirections.includes('all') && selectedHireMonths.includes('all')) {
                for (const k of Object.keys(operatorMap || {})) {
                    const info = operatorMap[k];
                    if (!info || info.count === 0) continue;
                    const nk = String(k).trim().toLowerCase();
                    if (apiNameSet.has(nk)) continue;
                    const desired = operatorParams[k]?.sampleCount != null ? Number(operatorParams[k].sampleCount) : Number(sampleCount) || 0;
                    if (onlyWithCsv && info.count === 0) continue;
                    if ((Number(desired) || 0) > 0 && (Number(info.count) || 0) < Number(desired)) {
                    res.push({ op: null, csvName: k, csvCount: info.count, desired, matchedKey: k });
                    }
                }
                }
                res.sort((a,b) => {
                const aShort = (a.desired || 0) - (a.csvCount || 0);
                const bShort = (b.desired || 0) - (b.csvCount || 0);
                if (aShort !== bShort) return bShort - aShort;
                const an = a.op ? (a.op.name || a.op.id) : a.csvName;
                const bn = b.op ? (b.op.name || b.op.id) : b.csvName;
                return String(an || '').localeCompare(String(bn || ''));
                });
                return res;
            }, [filteredSvOperators, operatorMap, operatorParams, sampleCount, onlyWithCsv, selectedDirections, selectedHireMonths, svOperators, calls]);

            const groupedBySvThenDir = React.useMemo(() => {
                const map = {};
                const insufficientNames = new Set(insufficientOperators.map(i => (i.op ? (i.op.name || i.op.id) : (i.csvName || '')).toString().trim().toLowerCase()));
                ...
                Object.keys(map[sv]).forEach(dir => {
                    ...
                    const bTs = bHire ? bHire.getTime() : null;
                    if (aTs != null && bTs != null && aTs !== bTs) return aTs - bTs;
                    if (aTs != null && bTs == null) return -1;
                    if (aTs == null && bTs != null) return 1;
                    const aDisplay = a.name || a.login || a.operator_name || String(a.id || '');
                    const bDisplay = b.name || b.login || b.operator_name || String(b.id || '');
                    return String(aDisplay).localeCompare(String(bDisplay));
                    });
                });
                });
                return map;
            }, [filteredSvOperators, insufficientOperators]);

            const unmatchedCsvOperators = React.useMemo(() => {
                if (!selectedHireMonths.includes('all')) return [];
                const apiNames = new Set((svOperators || []).map(o => String(o.name || o.login || o.operator_name || o.id || '').trim().toLowerCase()));
                return Object.keys(operatorMap || {}).filter(k => {
                if (!operatorMap[k] || operatorMap[k].count === 0) return false;
                const nk = String(k).trim().toLowerCase();
                if (apiNames.has(nk)) return false;
                for (const a of apiNames) {
                    if (a.includes(nk) || nk.includes(a)) return false;
                }
                return true;
                }).sort((a,b) => {
                const aCsv = operatorMap[a]?.count ?? 0;
                const bCsv = operatorMap[b]?.count ?? 0;
                const aDesired = operatorParams[a]?.sampleCount != null ? Number(operatorParams[a].sampleCount) : Number(sampleCount) || 0;
                const bDesired = operatorParams[b]?.sampleCount != null ? Number(operatorParams[b].sampleCount) : Number(sampleCount) || 0;
                const aIns = aCsv < aDesired ? 1 : 0;
                const bIns = bCsv < bDesired ? 1 : 0;
                if (aIns !== bIns) return bIns - aIns;
                return String(a).localeCompare(String(b));
                });
            }, [operatorMap, svOperators, operatorParams, sampleCount, selectedHireMonths]);

            const operatorSelectionList = React.useMemo(() => {
                const rows = [];
                const seen = new Set();
                ...
                const matchedKey = findOperatorMapKeyByName(displayName);
                const info = matchedKey ? operatorMap[matchedKey] : null;
                const csvCount = Number(info?.count || 0);
                const anyCsvCount = countAnyCallsForDisplayName(displayName);
                if (onlyWithCsv && anyCsvCount === 0) continue;
                rows.push({
                    name: displayName,
                    direction: op.direction_name || op.direction || '',
                    supervisor: op.supervisor_name || op.sv_name || op.supervisor || '',
                    status: op.status || '',
                    hire_date: op.hire_date || op.hireDate || null,
                    apiCount: Number(op.call_count || 0),
                    csvCount,
                    matchedKey
                });
                seen.add(key);
                ...
                for (const csvName of unmatchedCsvOperators) {
                    const key = normalizeName(csvName);
                    if (!key || seen.has(key)) continue;
                    const info = operatorMap[csvName];
                    const csvCount = Number(info?.count || 0);
                    if (onlyWithCsv && csvCount === 0) continue;
                    rows.push({
                    name: csvName,
                    direction: 'CSV-only',
                    supervisor: '-',
                ...
                const query = normalizeName(operatorSearch);
                return rows
                .filter(r => {
                    if (!query) return true;
                    return normalizeName(r.name).includes(query)
                    || normalizeName(r.direction).includes(query)
                    || normalizeName(r.supervisor).includes(query);
                })
                .sort((a, b) => {
                    const aHire = parseHireDateToDate(a?.hire_date);
                    const bHire = parseHireDateToDate(b?.hire_date);
                    const aTs = aHire ? aHire.getTime() : null;
                    const bTs = bHire ? bHire.getTime() : null;
                    if (aTs != null && bTs != null && aTs !== bTs) return aTs - bTs;
                    if (aTs != null && bTs == null) return -1;
            ...
            }, [filteredSvOperators, selectedDirections, unmatchedCsvOperators, operatorMap, onlyWithCsv, operatorSearch, calls]);

            const selectedOperatorLowerSet = React.useMemo(() => {
                return new Set(Array.from(selectedOperatorsForShuffle || []).map(v => normalizeName(v)));
            }, [selectedOperatorsForShuffle]);

            useEffect(() => {
                recomputeOperatorMap(calls, minDurationSec, maxDurationSec, Number(sampleCount) || 0, Number(variationSec) || 0, operatorParams, !!selectedOperator);
                // eslint-disable-next-line react-hooks/exhaustive-deps
            }, [operatorParams, calls, minDurationSec, maxDurationSec, sampleCount, variationSec, selectedOperator]);

            const visibleOperators = Object.entries(operatorMap).filter(([op, info]) => info && info.count > 0);
            function applyFilters() { recomputeOperatorMap(calls, minDurationSec === null ? null : Number(minDurationSec), maxDurationSec === null ? null : Number(maxDurationSec), Number(sampleCount) || 0, Number(variationSec) || 0, operatorParams); }

            // Combined action: parse uploaded files, fetch operators for the selected month, then apply filters
            async function splitAll() {
                if (!files || files.length === 0) return;
                try {
                    // parse files (this already recomputes operator map)
                    await handleParseFiles();
                } catch (err) {
                    console.error('Error during parsing in splitAll:', err);
                }
                try {
                    // refresh operators from server for the selected month
                    await fetchSvOperators(selectedMonth);
                } catch (err) {
                    console.error('Error fetching operators in splitAll:', err);
                }
                try {
                    // ensure filters are applied (recompute map)
                    applyFilters();
                } catch (err) {
                    console.error('Error applying filters in splitAll:', err);
                }
            }

            function handleClosePanel() {
                setPinnedOperator(false);
                setSelectedOperator(null);
                setOperatorView({ operatorKey: null, matching: [], nonMatching: [] });
            }
            ...
            function toggleSelectCall(callId) {
                const s = new Set(selectedCalls);
                if (s.has(callId)) s.delete(callId); else s.add(callId);
                setSelectedCalls(s);
            }

            function selectAllForOperator(opKey) {
                const foundKey = findOperatorMapKeyByName(opKey);
                if (!(foundKey && operatorMap[foundKey] && operatorMap[foundKey].count > 0)) return;
                const s = new Set(selectedCalls);
                (operatorMap[foundKey]?.all || []).forEach(c => s.add(c.id));
                setSelectedCalls(s);
            }

            function clearSelectedCallsForOperator(opKey) {
                const foundKey = findOperatorMapKeyByName(opKey);
                const matching = foundKey ? (operatorMap[foundKey]?.all || []) : [];
                const allRelated = getAllCallsForDisplayName(opKey);
            ...
            function countSelectedCallsForOperator(opKey) {
                if (!opKey) return 0;
                const foundKey = findOperatorMapKeyByName(opKey);
                const matching = foundKey ? (operatorMap[foundKey]?.all || []) : [];
                const allRelated = getAllCallsForDisplayName(opKey);
                ...
                let count = 0;
                relatedIds.forEach(id => { if (selectedCalls.has(id)) count += 1; });
                return count;
            }

            function toggleOperatorForShuffle(name) {
                const normalized = normalizeName(name);
                if (!normalized) return;
                setSelectedOperatorsForShuffle(prev => {
                const next = new Set(prev || []);
                let existing = null;
                for (const item of next) {
                    if (normalizeName(item) === normalized) { existing = item; break; }
                }
                if (existing != null) next.delete(existing);
                else next.add(name);
                return next;
                });
            }

            function clearSelectedOperatorsForShuffle() {
                setSelectedOperatorsForShuffle(new Set());
            }

            function selectAllVisibleOperatorsForShuffle() {
                setSelectedOperatorsForShuffle(prev => {
                const next = new Set(prev || []);
                operatorSelectionList.forEach(item => next.add(item.name));
                return next;
                });
            }
            ...
            function handleAssign() {
                ...
                for (const p of picks) {
                    csv.push([p.id, p.datetimeRaw || '', p.type, p.phone, p.project, p.operator, p.durationSec].map(x => '"'+String(x).replace(/"/g,'""')+'"').join(','));
                }
                const blob = new Blob([csv.join('\n')], { type: 'text/csv;charset=utf-8;' });
                const url = URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url; a.download = 'selected_calls.csv'; a.click(); URL.revokeObjectURL(url);
                }
            }

            // ----- NEW: shuffle / распределить logic -----
            function pickRandom(arr = [], n = 0) {
                const src = (arr || []).slice();
                const out = [];
            ...
            function handlePrepareShuffle() {
                // Build list of target operators according to current filters
                const toProcess = [];
                const restrictBySelectedOperators = distributionTab === 'by_operator' && selectedOperatorsForShuffle.size > 0;
                const selectedLower = new Set(Array.from(selectedOperatorsForShuffle).map(v => normalizeName(v)));
                ...
                // API operators visible after filters
                for (const op of filteredSvOperators) {
                const displayName = op.name || op.login || op.operator_name || String(op.id || '-');
                if (restrictBySelectedOperators && !selectedLower.has(normalizeName(displayName))) continue;
                const matchedKey = findOperatorMapKeyByName(displayName);
                const available = matchedKey ? (operatorMap[matchedKey]?.all || []) : getAllCallsForDisplayName(displayName);
                const anyCsvCount = countAnyCallsForDisplayName(displayName);
                if (onlyWithCsv && anyCsvCount === 0) continue; // skip
                const desired = getDesiredSampleCountForOperator(displayName, matchedKey);
                if ((Number(desired) || 0) <= 0 && distributionTab !== 'by_operator') continue;
                toProcess.push({ operatorName: displayName, matchedKey, available });
                }

                // CSV-only operators (unmatched) - include when directions selection is 'all'
                if (selectedDirections.includes('all')) {
                for (const csvName of unmatchedCsvOperators) {
                    if (restrictBySelectedOperators && !selectedLower.has(normalizeName(csvName))) continue;
                    const info = operatorMap[csvName];
                    if (!info || info.count === 0) continue;
                    if (onlyWithCsv && info.count === 0) continue;
                    const desired = operatorParams[csvName]?.sampleCount != null ? Number(operatorParams[csvName].sampleCount) : Number(sampleCount) || 0;
                    if ((Number(desired) || 0) <= 0 && distributionTab !== 'by_operator') continue;
                    toProcess.push({ operatorName: csvName, matchedKey: csvName, available: info.all || [] });
                }
                }

                // Now for each operator pick up to desired random calls or use manual picks in "by operator" tab
                const distribution = [];
                const insufficient = [];
                let totalPicked = 0;
                const chosenCallIds = new Set();
                ...
                const desired = getDesiredSampleCountForOperator(it.operatorName, it.matchedKey);
                const available = (it.available || []).filter(c => c && c.id);
                const dedupAvailable = available.filter(c => !chosenCallIds.has(c.id));

                const manualPicked = distributionTab === 'by_operator'
                    ? dedupAvailable.filter(c => selectedCalls.has(c.id))
                    : [];

                let picked = [];
                let desiredInSummary = Number(desired) || 0;
                let pickMode = 'auto';

                if (manualPicked.length > 0) {
                    picked = manualPicked;
                    desiredInSummary = manualPicked.length;
                    pickMode = 'manual';
                } else {
                    const pickCount = Math.min(Number(desired) || 0, dedupAvailable.length);
                    picked = pickRandom(dedupAvailable, pickCount);
                    if (dedupAvailable.length < (Number(desired) || 0)) {
                    insufficient.push({ operator: it.operatorName, desired: Number(desired) || 0, available: dedupAvailable.length });
                    }
                }

                picked.forEach(p => chosenCallIds.add(p.id));
                distribution.push({
                    operator: it.operatorName,
                    desired: desiredInSummary,
                    available: dedupAvailable.length,
                    picked,
                    mode: pickMode
                });
                ...
                // compute min/max duration across picked
                const allPicked = distribution.flatMap(d => d.picked || []);
                const durations = allPicked.map(p => (p && p.durationSec != null ? Number(p.durationSec) : null)).filter(v => v != null && !isNaN(v));
                ...
                totalPicked,
                insufficient,
                enoughCount: distribution.length - insufficient.length,
                minDur,
                maxDur,
                filters: {
                    statuses: selectedStatuses,
                    directions: selectedDirections,
                    hireMonth: selectedHireMonths.includes('all') ? 'all' : selectedHireMonths.join(','),
                    hireMonths: selectedHireMonths,
                    onlyWithCsv,
                    month: selectedMonth,
                    tab: distributionTab,
                    selectedOperators: Array.from(selectedOperatorsForShuffle)
                }
                };

                setShuffleSummary(summary);
                setShowShuffleModal(true);
            }
            async function handleConfirmShuffle() {
            if (!shuffleSummary) return;
            setShuffling(true);
            ...
            try {
                ...
                const payload = {
                month: selectedMonth,
                filters: shuffleSummary.filters,
                distribution: shuffleSummary.distribution.map(d => ({
                    operator: d.operator,
                    desired: d.desired,
                    available: d.available,
                    calls: (d.picked || []).map(c => ({ id: c.id, datetimeRaw: c.datetimeRaw, phone: c.phone, durationSec: c.durationSec, project: c.project }))
                })),
                total: shuffleSummary.totalPicked
                };

                const resp = await axios.post(`${API_BASE_URL}/api/admin/shuffle`, payload, { headers });
                const data = resp.data;

                // Установим результат и отобразим его в UI (в модальном)
                setShuffleResult({
                ok: resp.status === 200 && (data?.status === 'ok' || data?.status === 'success' || !data?.status),
                ...
                payload: err.response?.data || { error: err.message || 'Network error' }
                });
            } finally {
                setShuffling(false);
            }
            ...
            return (
                ...
                <div className="mb-4 border-b border-gray-200">
                    ...
                    <button
                        onClick={() => setDistributionTab('general')}
                        className={`px-3 py-1.5 rounded-md text-sm ${distributionTab === 'general' ? 'bg-white shadow text-blue-700' : 'text-gray-600 hover:text-gray-800'}`}
                    >
                        Общее
                    </button>
                    <button
                        onClick={() => setDistributionTab('by_operator')}
                        className={`px-3 py-1.5 rounded-md text-sm ${distributionTab === 'by_operator' ? 'bg-white shadow text-blue-700' : 'text-gray-600 hover:text-gray-800'}`}
                    >
                        По оператору
                    </button>
                    </div>
                ...
                <div className="space-y-4 mb-4">
                    ...
                    ) : Object.keys(groupedBySvThenDir).length === 0 && unmatchedCsvOperators.length === 0 ? (
                        ...
                        <details key={svName} className="p-3 bg-white rounded shadow-sm border">
                            ...
                            {Object.entries(dirs).map(([dirName, ops]) => (
                                ...
                                <ul className="space-y-1">
                                    ...
                                    const matchedKey = findOperatorMapKeyByName(displayName);
                                    const info = matchedKey ? operatorMap[matchedKey] : null;
                                    const csvCount = info ? info.count : 0;
                                    const anyCsvCount = countAnyCallsForDisplayName(displayName);
                                    if (onlyWithCsv && anyCsvCount === 0) return null;
                                    const apiCount = Number(op.call_count || 0);
                                    const desired = getDesiredSampleCountForOperator(displayName, matchedKey);
                                    return (
                                        <li
                                        key={op.id || displayName}
                                        className="flex items-center justify-between p-3 rounded-xl border bg-white hover:shadow-sm transition-all duration-150"
                                        >
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            {/* Статус и Custom теперь сразу под направлением */}
                                        ...
                                        <div className="flex items-center gap-2 flex-wrap justify-end text-xs">
                                            ...
                                            <button
                                            onClick={() => openEditOperator(displayName)}
                                            title="Настройки"
                                            className="flex items-center gap-1 px-3 py-1 border rounded-lg text-sm bg-gray-50 hover:bg-gray-100 transition-colors"
                                            >
                                            <i className="fa-solid fa-gear text-gray-500"></i>
                ...
                {showShuffleModal && shuffleSummary && (
                <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
                    {/* Backdrop */}
                    <div
                    className="absolute inset-0 bg-gradient-to-br from-black/40 to-black/25 backdrop-blur-sm"
                    onClick={() => { setShowShuffleModal(false); setShuffleResult(null); }}
                    aria-hidden="true"
                    />

                    {/* Modal */}
                    <div className="relative z-10 w-full max-w-4xl transform-gpu rounded-3xl bg-white shadow-2xl ring-1 ring-black/5 overflow-hidden transition-all duration-200">
                    {/* Header */}
                    <div className="flex items-center justify-between gap-4 px-6 py-5 bg-white/60 border-b">
                        <div className="flex items-center gap-3 min-w-0">
                        <div className="flex items-center justify-center w-12 h-12 rounded-lg bg-gradient-to-br from-green-100 to-green-50">
                            <svg xmlns="http://www.w3.org/2000/svg" className="w-6 h-6 text-green-700" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4" /></svg>
                        </div>
                        <div className="min-w-0">
                            <h3 className="text-lg font-semibold leading-tight truncate">Предпросмотр распределения</h3>
                            <p className="text-xs text-gray-500 truncate">Проверка результата перед отправкой. Списки выбранных звонков скрыты в целях приватности.</p>
                        </div>
                        </div>

                        <div className="flex flex-col items-end">
                        <div className="text-xs text-gray-500">Всего к распределению</div>
                        <div className="mt-1 text-lg font-semibold text-gray-800">{shuffleSummary.totalPicked}</div>
                        </div>
                    </div>

                    <div className="p-6 max-h-[65vh] overflow-auto space-y-5">
                        {/* Top summary cards */}
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        <div className="p-3 bg-gradient-to-br from-gray-50 to-white rounded-xl border flex flex-col">
                            <div className="text-xs text-gray-500">Доступные операторы</div>
                            <div className="mt-2 text-sm font-medium text-gray-800">{shuffleSummary.enoughCount}</div>
                            <div className="text-xs text-gray-400 mt-2">Операторы, у которых достаточно звонков</div>
                        </div>

                        <div className="p-3 bg-gradient-to-br from-yellow-50 to-white rounded-xl border flex flex-col">
                            <div className="text-xs text-gray-500">Операторы с нехваткой</div>
                            <div className="mt-2 text-sm font-medium text-gray-800">{shuffleSummary.insufficient.length}</div>
                            <div className="text-xs text-gray-400 mt-2">Требуется ручная проверка</div>
                        </div>

                        <div className="p-3 bg-gradient-to-br from-blue-50 to-white rounded-xl border flex flex-col">
                            <div className="text-xs text-gray-500">Диапазон длительностей</div>
                            <div className="mt-2 text-sm font-medium text-gray-800">{shuffleSummary.minDur != null ? secondsToHms(shuffleSummary.minDur) : '—'} — {shuffleSummary.maxDur != null ? secondsToHms(shuffleSummary.maxDur) : '—'}</div>
                            <div className="text-xs text-gray-400 mt-2">Мин / Макс среди выбранных</div>
                        </div>
                        </div>

                        {/* Distribution — без перечисления конкретных звонков */}
                        <div>
                        <div className="flex items-center justify-between mb-3">
                            <h4 className="text-sm font-medium">Детали по операторам</h4>
                            <div className="text-xs text-gray-500">Скрыты конкретные звонки — отображаются только агрегаты</div>
                        </div>

                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            {shuffleSummary.distribution.map((d, idx) => {
                            const picked = d.picked || [];
                            const pickedCount = picked.length;
                            const totalSec = picked.reduce((s, p) => s + (p.durationSec || 0), 0);
                            const avgSec = pickedCount ? Math.round(totalSec / pickedCount) : 0;
                            return (
                                <div key={d.operator + idx} className="p-3 bg-white border rounded-2xl shadow-sm hover:shadow-md transition-shadow">
                                <div className="flex items-start justify-between gap-3">
                                    <div className="min-w-0">
                                    <div className="flex items-center gap-2">
                                        <div className="text-sm font-medium truncate">{d.operator}</div>
                                        {d.available < d.desired ? (
                                        <span className="ml-1 text-xs bg-red-50 text-red-700 px-2 py-0.5 rounded-full">Нехватка</span>
                                        ) : (
                                        <span className="ml-1 text-xs bg-green-50 text-green-700 px-2 py-0.5 rounded-full">ОК</span>
                                        )}
                                    </div>
                                    <div className="text-xs text-gray-500 mt-1">подано: <strong>{pickedCount}</strong> / требовалось: <strong>{d.desired}</strong> / доступно: <strong>{d.available}</strong></div>
                                    </div>

                                    <div className="min-w-[160px] text-right">
                                    <div className="text-xs text-gray-600">Общая длительность</div>
                                    <div className="mt-1 text-sm font-medium text-gray-800">{pickedCount ? secondsToHms(totalSec) : '—'}</div>
                                    <div className="text-xs text-gray-400 mt-1">Средняя: {pickedCount ? secondsToHms(avgSec) : '—'}</div>
                                    </div>
                                </div>
                                </div>
                            );
                        ...
                        {/* Server response — только после отправки */}
                        {shuffleResult && (
                        <div className="mt-2 p-4 rounded-2xl bg-gradient-to-br from-gray-50 to-white border shadow-inner">
                            <div className="flex items-start gap-4">
                            <div className="shrink-0 mt-1">
                                {shuffleResult.ok ? (
                                <div className="w-12 h-12 rounded-xl bg-green-50 flex items-center justify-center">
                                    <svg xmlns="http://www.w3.org/2000/svg" className="w-6 h-6 text-green-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7" /></svg>
                                </div>
                                ) : (
                                <div className="w-12 h-12 rounded-xl bg-red-50 flex items-center justify-center">
                                    <svg xmlns="http://www.w3.org/2000/svg" className="w-6 h-6 text-red-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" /></svg>
                                </div>
                                )}
                            </div>
                            ...
                            <div className="flex-1">
                                ...
                                <div>
                                    ...
                                    <div className={`mt-1 text-base font-semibold ${shuffleResult.ok ? 'text-green-700' : 'text-red-700'}`}>
                                    {shuffleResult.ok ? 'Успешно' : `Ошибка (HTTP ${shuffleResult.httpStatus ?? '—'})`}
                                    </div>
                                </div>
                                ...
                                <div className="mt-3 text-xs text-gray-700 space-y-2">
                                    {Array.isArray(shuffleResult.payload.missing_operators) && shuffleResult.payload.missing_operators.length > 0 && (
                                    <div>Не найденные операторы: <strong>{shuffleResult.payload.missing_operators.join(', ')}</strong></div>
                                    )}

                                    {Array.isArray(shuffleResult.payload.errors) && shuffleResult.payload.errors.length > 0 && (
                                    <div>
                                        Ошибки ({shuffleResult.payload.errors.length}):
                                        <ul className="list-disc ml-5 mt-1">
                                        {shuffleResult.payload.errors.map((e, i) => <li key={i} className="text-xs">{e.external_id ? `${e.external_id}: ` : ''}{e.error || JSON.stringify(e)}</li>)}
                                        </ul>
                                    </div>
                                    )}

                                    <details className="mt-2 text-xs text-gray-500">
                                    <summary className="cursor-pointer">Показать полный ответ (json)</summary>
                                    <pre className="text-xs whitespace-pre-wrap mt-2 max-h-52 overflow-auto bg-gray-50 p-2 rounded">{JSON.stringify(shuffleResult.payload, null, 2)}</pre>
                                    </details>
                                </div>
                                )}
                            </div>
                    ...
                    {/* Footer actions */}
                    <div className="px-6 py-4 border-t bg-white/60 flex items-center justify-between gap-3">
                        <div className="text-xs text-gray-500">Подтверждение затронет {shuffleSummary.totalPicked} звонков.</div>
            ...
            const pushCentered = (c, size) => {
                const center = snap5(c);
            ...
            const sorted = arr.slice().sort((x,y) => x.start - y.start);
            const res = [ { ...sorted[0] } ];
            for (let i = 1; i < sorted.length; i++) {
                const cur = sorted[i];
                const last = res[res.length -1];
                if (cur.start <= last.end) last.end = Math.max(last.end, cur.end); else res.push({ ...cur });
            }
            return res;
            };
            ...
            { id: 1, name: "Али", shifts: {} },
            { id: 2, name: "Бек", shifts: {} },
            { id: 3, name: "Самира", shifts: {} },
            { id: 4, name: "Данияр", shifts: {} },
            ];

            function startOfMonth(d) { return new Date(d.getFullYear(), d.getMonth(), 1); }
            function endOfMonth(d) { return new Date(d.getFullYear(), d.getMonth() + 1, 0); }
            function weekStart(d) { const day = d.getDay() || 7; const s = new Date(d); s.setDate(d.getDate() - (day - 1)); s.setHours(0,0,0,0); return s; }
            function weekEnd(d) { const s = weekStart(d); const e = addDays(s, 6); e.setHours(23,59,59,999); return e; }

            function SmallCalendar({ currentDate, onChange, viewMode }) {
            ...
            return (
                ...
                <div className="grid grid-cols-7 gap-1">
                    ...
                    <React.Fragment key={wi}>
                        ...
                        if (viewMode === 'week') {
                            const s = weekStart(currentDate); const e = weekEnd(currentDate);
                            isInSelectedWeek = day >= s && day <= e;
                        }
                        if (viewMode === 'month') {
                            isInSelectedMonth = day.getMonth() === currentDate.getMonth() && day.getFullYear() === currentDate.getFullYear();
                        }
                        const base = 'p-2 text-[13px] rounded cursor-pointer';
                        const cls = ` ${inMonth ? '' : 'text-slate-400'} ${isSelectedDay ? 'bg-sky-600 text-white' : ''} ${isInSelectedWeek && !isSelectedDay ? 'bg-sky-200' : ''} ${isInSelectedMonth && !isInSelectedWeek && !isSelectedDay ? 'bg-slate-100' : ''}`;
                        return (
                            <div key={day.toISOString()} onClick={() => onChange(new Date(day))} className={base + cls}>
                            {day.getDate()}
                            </div>
                        );
                        })}
                ...
                <div className="mt-3 text-[12px] text-slate-500">Режим: <strong>{viewMode}</strong>. Клик — выбрать дату.</div>
                </div>
            );
            }
        ...
        function ShiftPlannerViewWithCalendar({ initialOperators, user }) {
            ...
            const uniqueSupervisors = useMemo(() => {
                ...
                (initialOperators || []).forEach(op => {
                    if (op.supervisor_id && op.supervisor_name) {
                        ...
                        supervisorsOperators.get(op.supervisor_id).push(op);
                        
                        supervisorsMap.set(op.supervisor_id, {
                            id: op.supervisor_id,
                            name: op.supervisor_name
                        });
                    }
                });
                ...
                const filtered = Array.from(supervisorsMap.values()).filter(sv => {
                    const operators = supervisorsOperators.get(sv.id) || [];
                    return operators.some(op => op.status !== 'fired');
                });
            ...
            const weekRange = useMemo(() => {
                ...
                const res = [];
                for (let i = 0; i < 7; i++) res.push(todayDateStr(new Date(monday.getFullYear(), monday.getMonth(), monday.getDate() + i)));
                return res;
            }, [currentDate]);
            const monthRange = useMemo(() => {
                ...
                const res = [];
                for (let day = 1; day <= last.getDate(); day++) res.push(todayDateStr(new Date(year, month, day)));
                return res;
            }, [currentDate]);
            ...
            const toggleDayOff = async (opId, date) => {
                try {
                    ...
                    if (!response.ok) {
                        throw new Error('Failed to toggle day off');
                    }
                    ...
                    setOperators(prev => {
                        ...
                        if (result.is_day_off) {
                            // Добавляем выходной
                            if (!op.daysOff.includes(date)) {
                                op.daysOff.push(date);
                            }
                            // Удаляем все смены в этот день
                            if (op.shifts[date]) {
                                delete op.shifts[date];
                            }
                        } else {
                            // Убираем выходной
                            op.daysOff = op.daysOff.filter(d => d !== date);
                        }
            ...
            const handleDayClick = (e, opId, date) => {
                if (e.ctrlKey || e.metaKey) {
                    // Ctrl+клик - добавить/удалить день из выбранных
                    e.stopPropagation();
                    setSelectedDays(prev => {
                        if (prev.opId !== opId) {
                            // Если выбран другой оператор, начинаем новый выбор
                            return { opId, dates: [date] };
                        }
                    ...
                    // Если есть выбранные дни для этого оператора, открыть для всех
                    if (selectedDays.opId === opId && selectedDays.dates.length > 0) {
                        openEditModalForMultipleDays(opId, selectedDays.dates);
                    } else {
            ...
            const openEditModalForMultipleDays = (opId, dates) => {
                setModalState({ 
                    open: true, 
                    opId, 
                    date: dates[0], // используем первую дату как основную
                    start: '09:00', 
                    end: '17:00', 
                    editIndex: null, 
                    breaks: [], 
            ...
            const saveSegmentToMultipleDays = async ({ opId, dates, start, end, breaks = null }) => {
                try {
                  ...
                  // Обновляем локальное состояние (ваша текущая логика)
                  setOperators(prev => {
            ...
            const removeSegment = async ({ opId, date, index }) => {
                const op = operators.find(x => x.id === opId);
                if (!op) return;
                const shift = op.shifts[date]?.[index];
                if (!shift) return;
            ...
            const getShiftPartsForDate = (op, dateStr) => {
                const parts = [];
                const arr = op.shifts?.[dateStr] ?? [];
                arr.forEach((sh, idx) => {
                ...
                const prevStr = todayDateStr(prev);
                const prevArr = op.shifts?.[prevStr] ?? [];
                prevArr.forEach((sh, idx) => {
                const ps = timeToMinutes(sh.start);
                let pe = timeToMinutes(sh.end);
                if (pe <= ps) {
                    pe += 1440;
                    const partStart = 0;
            ...
            const totalsPerHourForDate = (dateStr) => {
                ...
                parts.forEach(p => {
                    ...
                    const overlap = Math.max(0, Math.min(p.end, hEnd) - Math.max(p.start, hStart));
                    totals[hour] += overlap / 60;
                    }
                });
                });
                return totals;
            ...
            const getBreakPartsForPart = (op, p, dateStr) => {
                ...
                let bEndInDay = b.end;
                if (p.sourceDate !== dateStr) {
                    bStartInDay = b.start - 1440;
                    bEndInDay = b.end - 1440;
                }
            ...
            return (
                ...
                <SimpleModal open={modalState.open} onClose={() => { setModalState((m) => ({ ...m, open: false })); clearSelectedDays(); }}>
                    ...
                    <div className="mb-6 pb-4 border-b border-slate-200">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h3 className="text-xl font-bold text-slate-900">
                                        {(() => {
                                            const op = operators.find(o => o.id === modalState.opId);
                                            return op?.name || `Оператор ${modalState.opId}`;
                                        })()}
                                    </h3>
                        ...
                        <div className="flex items-center gap-2 px-3 py-2 bg-slate-50 rounded-lg">
                            ...
                            <span className="text-sm text-slate-600 font-medium">
                                {modalState.multipleDates && modalState.multipleDates.length > 1 
                                    ? `${modalState.multipleDates.length} ${modalState.multipleDates.length === 1 ? 'день' : modalState.multipleDates.length < 5 ? 'дня' : 'дней'}`
                                    : modalState.date
                                }
                            </span>
                            {modalState.multipleDates && modalState.multipleDates.length > 1 && (
                                <span className="ml-auto text-xs text-slate-500">
                                    {modalState.multipleDates.sort()[0]} — {modalState.multipleDates.sort()[modalState.multipleDates.length - 1]}
                                </span>
                            )}
                        </div>
                    </div>
                    ...
                    {modalState.multipleDates && modalState.multipleDates.length > 1 && (
                        <div className="mb-6 p-4 bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-xl">
                            ...
                            <div className="text-xs text-green-700">
                                Изменения будут применены ко всем выбранным дням
                            </div>
                        </div>
                    )}
                    ...
                    <div className="mb-6">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                <label className="block text-xs font-medium text-slate-600 mb-2">Конец смены</label>
                                <input 
                                    type="time" 
                        ...
                        <div className="mt-4 p-3 bg-blue-50 border border-blue-100 rounded-lg">
                            <div className="flex items-center justify-between">
                                ...
                                <span className="text-lg font-bold text-blue-600">
                                    {(() => { const s = timeToMinutes(modalState.start); let e = timeToMinutes(modalState.end); if (e <= s) e += 1440; const dur = e - s; return dur > 0 ? `${(dur / 60).toFixed(2)} ч` : '—'; })()}
                                </span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="border-2 border-slate-200 rounded-lg p-4 bg-white">
                        ...
                        {modalState.editIndex !== null && (() => {
                            ...
                            return (
                                ...
                                onDrag={(e) => {
                                    if (!dragState.current || !e.clientX) return;
                    ...
                    <div className="mt-6 pt-4 border-t border-slate-200">
                        ...
                        <div className="flex gap-3 justify-end">
                            ...
                            {!modalState.isDayOff && modalState.multipleDates && modalState.multipleDates.length > 1 && (
                                <button 
                                    ...
                                    onClick={() => saveSegmentToMultipleDays({ opId: modalState.opId, dates: modalState.multipleDates, start: modalState.start, end: modalState.end, breaks: modalState.breaks && modalState.breaks.length ? modalState.breaks : null })}
                                >
                ...
                {selectedDays.dates.length > 0 && (
                    <div 
                        className="fixed bottom-6 right-6 p-4 bg-gradient-to-r from-green-600 to-green-500 text-white rounded-xl shadow-2xl z-50 flex items-center gap-4 backdrop-blur-sm"
                        style={{
                        ...
                        <div className="flex items-center gap-3 flex-1">
                            ...
                            <div>
                                <div className="text-lg font-bold">
                                    {selectedDays.dates.length} {selectedDays.dates.length === 1 ? 'день' : selectedDays.dates.length < 5 ? 'дня' : 'дней'}
                                </div>
        ...
        function startConfetti() {
            ...
            const colors = ['#0ea5e9','#7c3aed','#06b6d4','#f97316','#ef4444','#10b981'];
            ...
            // increase particle count for a denser effect
            for (let i = 0; i < 200; i++) {
                particles.push({
            ...
            function render() {
                ...
                const now = Date.now();
                if (now >= end) ending = true;

                particles.forEach(p => {
                    if (!ending) p.vy += gravity;
                    p.x += p.vx;
                    p.y += p.vy;
                    p.rot += p.vr;
                    if (ending) {
                ...
                // continue animation while any particle still visible
        ...
        if (priority.toLowerCase() === "high") return "bg-red-100 text-red-800";
        ...
        const perCriterion = result?.per_criterion || [];
        const summary = result?.summary || {};

        const avgScore = meta?.avg_score ?? summary?.avg_score ?? null;
        const evaluatedCalls = meta?.evaluated_calls ?? null;
        ...
        const downloadJSON = () => {
            ...
            const a = document.createElement("a");
            a.href = url;
            a.download = `ai_monthly_feedback_${month || "unknown"}.json`;
            a.click();
            URL.revokeObjectURL(url);
        };

        const toggleTech = (idx) => {
            setExpandedTech((s) => ({ ...s, [idx]: !s[idx] }));
        };

        const scoreColor = useMemo(() => {
            if (avgScore == null) return "bg-gray-200";
            if (avgScore >= 90) return "bg-green-500";
            if (avgScore >= 75) return "bg-yellow-400";
            return "bg-red-400";
        }, [avgScore]);
        ...
        return (
            ...
            <div className="fixed inset-0 z-50 flex items-start md:items-center justify-center p-4 overflow-y-auto">
                ...
                role="dialog"
                aria-modal="true"
                className="bg-white rounded-2xl shadow-2xl w-full max-w-6xl max-h-[calc(92vh-72px)] overflow-hidden border border-gray-200"
                onClick={(e) => e.stopPropagation()}
                >
                {/* Header */}
                <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
                    ...
                    <div className="text-lg font-semibold text-gray-900 truncate">Обратная связь от ИИ — сводка за месяц</div>
                    </div>
                    <div className="flex items-center gap-3">
                    <button
                        onClick={copyJSON}
                ...
                {/* Body */}
                <div className="p-5 overflow-y-auto max-h-[calc(92vh-72px)] space-y-4">
                    {loading ? (
                    <div className="flex items-center gap-3">
                        ...
                        <div className="text-sm text-gray-700">Генерируем обратную связь — это может занять несколько секунд...</div>
                    </div>
                    ) : error ? (
                    <div className="text-sm text-red-600 whitespace-pre-wrap">{typeof error === "string" ? error : JSON.stringify(error)}</div>
                    ) : result ? (
                    <>
                        {/* Top summary */}
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                        <div className="col-span-1 p-4 border rounded-2xl">
                            <div className="text-xs text-gray-500">Месяц</div>
                            <div className="text-sm font-medium text-gray-900">{formatMonth(month) || "—"}</div>

                            <div className="mt-4">
                            <div className="text-xs text-gray-500">Оценено звонков</div>
                            <div className="text-lg font-semibold">{evaluatedCalls ?? "—"}</div>
                            </div>
                        </div>

                        <div className="col-span-1 p-4 border rounded-2xl flex flex-col justify-center items-center">
                            <div className="text-xs text-gray-500">Средний балл</div>
                            <div className="mt-2 flex items-center gap-3">
                            <div className={`rounded-full w-20 h-20 flex items-center justify-center text-white font-semibold ${scoreColor}`}>
                                {avgScore != null ? `${avgScore}` : "—"}
                            </div>
                            <div className="text-sm text-gray-700 max-w-xs">
                                <div className="text-xs text-gray-500">Уровень: <span className="font-medium capitalize">{summary?.overall_level ?? "—"}</span></div>
                                <div className="text-xs text-gray-500 mt-1">Направление: <span className="font-medium">{direction || "—"}</span></div>
                            </div>
                            </div>
                        </div>

                        <div className="col-span-2 p-4 border rounded-2xl">
                            <div className="text-xs text-gray-500">Топ приоритеты</div>
                            <ul className="text-sm list-disc list-inside mt-2 space-y-1">
                            {(summary?.top_priorities || []).map((p, i) => <li key={i}>{p}</li>)}
                            {(!summary?.top_priorities || !summary.top_priorities.length) && <li>—</li>}
                            </ul>

                            <div className="text-xs text-gray-500 mt-3">План тренировок</div>
                            <ol className="list-decimal list-inside text-sm mt-2 space-y-1">
                            {(summary?.training_plan || []).map((t, i) => (
                                <li key={i} className="text-sm text-gray-700">{t}</li>
                            ))}
                            {(!summary?.training_plan || !summary.training_plan.length) && <li className="text-sm text-gray-700">Нет предложений</li>}
                            </ol>
                        </div>
                        </div>
                        ...
                        {/* Per-criterion list */}
                        <div>
                        <h3 className="text-sm font-medium text-gray-900 mb-3">Детализация по критериям</h3>
                        <div className="space-y-4">
                            {perCriterion.length === 0 && <div className="text-sm text-gray-600">Нет данных по критериям.</div>}
                            ...
                            <div key={idx} className="border rounded-2xl p-4 bg-white shadow-sm">
                                ...
                                <div className="min-w-0">
                                    <div
                                    className="text-sm font-semibold text-gray-900 whitespace-normal break-words leading-tight"
                                    style={{ maxWidth: "min(70ch, 100%)" }}
                                    title={c.criterion || `Критерий ${idx+1}`}
                                    >
                                    {c.criterion || `Критерий ${idx+1}`}
                                    </div>
                                    {/* если нужно — можно показать краткое описание под названием */}
                                </div>

                                <div className="flex-shrink-0">
                                    <div className={`text-xs px-2 py-0.5 rounded-full ${priorityColor(c.priority)}`}>
                                    {c.priority ?? "low"}
                                    </div>
                                </div>
                                </div>
                                    {c.criterion_description && (
                                    <div className="text-xs text-gray-500 mt-1">{c.criterion_description}</div>
                                    )}
                                </div>
                                <div className="text-sm text-gray-500">
                                    {/* small right column kept for short info */}
                                    <div>Состояние: <span className="text-gray-700">{c.status ?? "—"}</span></div>
                                </div>
                                </div>

                                {/* Ошибки: теперь под заголовком, занимает всю ширину карточки и поддерживает перенос/скролл */}
                                <div className="mt-3">
                                <div className="text-xs text-gray-500">Ошибки</div>
                                <div
                                    className="mt-1 text-sm text-gray-700 whitespace-pre-wrap break-words bg-gray-50 p-3 rounded-lg border border-gray-100 max-h-36 overflow-auto"
                                    title={typeof c.issues === "string" ? c.issues : ""}
                                >
                                    {c.issues ? String(c.issues) : "—"}
                                </div>
                                </div>

                                <div className="mt-3 grid grid-cols-1 md:grid-cols-3 gap-3">
                                <div className="col-span-1">
                                    <div className="text-xs text-gray-500">Сильные стороны</div>
                                    <div className="text-sm text-gray-700 mt-1">{c.strengths || "—"}</div>
                                </div>
                                <div className="col-span-1">
                                    <div className="text-xs text-gray-500">Рекомендация</div>
                                    <div className="text-sm text-gray-700 mt-1">{c.recommendation || "—"}</div>
                                </div>
                                <div className="col-span-1">
                                    <div className="text-xs text-gray-500 flex items-center justify-between">
                                    <span>Техники / фразы</span>
                                    {Array.isArray(c.techniques) && c.techniques.length > 0 && (
                                        <button
                                        onClick={() => toggleTech(idx)}
                                        className="text-xs text-blue-600 hover:underline"
                                        >
                                        {expandedTech[idx] ? "Скрыть" : `Показать (${c.techniques.length})`}
                                        </button>
                                    )}
                                    </div>
                                    <div className="mt-1">
                                    {Array.isArray(c.techniques) && c.techniques.length > 0 ? (
                                        ...
                                        <ul className="list-disc list-inside text-sm text-gray-700">
                                            {c.techniques.map((t, i) => <li key={i}>{t}</li>)}
                                        </ul>
                                        ) : (
                                        <div className="text-sm text-gray-600">Свернуто</div>
                                        )
                                    ) : (
                        ...
                        <div className="text-xs text-gray-500">Исходный JSON</div>
                        <div className="flex items-center gap-3">
                            <label className="text-sm text-gray-600 flex items-center gap-2">
                            <input type="checkbox" checked={showRaw} onChange={() => setShowRaw(s => !s)} />
                            Показать
                            </label>
                        </div>
                        </div>
                        ...
                        {showRaw && (
                        <pre className="text-xs text-gray-800 whitespace-pre-wrap bg-gray-50 border border-gray-200 rounded-xl p-4 mt-2 max-h-72 overflow-auto">
        ...
        function MonthlyFeedbackPage({ user }) {
        ...
        // Генерация последних 12 месяцев
        ...
        const fetchFeedback = async (month) => {
            ...
            const resp = await axios.post(
                `${API_BASE_URL}/api/ai/monthly_feedback`,
                {
                operator_id: user.id,
                month,
                },
            ...
            if (data?.status === 'success') {
                ...
                typeof data?.error === 'string'
                    ? data.error
                    : JSON.stringify(data?.error || data || 'Ошибка', null, 2)
                );
            }
            } catch (err) {
        ...
        const downloadJSON = () => {
            ...
            const a = document.createElement('a');
            a.href = url;
            a.download = `my_ai_feedback_${selectedMonth || 'unknown'}.json`;
            a.click();
            URL.revokeObjectURL(url);
        };
        ...
        return (
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                <div className="flex items-center justify-between mb-6">
                    ...
                    {selectedMonth && (
                    <div className="text-sm text-gray-500">
                        Выбрано: <span className="font-semibold text-gray-900">{formatMonth(selectedMonth)}</span>
                    </div>
                    )}
                </div>
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
                    ...
                    return (
                        ...
                        disabled={loading}
                        className={`
                            px-4 py-4 rounded-xl font-medium text-sm transition-all duration-200
                            ${
                            isSelected
                                ? 'bg-gradient-to-br from-blue-600 to-blue-700 text-white shadow-lg shadow-blue-200 scale-105 ring-2 ring-blue-300'
                ...
                <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-12">
                    ...
                    <div className="text-xl text-gray-900 font-semibold">
                        Генерируем вашу обратную связь...
                    </div>
                    <div className="text-sm text-gray-500">
                        Анализируем ваши звонки за {formatMonth(selectedMonth)}
                    </div>
                ...
                <div className="bg-red-50 border-2 border-red-200 rounded-2xl p-6 shadow-lg">
                    ...
                    <div className="flex-1">
                        ...
                        <pre className="text-sm text-red-700 whitespace-pre-wrap bg-red-100 rounded-lg p-4 border border-red-200">
                        {error}
                        </pre>
                    </div>
                    </div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-6 hover:shadow-xl transition-shadow">
                        ...
                        <div
                            className={`w-28 h-28 rounded-full flex items-center justify-center text-white font-bold text-4xl shadow-xl ${scoreColor}`}
                        >
                            {avgScore != null ? avgScore : '—'}
                        </div>
                    ...
                    <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-6 hover:shadow-xl transition-shadow">
                        ...
                        {(summary?.training_plan || []).map((t, i) => (
                            ...
                            className="text-sm text-gray-700 flex items-start gap-2"
                            >
                            <span className="text-green-600 font-bold min-w-[20px]">
                                {i + 1}.
                            </span>
                            <span>{t}</span>
                            </li>
                        ))}
                        {(!summary?.training_plan || !summary.training_plan.length) && (
                            <li className="text-sm text-gray-500 italic">
                    ...
                    <div className="space-y-4">
                        ...
                        <div
                            key={idx}
                            className="border-2 border-gray-100 rounded-2xl p-5 hover:border-blue-200 hover:shadow-md transition-all bg-gradient-to-br from-white to-gray-50"
                        >
                            {/* Header */}
                            <div className="flex items-start justify-between gap-4 mb-4">
                            <div className="flex-1 min-w-0">
                            ...
                            <div>
                                ...
                                {Array.isArray(c.techniques) &&
                                    ...
                                    >
                                        {expandedTech[idx]
                                        ? 'Скрыть'
                                        : `Показать (${c.techniques.length})`}
                                    </button>
                                    )}
                                </div>
                                <div className="bg-blue-50 border border-blue-100 rounded-lg p-3 min-h-[80px]">
                                {Array.isArray(c.techniques) &&
                                c.techniques.length > 0 ? (
                                    expandedTech[idx] ? (
                                    <ul className="space-y-1">
                                    ...
                                    <p className="text-sm text-gray-600 italic flex items-center justify-center h-full">
                                        Нажмите "Показать" чтобы увидеть
                                    </p>
                                    )
                                ) : (
                                    <p className="text-sm text-gray-600 italic flex items-center justify-center h-full">
                    ...
                    {showRaw && (
                        <pre className="text-xs text-gray-800 whitespace-pre-wrap bg-gray-900 text-green-400 rounded-xl p-4 max-h-96 overflow-auto font-mono shadow-inner">
                        {JSON.stringify(result, null, 2)}
                        </pre>
                    )}
                    </div>
                ...
                {/* Empty State */}
                {!selectedMonth && !loading && !error && !result && (
                <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-16">
                    ...
                    <p className="text-gray-600 leading-relaxed">
                        Выберите месяц из календаря выше, чтобы увидеть детальную обратную связь от ИИ по вашим звонкам и получить персональные рекомендации для улучшения качества работы
                    </p>
                    </div>
                </div>
                )}
        ...
        // Contests management component (admin only)
        const ContestsApp = ({ user: currentUser, operators = [], directions = [] }) => {
            ...
            // Baiga period management
            const monthNamesShort = ['янв','фев','мар','апр','май','июн','июл','авг','сен','окт','ноя','дек'];
            
            const getRecentPeriods = (count = 12, days = 3) => {
                const periods = [];
                const base = new Date('2026-02-09');
                const now = new Date();
                for (let i = 0; i < count; i++) {
                    const start = new Date(base);
                    start.setUTCDate(base.getUTCDate() + (i * days));
                    ...
                    const label = `${startDay} ${monthNamesShort[startMonth]} - ${endDay} ${monthNamesShort[endMonth]} (${days} дня)`;
                    periods.push({
                        ...
                        days: Array.from({length: days}, (_, idx) => {
                            ...
                            return {
                                ...
                                label: `${d.getUTCDate()} ${monthNamesShort[d.getUTCMonth()]}`
                            };
                        })
                    });
                }

                const valid = periods.filter(p => new Date(p.start) <= now);
                if (valid.length === 0) {
                    return [periods[0]];
                }
            ...
            const [baigaPeriodOptions, setBaigaPeriodOptions] = useState([]);
            const [selectedBaigaPeriod, setSelectedBaigaPeriod] = useState(0);

            useEffect(() => {
                setBaigaPeriodOptions(getRecentPeriods(12, 3));
                setSelectedBaigaPeriod(0);
            }, []);

            // Quality weeks management
            const [qualityWeekOptions, setQualityWeekOptions] = useState([]);
            const [selectedQualityWeek, setSelectedQualityWeek] = useState(0);
            ...
            const getRecentWeeks = (count = 12) => {
                ...
                end.setUTCDate(today.getUTCDate() + offsetToSunday);
                let cursor = end;
                for (let i = 0; i < count; i++) {
                    const start = new Date(cursor);
                    start.setUTCDate(cursor.getUTCDate() - 6);
                    const label = `${start.getUTCDate()} ${monthNamesShort[start.getUTCMonth()]} - ${cursor.getUTCDate()} ${monthNamesShort[cursor.getUTCMonth()]}`;
                    weeks.push({ start: start.toISOString().slice(0,10), end: cursor.toISOString().slice(0,10), label });
                    const prev = new Date(start);
                    prev.setUTCDate(start.getUTCDate() - 1);
                    cursor = prev;
                }
                return weeks;
            };
            ...
            useEffect(()=>{
                ...
                setQualityWeekOptions(weeks);
                setSelectedQualityWeek(0);
            }, []);

            // initialize contestData from operators when provided
            useEffect(() => {
                if (!Array.isArray(operators) || operators.length === 0) return;

                const byDirectionName = (op) => op.direction || op.direction_name || (op.direction_id ? String(op.direction_id) : 'Без направления');

                const determineBaigaGroup = (op) => {
                ...
                const isNewbie = (op) => {
                    const hd = op.hire_date || op.hireDate || op.hire || '';
                    if (!hd) return false;
                    let d = null;
                    if (/^\d{4}-\d{2}-\d{2}$/.test(hd)) d = new Date(hd);
                    else if (/^\d{2}-\d{2}-\d{4}$/.test(hd)) {
                        const [dd, mm, yyyy] = hd.split('-'); d = new Date(`${yyyy}-${mm}-${dd}`);
                    } else {
                        const parsed = Date.parse(hd);
                        if (!Number.isNaN(parsed)) d = new Date(parsed);
                    }
                    if (!d) return false;
                    const msInMonth = 1000 * 60 * 60 * 24 * 30.44;
                    const months = (Date.now() - d.getTime()) / msInMonth;
                    return months < 2;
                };
                ...
                const newData = {
                    ...
                    quality: { groups: { new: [], experienced: [] }, period: 'неделя', lastUpdate: new Date().toLocaleString('ru-RU') },
                    chat: { results: [], period: 'день', lastUpdate: new Date().toLocaleString('ru-RU') },
                    smz: { results: [], period: 'период', lastUpdate: new Date().toLocaleString('ru-RU') }
                };

                for (const op of operators) {
                    const status = (op.status || op.state || '').toString().toLowerCase();
                    if (status === 'fired') continue;
                    const dir = byDirectionName(op);
                    const ndir = String(dir || '').toLowerCase();
                    const isOsnova = ndir.includes('основ') || ndir.includes('основа');
                    const isSmz = ndir.includes('смз') || ndir.includes('sмz') || ndir.includes('smz');
                    const isChat = ndir.includes('чат') || ndir.includes('chat');
                    const isModerator = ndir.includes('модер') || ndir.includes('moder');

                    if (isOsnova || isSmz) {
                        const bgroup = determineBaigaGroup(op);
                        newData.baiga.groups[bgroup].push({ 
                            id: op.id,
                            name: op.name || op.login || `#${op.id}`, 
                            scores: Number(op.scores || 0), 
                            group: bgroup, 
                            direction: dir,
                            day1: 0,
                            day2: 0,
                        ...
                        const qgroup = isNewbie(op) ? 'new' : 'experienced';
                        newData.quality.groups[qgroup].push({ id: op.id, name: op.name || op.login || `#${op.id}`, score: Number(op.quality_score || op.quality || 0), group: qgroup, direction: dir });
                    }

                    if (isChat) {
                        newData.chat.results.push({ name: op.name || op.login || `#${op.id}`, thanks: Number(op.thanks || 0), direction: dir });
                    }

                    if (isSmz || isModerator) {
                        newData.smz.results.push({ name: op.name || op.login || `#${op.id}`, signatures: Number(op.signatures || 0), calls: Number(op.calls || 0), total: Number(op.signatures || 0) + Number(op.calls || 0), direction: dir });
                    }
                }

                for (const g of ['0.5','0.75','1']) {
                    newData.baiga.groups[g].sort((a,b) => (b.scores||0) - (a.scores||0));
                }
                
                const allBaigaEntries = [...newData.baiga.groups['0.5'], ...newData.baiga.groups['0.75'], ...newData.baiga.groups['1']];
                newData.baiga.top10 = allBaigaEntries.sort((a,b) => (b.scores||0) - (a.scores||0)).slice(0,10);

                if (baigaPeriodOptions.length > 0) {
                    newData.baiga.periodDays = baigaPeriodOptions[0].days;
                }

                newData.quality.groups.new.sort((a,b)=> (b.score||0)-(a.score||0));
                newData.quality.groups.experienced.sort((a,b)=> (b.score||0)-(a.score||0));

                const qualityIds = [];
                for (const g of ['new','experienced']) {
                    for (const p of newData.quality.groups[g]) {
                        if (p.id) qualityIds.push(p.id);
                    }
                }
                
                if (qualityIds.length > 0 && qualityWeekOptions.length > 0) {
                    const currentWeek = qualityWeekOptions[0];
                    fetchQualityAveragesForPeriod(Array.from(new Set(qualityIds)), currentWeek.start, currentWeek.end);
                }

                newData.chat.results.sort((a,b)=> (b.thanks||0)-(a.thanks||0));
                newData.smz.results.sort((a,b)=> (b.total||0)-(a.total||0));

                setContestData(newData);
            }, [operators, baigaPeriodOptions, qualityWeekOptions]);
            ...
            const [contestData, setContestData] = useState({
                ...
                quality: { groups: { new: [], experienced: [] }, period: 'неделя', lastUpdate: null },
                chat: { results: [], period: 'день', lastUpdate: null },
                smz: { results: [], period: 'период', lastUpdate: null }
            });

            const [baigaForm, setBaigaForm] = useState({ name: '', group: '0.5', scores: '' });
            const [qualityForm, setQualityForm] = useState({ name: '', group: 'new', score: '' });
            const [chatForm, setChatForm] = useState({ name: '', thanks: '' });
            const [smzForm, setSmzForm] = useState({ name: '', signatures: '', calls: '' });
            const [qualityLoading, setQualityLoading] = useState(false);
            const [baigaLoading, setBaigaLoading] = useState(false);
            ...
            const fetchQualityAveragesForPeriod = async (opIds, start=null, end=null) => {
                ...
                try {
                    if (!start || !end) {
                        if (qualityWeekOptions.length > 0) {
                            const currentWeek = qualityWeekOptions[selectedQualityWeek || 0];
                    ...
                    let opMap = {};
                    if (!raw) return;
                    const operatorsField = raw.operators || raw;
                    if (Array.isArray(operatorsField)) {
                        for (const item of operatorsField) {
                            const key = item.operator_id ?? item.id ?? item.operatorId ?? item.operator;
                            if (key != null) opMap[String(key)] = item;
                        }
                    } else if (typeof operatorsField === 'object') {
                        opMap = operatorsField;
                    }
                    ...
                    setContestData(prev => {
                        ...
                        for (const grp of ['new','experienced']) {
                            const originalGroup = prev.quality.groups[grp] || [];
                            updated.quality.groups[grp] = originalGroup.map(p => {
                                const resetParticipant = { ...p, score: 0, eval_count: 0 };
                                
                                const info = opMap[String(p.id)];
                                if (info) {
                                    const avg = info.avg_score != null ? Number(info.avg_score) : (info.avg || 0);
                                    const count = info.count != null ? Number(info.count) : (info.eval_count || 0);
                                    return { ...resetParticipant, score: avg, eval_count: count };
                                }
                    ...
                    console.error('Error fetching quality averages', err);
                    addToast('Не удалось загрузить средние оценки качества', 'error');
                } finally {
                    setQualityLoading(false);
                }
            };

            const parseCSVAndCountScores = (csvText, sep = ';') => {
                ...
                const normalizeFioKey = (s) => (s || '').replace(/"/g, '').replace(/\s+/g, ' ').trim();

                for (let i = 1; i < lines.length; i++) {
                    const line = lines[i];
                    const fields = parseLine(line);
                    if (fields.length > 7) {
                        const fioRaw = fields[5];
                        const scoreRaw = fields[7];
                        const fio = normalizeFioKey(fioRaw);
                        const scoreVal = (scoreRaw || '').replace(/"/g, '').trim();
                        if (fio) {
                            const num = Number(scoreVal.replace(',', '.'));
                            if (!Number.isNaN(num) ? num === 5 : scoreVal === '5') {
                                scoreCounts[fio] = (scoreCounts[fio] || 0) + 1;
                            }
                        }
                    }
                }
            ...
            const handleBaigaDayUpload = async (dayIndex, file) => {
                if (!file) return;
                try {
                    const periodDay = contestData.baiga.periodDays && contestData.baiga.periodDays[dayIndex] && contestData.baiga.periodDays[dayIndex].date;
                    if (!periodDay) { addToast('Неизвестная дата периода', 'error'); return; }
                    ...
                    const scores = (resp && resp.data && resp.data.scores) ? resp.data.scores : [];
                    ...
                    setContestData(prev => {
                        ...
                        for (const r of scores) {
                            if (r && r.operator_id != null) mapById[String(r.operator_id)] = Number(r.points || 0);
                        }

                        for (const group of ['0.5', '0.75', '1']) {
                            updated.baiga.groups[group] = (updated.baiga.groups[group] || []).map(participant => {
                                const currentDayScore = participant[dayKey] || 0;
                                let matchedScore = 0;
                                if (participant.id && mapById[String(participant.id)] != null) {
                                    matchedScore = mapById[String(participant.id)];
                                } else {
                                    const normalizedParticipant = (participant.name || '').replace(/"/g, '').trim().toLowerCase();
                                    for (const r of scores) {
                                        if (r && r.name && r.name.trim().toLowerCase() === normalizedParticipant) { matchedScore = Number(r.points || 0); break; }
                                    }
                                }

                                const newTotalScores = (participant.scores || 0) - currentDayScore + matchedScore;
                                return { ...participant, [dayKey]: matchedScore, scores: newTotalScores };
                            }).sort((a, b) => (b.scores || 0) - (a.scores || 0));
                        }

                        const allEntries = [...updated.baiga.groups['0.5'], ...updated.baiga.groups['0.75'], ...updated.baiga.groups['1']];
                        updated.baiga.top10 = allEntries.sort((a, b) => (b.scores || 0) - (a.scores || 0)).slice(0, 10);
                        updated.baiga.lastUpdate = new Date().toLocaleString('ru-RU');
                        return updated;
                    });

                    addToast(`Данные за ${contestData.baiga.periodDays[dayIndex]?.label} загружены успешно.`);
                } catch (err) {
            ...
            const fetchBaigaPeriod = async (period) => {
                if (!period || !Array.isArray(period.days) || period.days.length === 0) return;
                ...
                try {
                    ...
                    setContestData(prev => {
                        ...
                        for (let dayIndex = 0; dayIndex < period.days.length; dayIndex++) {
                            const dayObj = days[dayIndex] || { date: period.days[dayIndex].date, scores: [] };
                            const scores = dayObj.scores || [];
                            const dayKey = `day${dayIndex + 1}`;
                            const mapById = {};
                            for (const r of scores) if (r && r.operator_id != null) mapById[String(r.operator_id)] = Number(r.points || 0);
                    ...
                    console.error('Error fetching baiga period data', err);
                    addToast('Ошибка при загрузке данных периода', 'error');
                } finally {
                    setBaigaLoading(false);
                }
            };

            useEffect(() => {
                if (!Array.isArray(baigaPeriodOptions) || baigaPeriodOptions.length === 0) return;
                const idx = Number(selectedBaigaPeriod) || 0;
                const period = baigaPeriodOptions[idx];
                if (!period) return;
                setContestData(prev => ({ ...prev, baiga: { ...prev.baiga, periodDays: period.days, periodStart: period.start } }));
                fetchBaigaPeriod(period);
            }, [selectedBaigaPeriod, baigaPeriodOptions]);

            const addBaigaResult = () => {
                if (!baigaForm.name || !baigaForm.scores) { addToast('Заполните все поля', 'error'); return; }
                const newEntry = { name: baigaForm.name, scores: parseInt(baigaForm.scores), group: baigaForm.group, day1: 0, day2: 0, day3: 0 };
                setContestData(prev => {
                    const updated = { ...prev };
                    updated.baiga.groups[baigaForm.group] = [...updated.baiga.groups[baigaForm.group], newEntry].sort((a, b) => b.scores - a.scores);
                    const allEntries = [...updated.baiga.groups['0.5'], ...updated.baiga.groups['0.75'], ...updated.baiga.groups['1']];
                    updated.baiga.top10 = allEntries.sort((a, b) => b.scores - a.scores).slice(0, 10);
                    updated.baiga.lastUpdate = new Date().toLocaleString('ru-RU');
                    return updated;
                });
                setBaigaForm({ name: '', group: '0.5', scores: '' });
                addToast('Результат добавлен');
            };

            const addQualityResult = () => {
                if (!qualityForm.name || !qualityForm.score) { addToast('Заполните все поля', 'error'); return; }
                const newEntry = { name: qualityForm.name, score: parseFloat(qualityForm.score), group: qualityForm.group };
                setContestData(prev => { const updated = { ...prev }; updated.quality.groups[qualityForm.group] = [...updated.quality.groups[qualityForm.group], newEntry].sort((a,b)=>b.score-a.score); updated.quality.lastUpdate = new Date().toLocaleString('ru-RU'); return updated; });
                setQualityForm({ name: '', group: 'new', score: '' }); addToast('Результат добавлен');
            };

            const addChatResult = () => {
                if (!chatForm.name || !chatForm.thanks) { addToast('Заполните все поля', 'error'); return; }
                const newEntry = { name: chatForm.name, thanks: parseInt(chatForm.thanks) };
                setContestData(prev => { const updated = { ...prev }; updated.chat.results = [...updated.chat.results, newEntry].sort((a,b)=>b.thanks-a.thanks); updated.chat.lastUpdate = new Date().toLocaleString('ru-RU'); return updated; });
                setChatForm({ name: '', thanks: '' }); addToast('Результат добавлен');
            };

            const addSmzResult = () => {
                if (!smzForm.name || !smzForm.signatures || !smzForm.calls) { addToast('Заполните все поля', 'error'); return; }
                const newEntry = { name: smzForm.name, signatures: parseInt(smzForm.signatures), calls: parseInt(smzForm.calls), total: parseInt(smzForm.signatures) + parseInt(smzForm.calls) };
                setContestData(prev => { const updated = { ...prev }; updated.smz.results = [...updated.smz.results, newEntry].sort((a,b)=>b.total-a.total); updated.smz.lastUpdate = new Date().toLocaleString('ru-RU'); return updated; });
                setSmzForm({ name: '', signatures: '', calls: '' }); addToast('Результат добавлен');
            };

            // Функция для поиска позиции оператора в Байга
            const getBaigaUserStats = () => {
                if (!currentUser?.id && !currentUser?.name) return null;
                
                const allEntries = [...contestData.baiga.groups['0.5'], ...contestData.baiga.groups['0.75'], ...contestData.baiga.groups['1']];
                ...
                // Поиск по ID
                if (currentUser.id) {
                    userEntry = allEntries.find(p => p.id === currentUser.id);
                }
                
                // Если не нашли по ID, ищем по имени
                if (!userEntry && currentUser.name) {
                    const normalizedUserName = (currentUser.name || '').toLowerCase().trim();
                    userEntry = allEntries.find(p => (p.name || '').toLowerCase().trim() === normalizedUserName);
                }
                
                if (!userEntry) return null;
                
                // Находим позицию в общем рейтинге
                const sortedAll = [...allEntries].sort((a, b) => (b.scores || 0) - (a.scores || 0));
                overallPosition = sortedAll.findIndex(p => p.id === userEntry.id || (p.name || '').toLowerCase() === (userEntry.name || '').toLowerCase()) + 1;
                
                // Находим позицию в группе
                const groupEntries = contestData.baiga.groups[userEntry.group] || [];
                groupPosition = groupEntries.findIndex(p => p.id === userEntry.id || (p.name || '').toLowerCase() === (userEntry.name || '').toLowerCase()) + 1;
                
                return {
                    entry: userEntry,
                    overallPosition,
                    groupPosition,
                    totalInGroup: groupEntries.length,
                    totalOverall: allEntries.length
                };
            };

            // Функция для поиска позиции оператора в Качестве
            const getQualityUserStats = () => {
                if (!currentUser?.id && !currentUser?.name) return null;
                
                const allEntries = [...contestData.quality.groups.new, ...contestData.quality.groups.experienced];
                ...
                userGroup = userEntry.group; // 'new' или 'experienced'
                
                // Находим позицию в общем рейтинге
                const sortedAll = [...allEntries].sort((a, b) => (b.score || 0) - (a.score || 0));
                overallPosition = sortedAll.findIndex(p => p.id === userEntry.id || (p.name || '').toLowerCase() === (userEntry.name || '').toLowerCase()) + 1;
                
                // Находим позицию в группе
                const groupEntries = contestData.quality.groups[userGroup] || [];
                groupPosition = groupEntries.findIndex(p => p.id === userEntry.id || (p.name || '').toLowerCase() === (userEntry.name || '').toLowerCase()) + 1;
                ...
                return {
                    ...
                    totalInGroup: groupEntries.length,
                    totalOverall: allEntries.length,
                    groupName: userGroup === 'new' ? 'Новички' : 'Опытные'
                };
            };
            ...
            const ParticipantCard = ({ participant, position, scoreLabel = 'Оценки', showExtra = null }) => (
                React.createElement('div',{className:'contest-card bg-white rounded-xl p-4 shadow-md hover:shadow-lg flex items-center gap-4'},
                    React.createElement(Medal,{position}),
                    React.createElement('div',{className:'flex-1'},
                        React.createElement('h4',{className:'font-semibold text-gray-800'}, participant.name),
                        React.createElement('div',{className:'flex gap-4 mt-1 items-center'},
                            React.createElement('p',{className:'text-sm text-gray-600'}, `${scoreLabel}: `, React.createElement('span',{className:'font-bold text-blue-600'}, participant.scores || participant.score || participant.thanks || participant.total)),
                            showExtra && showExtra(participant),
                            participant.direction && React.createElement('span',{className:'text-xs bg-gray-100 text-gray-700 px-2 py-1 rounded-full'}, participant.direction)
                        )
                    )
                )
            );

            // Компонент для отображения личной статистики оператора в Байга
            const renderBaigaUserStats = () => {
                if (isAdmin) return null;
                
                const stats = getBaigaUserStats();
                if (!stats) return null;
                
                const isFirst = stats.overallPosition === 1 || stats.groupPosition === 1;
                ...
                return React.createElement('div', { className: `bg-gradient-to-br ${isFirst ? 'from-yellow-400 via-yellow-500 to-yellow-600' : 'from-blue-400 to-blue-600'} rounded-2xl p-6 shadow-2xl mb-6 text-white` },
                    ...
                    React.createElement('div', { className: 'grid grid-cols-1 md:grid-cols-3 gap-4' },
                        React.createElement('div', { className: 'bg-white bg-opacity-20 rounded-xl p-4 backdrop-blur-sm' },
                            ...
                            React.createElement('p', { className: 'text-sm opacity-90 mt-1' }, `из ${stats.totalOverall} операторов`)
                        ),
                        React.createElement('div', { className: 'bg-white bg-opacity-20 rounded-xl p-4 backdrop-blur-sm' },
                            React.createElement('div', { className: 'flex items-center gap-3 mb-2' },
                            ...
                            React.createElement('p', { className: 'text-sm opacity-90 mt-1' }, `из ${stats.totalInGroup} операторов`)
                        ),
                        React.createElement('div', { className: 'bg-white bg-opacity-20 rounded-xl p-4 backdrop-blur-sm' },
                            React.createElement('div', { className: 'flex items-center gap-3 mb-2' },
                            ...
                            React.createElement('div', { className: 'flex gap-2 mt-2' },
                                ...((contestData.baiga.periodDays || []).map((day, idx) =>
                                    React.createElement('div', { key: idx, className: 'text-center' },
                                        React.createElement('p', { className: 'text-xs opacity-75' }, day.label.split(' ')[0]),
                                        React.createElement('p', { className: 'text-sm font-bold' }, stats.entry[`day${idx + 1}`] || 0)
                                    )
                                ))
                            )
                        )
            ...
            // Компонент для отображения личной статистики оператора в Качестве
            const renderQualityUserStats = () => {
                if (isAdmin) return null;
                
                const stats = getQualityUserStats();
                if (!stats) return null;
                ...
                return React.createElement('div', { className: `bg-gradient-to-br ${isFirst ? 'from-yellow-400 via-yellow-500 to-yellow-600' : 'from-green-400 to-green-600'} rounded-2xl p-6 shadow-2xl mb-6 text-white` },
                    ...
                    React.createElement('div', { className: 'grid grid-cols-1 md:grid-cols-3 gap-4' },
                        ...
                        React.createElement('div', { className: 'bg-white bg-opacity-20 rounded-xl p-4 backdrop-blur-sm' },
                            ...
                            React.createElement('p', { className: 'text-sm opacity-90 mt-1' }, `Оценок: ${stats.entry.eval_count || 0}`)
                        )
                    )
                );
            };

            const renderGroupLeaders = () => {
                const groups = ['0.5','0.75','1'];
                const leaders = groups.map(g => (contestData.baiga.groups[g] && contestData.baiga.groups[g].length>0) ? contestData.baiga.groups[g][0] : null);
                return React.createElement('div', { className: 'grid grid-cols-1 md:grid-cols-3 gap-4 mb-6' },
                    leaders.map((p, i) => p ? React.createElement('div', { key: i, className: 'bg-white rounded-2xl p-4 shadow-lg flex items-center gap-4' },
                        React.createElement(Medal, { position: 1 }),
                        React.createElement('div', { className: 'flex-1' },
                            React.createElement('h4', { className: 'font-semibold text-gray-800' }, p.name),
                            React.createElement('div', { className: 'text-sm text-gray-600 mt-1 flex items-center gap-3' },
                                React.createElement('span', null, `Группа ${p.group}`),
                                React.createElement('span', { className: 'font-bold text-blue-600' }, p.scores || 0)
                            )
                        )
                    ) : React.createElement('div', { key: i, className: 'bg-white rounded-2xl p-4 shadow-lg text-center text-gray-500' }, 'Нет данных') )
                );
            };

            const renderBaigaTable = () => {
                const periodDays = contestData.baiga.periodDays || [];
                    ...
                    React.createElement('table', { className: 'w-full min-w-max' },
                        React.createElement('thead', null,
                            React.createElement('tr', { className: 'border-b-2 border-gray-200' },
                                React.createElement('th', { className: 'px-4 py-3 text-left text-sm font-semibold text-gray-700' }, '№'),
                                React.createElement('th', { className: 'px-4 py-3 text-left text-sm font-semibold text-gray-700' }, 'Имя'),
                                React.createElement('th', { className: 'px-4 py-3 text-left text-sm font-semibold text-gray-700' }, 'Группа'),
                                ...periodDays.map((day, idx) =>
                                    React.createElement('th', {
                                        key: idx,
                                        className: `px-4 py-3 text-center text-sm font-semibold text-gray-700 border-2 border-blue-400 bg-blue-50 cursor-pointer hover:bg-blue-100 transition-colors ${isAdmin ? 'relative group' : ''}`,
                                        onClick: isAdmin ? () => {
                                            const input = document.createElement('input');
                                            input.type = 'file';
                                            input.accept = '.csv';
                                            input.onchange = (e) => handleBaigaDayUpload(idx, e.target.files[0]);
                                            input.click();
                                        } : null
                                    },
                                        day.label,
                        ...
                        React.createElement('tbody', null,
                            contestData.baiga.top10.length > 0 ? contestData.baiga.top10.map((p, i) =>
                                React.createElement('tr', {
                                    key: i,
                                    className: `border-b border-gray-100 hover:bg-gray-50 transition-colors ${i < 3 ? 'bg-gradient-to-r from-yellow-50 to-transparent' : ''}`
                                },
                                    React.createElement('td', { className: 'px-4 py-4' },
                                        React.createElement(Medal, { position: i + 1 })
                                    ),
                                    React.createElement('td', { className: 'px-4 py-4' },
                                        React.createElement('div', null,
                                            React.createElement('div', { className: 'font-semibold text-gray-800' }, p.name),
                                            p.direction && React.createElement('div', { className: 'text-xs text-gray-500 mt-1' }, p.direction)
                                        )
                                    ),
                                    React.createElement('td', { className: 'px-4 py-4' },
                                        React.createElement('span', { className: 'text-xs bg-blue-100 text-blue-700 px-3 py-1 rounded-full font-medium' }, `Группа ${p.group}`)
                                    ),
                                    ...periodDays.map((day, idx) =>
                                        React.createElement('td', {
                                            key: idx,
                                            className: 'px-4 py-4 text-center border-l border-r border-blue-200'
                                        },
                                            React.createElement('span', { className: 'font-bold text-gray-700' }, p[`day${idx + 1}`] || 0)
                                        )
                                    ),
                                    React.createElement('td', { className: 'px-4 py-4 text-center bg-yellow-50 border-l border-r border-yellow-200' },
                                        React.createElement('span', { className: 'font-bold text-lg text-blue-600' }, p.scores || 0)
                                    )
                                )
                            ) : React.createElement('tr', null,
                                React.createElement('td', { colSpan: 4 + periodDays.length + 1, className: 'text-center text-gray-500 py-8' }, 'Нет данных')
                            )
                        )
                    )
                );
            ...
            const renderBaiga = () => (
                React.createElement('div',{className:'space-y-6'},
                    React.createElement('div',{className:'bg-gradient-to-br from-blue-500 to-blue-700 rounded-2xl p-6 text-white'}, 
                        React.createElement('div', { className: 'flex items-center justify-between mb-2' },
                            React.createElement('h3',{className:'text-2xl font-bold'}, 'Байга ОТП'),
                            React.createElement('div', { className: 'flex items-center gap-3' },
                                React.createElement('label', { className: 'text-sm opacity-90 mr-2' }, 'Период:'),
                                React.createElement('select', {
                                    value: selectedBaigaPeriod,
                                    onChange: (e) => {
                                        const idx = Number(e.target.value);
                                        setSelectedBaigaPeriod(idx);
                                        const period = baigaPeriodOptions[idx];
                                        if (period) {
                                            setContestData(prev => ({
                                                ...prev,
                                    ...
                                    className: 'px-4 py-2 rounded-xl border-2 border-blue-300 bg-white text-gray-800 font-medium shadow-sm text-sm focus:outline-none focus:ring-2 focus:ring-blue-300'
                                },
                                    (baigaPeriodOptions || []).map((p, i) => 
                                        React.createElement('option', { key: i, value: i }, p.label)
                                    )
                                )
                            )
                        ),
                        React.createElement('p',{className:'opacity-90'}, `Направление: Основа • Период: ${contestData.baiga.period}`), 
                        contestData.baiga.lastUpdate && React.createElement('p',{className:'text-sm opacity-75 mt-2'}, `Последнее обновление: ${contestData.baiga.lastUpdate}`)
                    ),
                    baigaLoading ? React.createElement('div',{className:'flex items-center justify-center py-12'}, 
                        React.createElement('div',{className:'flex flex-col items-center gap-3'}, 
                            ...
                            React.createElement('p',{className:'text-gray-600 font-medium'}, 'Загрузка данных...')
                        )
                    ) : React.createElement(React.Fragment, null,
                        renderBaigaUserStats(),
                        renderGroupLeaders(),
                        renderBaigaTable(),
                            ...
                            ['0.5','0.75','1'].map(group=> 
                                React.createElement('div',{key:group, className:'bg-white rounded-2xl p-6 shadow-lg'}, 
                                    ...
                                    React.createElement('div',{className:'space-y-3'}, 
                                        contestData.baiga.groups[group].length>0 ? 
                                            contestData.baiga.groups[group].map((p,i)=> 
                                                React.createElement(ParticipantCard,{key:i, participant:p, position:i+1, scoreLabel:'Оценки'})
                                            ) : 
                                            React.createElement('p',{className:'text-center text-gray-500 py-4'}, 'Нет данных')
                                    )
                                )
                            )
                        )
            ...
            const renderQuality = () => (
                React.createElement('div',{className:'space-y-6'},
                    React.createElement('div',{className:'bg-gradient-to-br from-green-500 to-green-700 rounded-2xl p-6 text-white'}, React.createElement('div',{className:'flex items-center justify-between'}, React.createElement('h3',{className:'text-2xl font-bold mb-2'}, 'Качество'), React.createElement('div',{className:'flex items-center gap-3'},
                            React.createElement('label',{className:'text-sm opacity-90 mr-2'}, 'Период:'),
                            React.createElement('select',{value:selectedQualityWeek, onChange:(e)=>{
                                const idx = Number(e.target.value); setSelectedQualityWeek(idx);
                                const opt = qualityWeekOptions[idx];
                                const qualityIds = [];
                                for (const g of ['new','experienced']) { for (const p of (contestData.quality.groups[g]||[])) { if (p.id) qualityIds.push(p.id); } }
                                if (opt && qualityIds.length>0) fetchQualityAveragesForPeriod(Array.from(new Set(qualityIds)), opt.start, opt.end);
                            }, className:'px-4 py-2 rounded-xl border-2 border-green-300 bg-white text-gray-800 font-medium shadow-sm text-sm focus:outline-none focus:ring-2 focus:ring-green-300'},
                                (qualityWeekOptions || []).map((w,i)=> React.createElement('option',{key:i, value:i}, w.label))
                            )
                        )
                    ), React.createElement('p',{className:'opacity-90'}, `Направление: Основа • Период: ${contestData.quality.period}`), contestData.quality.lastUpdate && React.createElement('p',{className:'text-sm opacity-75 mt-2'}, `Последнее обновление: ${contestData.quality.lastUpdate}`)),
                        qualityLoading ? React.createElement('div',{className:'flex items-center justify-center py-12'}, 
                            React.createElement('div',{className:'flex flex-col items-center gap-3'}, 
                                React.createElement('div',{className:'animate-spin rounded-full h-12 w-12 border-b-2 border-green-600'}),
                                React.createElement('p',{className:'text-gray-600 font-medium'}, 'Загрузка данных...')
                            )
                        ) : React.createElement(React.Fragment, null,
                            renderQualityUserStats(),
                            React.createElement('div',{className:'grid grid-cols-1 lg:grid-cols-2 gap-6'}, 
                                React.createElement('div',{className:'bg-white rounded-2xl p-6 shadow-lg'}, 
                                    ...
                                    React.createElement('div',{className:'space-y-3'}, 
                                        contestData.quality.groups.new.length>0 ? contestData.quality.groups.new.map((p,i)=> React.createElement(ParticipantCard,{key:i, participant:p, position:i+1, scoreLabel:'Качество', showExtra:(p)=> React.createElement('span',{className:'text-xs bg-green-100 text-green-700 px-2 py-1 rounded-full'}, `Оценок: ${p.eval_count || 0}`)})) : React.createElement('p',{className:'text-center text-gray-500 py-4'}, 'Нет данных')
                                    )
                                ), 
                                React.createElement('div',{className:'bg-white rounded-2xl p-6 shadow-lg'}, 
                                    React.createElement('h3',{className:'text-lg font-bold text-gray-800 mb-4 flex items-center gap-2'}, 
                                    ...
                                    React.createElement('div',{className:'space-y-3'}, 
                                        contestData.quality.groups.experienced.length>0 ? contestData.quality.groups.experienced.map((p,i)=> React.createElement(ParticipantCard,{key:i, participant:p, position:i+1, scoreLabel:'Качество', showExtra:(p)=> React.createElement('span',{className:'text-xs bg-green-100 text-green-700 px-2 py-1 rounded-full'}, `Оценок: ${p.eval_count || 0}`)})) : React.createElement('p',{className:'text-center text-gray-500 py-4'}, 'Нет данных')
                                    )
                                )
                            )
                        )
            ...
            const getAvailableTabs = () => {
                if (isOperatorOrSv) {
                    return [
                        { id: 'baiga', label: 'Байга ОТП', icon: 'fa-running' },
                        { id: 'quality', label: 'Качество', icon: 'fa-star' }
                    ];
                }
                return [
                    { id: 'baiga', label: 'Байга ОТП', icon: 'fa-running' },
                    { id: 'quality', label: 'Качество', icon: 'fa-star' },
                    { id: 'chat', label: 'Чат', icon: 'fa-comments' },
                    { id: 'smz', label: 'СМЗ', icon: 'fa-chart-bar' }
                ];
            };
        ...
        return (
            ...
            {confirmOpen && (
                ...
                <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm mx-4"
                    ...
                    <button onClick={() => setConfirmOpen(false)}
                        className="flex-1 px-4 py-2 rounded-lg border border-gray-200 text-sm font-medium text-gray-600 hover:bg-gray-50 transition-all">
                        Отмена
                    </button>
            ...
            <div className="flex items-start justify-between gap-4">
                ...
                <svg className={`w-4 h-4 ${isAdminSessionsLoading ? 'animate-spin' : ''}`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
            ...
            <div className="flex flex-col sm:flex-row gap-2">
                ...
                <input type="text" placeholder="Имя, логин, IP, ID сессии, устройство…"
                    value={search} onChange={(e) => setSearch(e.target.value)}
                    className="w-full pl-9 pr-9 py-2 rounded-lg border border-gray-200 bg-white text-sm text-gray-700 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-400 transition-all"/>
                {search && (
                ...
                {[
                    { val: 'all', label: 'Все' }, { val: 'admin', label: 'Админы' },
                    { val: 'sv', label: 'Супервайзеры' }, { val: 'operator', label: 'Операторы' },
                ].map(({ val, label }) => (
                    <button key={val} onClick={() => setRole(val)}
                    className={`px-3 py-1.5 rounded-md text-xs font-medium transition-all ${roleFilter === val ? 'bg-white text-gray-800 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
                    {label}
                    </button>
                ))}
                </div>
            ...
            {/* ── Bulk action bar ── */}
            <div className={`overflow-hidden transition-all duration-200 ease-in-out ${selected.size > 0 ? 'max-h-20 opacity-100' : 'max-h-0 opacity-0'}`}>
                <div className="flex items-center justify-between gap-3 px-4 py-2.5 rounded-xl bg-gray-900 text-white shadow-lg">
                <div className="flex items-center gap-3">
                    {bulkRevoking ? (
                    <>
                ...
                {!bulkRevoking && (
                    ...
                    <button onClick={clearSelection}
                        className="px-3 py-1.5 rounded-lg text-xs font-medium text-white/70 hover:text-white hover:bg-white/10 transition-all">
                        Снять выбор
                    </button>
                    <button onClick={() => setConfirmOpen(true)}
                        className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg bg-red-500 hover:bg-red-400 text-white text-xs font-medium shadow-sm transition-all">
                        <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2.5}>
                        <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12"/>
                        </svg>
                        Прервать выбранные
            ...
            {totalUsers === 0 ? (
                ...
                <p className="text-sm font-medium">Активных сессий не найдено</p>
                </div>
            ) : (
        ...
        const App = () => {
            ...
            const [selectedSvData, setSelectedSvData] = useState(null);
            const [showChangeTableForm, setShowChangeTableForm] = useState(false);
            ...
            const [converterData, setConverterData] = useState({amount: "", kpi: ""});
            ...
            // Images and state for Dev Letter carousel
            ...
            const devAutoplayPausedRef = useRef(false);
            const thumbsContainerRef = useRef(null);
            const thumbItemsRef = useRef([]);

            // Автопрокрутка карусели, активна только когда модалка открыта и не на hover


            // Smooth-scroll thumbnails into view when slide changes
            useEffect(() => {
                try {
                    const el = thumbItemsRef.current[devLetterIndex];
                    if (el && typeof el.scrollIntoView === 'function') {
                        el.scrollIntoView({ behavior: 'smooth', inline: 'center', block: 'nearest' });
                    } else if (thumbsContainerRef.current && el) {
                        // fallback: center by adjusting scrollLeft
                        const container = thumbsContainerRef.current;
                        const offset = el.offsetLeft + el.offsetWidth / 2 - container.clientWidth / 2;
            ...
            useEffect(() => {
                window.ShowDevLetterModal = () => { console.log('window.ShowDevLetterModal called'); showToast?.('С наступающим вас!'); setShowDevLetterModal(true); };
                window.HideDevLetterModal = () => { console.log('window.HideDevLetterModal called'); setShowDevLetterModal(false); };

                const onKeyDown = (e) => {
                    if (e.key === 'Escape') setShowDevLetterModal(false);
                };
            ...
            const [opsSortField, setOpsSortField] = useState('name'); // primary sort field
            ...
            if (viewScoresSortField === field) {
                setViewScoresSortDirection(prev => (prev === 'asc' ? 'desc' : 'asc'));
            } else {
                setViewScoresSortField(field);
                setViewScoresSortDirection('asc');
            }
            };

            const getViewScoresSortIcon = (field) => {
            if (viewScoresSortField !== field) return <span className="ml-2 text-gray-400 text-xs">⇅</span>;
            return viewScoresSortDirection === 'asc' ? <span className="ml-2 text-xs">▲</span> : <span className="ml-2 text-xs">▼</span>;
            };

            const parseNumberSafeViewScores = (v) => {
            const n = Number(v);
            return Number.isFinite(n) ? n : 0;
            };

            // combined sort: primary (viewScoresSortField) + secondary by name
            const compareViewScoresByField = (a, b, field) => {
            const dir = viewScoresSortDirection === 'asc' ? 1 : -1;
            const nameA = (a?.name || '').toString();
            const nameB = (b?.name || '').toString();

            if (field === 'listened') {
                const ca = parseNumberSafeViewScores(a.call_count);
                const cb = parseNumberSafeViewScores(b.call_count);
                if (ca === cb) return nameA.localeCompare(nameB, 'ru', { sensitivity: 'base' });
                return (ca - cb) * dir;
            }

            if (field === 'avg_score') {
                const sa = parseNumberSafeViewScores(a.avg_score);
            ...
            if (usersSortField !== field) return <span className="ml-2 text-gray-400 text-xs">⇅</span>;
            return usersSortDirection === 'asc' ? <span className="ml-2 text-xs">▲</span> : <span className="ml-2 text-xs">▼</span>;
            };

            const parseDateForUsersSort = (str) => {
            if (!str) return 0;
            const [d, m, y] = (str || "").split("-");
            const dt = new Date(`${y || 0}-${m || 0}-${d || 0}`);
            return dt.getTime() || 0;
            };
            ...
            // combined sort: primary field (usersSortField) + secondary by name
            const compareUsersByField = (a, b, field) => {
            ...
            if (field === 'rate') {
                ...
                const rb = safeNum(b.rate);
                if (ra === rb) return nameA.localeCompare(nameB, 'ru', { sensitivity: 'base' });
                return (ra - rb) * dir;
            }

            if (field === 'hire_date') {
                const da = parseDateForUsersSort(a.hire_date);
                const db = parseDateForUsersSort(b.hire_date);
                if (da === db) return nameA.localeCompare(nameB, 'ru', { sensitivity: 'base' });
                return (da - db) * dir;
            }

            if (field === 'call_count' || field === 'avg_score') {
                // fallback numeric fields if present
                const na = safeNum(a[field]);
                const nb = safeNum(b[field]);
                if (na === nb) return nameA.localeCompare(nameB, 'ru', { sensitivity: 'base' });
                return (na - nb) * dir;
            }

            if (field === 'status') {
                const order = { working: 1, unpaid_leave: 2, fired: 3 };
                const oa = order[a.status || 'working'] || 99;
                const ob = order[b.status || 'working'] || 99;
                if (oa === ob) return nameA.localeCompare(nameB, 'ru', { sensitivity: 'base' });
                return (oa - ob) * dir;
            }
            ...
            const handleOpsSort = (field) => {
            if (opsSortField === field) {
                setOpsSortDirection(prev => (prev === 'asc' ? 'desc' : 'asc'));
            } else {
                setOpsSortField(field);
                setOpsSortDirection('asc');
            }
            };

            const getOpsSortIcon = (field) => {
            if (opsSortField !== field) return <span className="ml-2 text-gray-400 text-xs">⇅</span>;
            return opsSortDirection === 'asc' ? <span className="ml-2 text-xs">▲</span> : <span className="ml-2 text-xs">▼</span>;
            };
            ...
            // combined sort: primary field (opsSortField) + secondary by name
            const compareOpsByField = (a, b, field) => {
            const dir = opsSortDirection === 'asc' ? 1 : -1;
            const nameA = (a?.name || '').toString();
            const nameB = (b?.name || '').toString();

            if (field === 'call_count') {
                const ca = parseNumberSafe(a.call_count);
                const cb = parseNumberSafe(b.call_count);
                if (ca === cb) return nameA.localeCompare(nameB, 'ru', { sensitivity: 'base' });
                return (ca - cb) * dir;
            }

            if (field === 'avg_score') {
                const sa = parseNumberSafe(a.avg_score);
                const sb = parseNumberSafe(b.avg_score);
                if (sa === sb) return nameA.localeCompare(nameB, 'ru', { sensitivity: 'base' });
                return (sa - sb) * dir;
            }
            ...
            if (sortField !== field) return <span className="ml-2 text-gray-400 text-xs">⇅</span>;
            return sortDirection === 'asc' ? <span className="ml-2 text-xs">▲</span> : <span className="ml-2 text-xs">▼</span>;
            };
            ...
            // compare with secondary key = name (combined sort)
            ...
            if (field === 'rate') {
                const na = Number(a?.rate || 0);
                const nb = Number(b?.rate || 0);
                if (na === nb) return safe(a?.name).toString().localeCompare(safe(b?.name).toString(), 'ru', { sensitivity: 'base' }) * 1;
                return (na - nb) * dir;
            }

            if (field === 'hire_date') {
                const da = parseDateForSort(a?.hire_date);
                const db = parseDateForSort(b?.hire_date);
                if (da === db) return safe(a?.name).toString().localeCompare(safe(b?.name).toString(), 'ru', { sensitivity: 'base' }) * 1;
                return (da - db) * dir;
            }

            if (field === 'status') {
                // optional: define logical order for statuses (working -> unpaid_leave -> fired -> others)
                const order = { working: 1, unpaid_leave: 2, fired: 3 };
                const sa = a?.status || 'working';
                const sb = b?.status || 'working';
                const oa = order[sa] || 99;
                const ob = order[sb] || 99;
                if (oa === ob) {
                return safe(a?.name).toString().localeCompare(safe(b?.name).toString(), 'ru', { sensitivity: 'base' }) * 1;
                }
                return (oa - ob) * dir;
            }

            // direction handled separately (groups); default: string compare then by name
            const sa = safe(a?.[field]).toString();
            const sb = safe(b?.[field]).toString();
            const res = sa.localeCompare(sb, 'ru', { sensitivity: 'base' });
            if (res === 0) {
                return safe(a?.name).toString().localeCompare(safe(b?.name).toString(), 'ru', { sensitivity: 'base' }) * 1;
            }
            return res * dir;
            ...
            useEffect(() => {
            if (showChangeLoginForm && inputRef.current) {
            ...
            function handleClickOutside(e) {
                if (sidebarAccountRef.current && !sidebarAccountRef.current.contains(e.target)) {
                if (showSidebarAccountDropdown) {
            ...
            function TrainingModal({ isOpen, onClose, onSave, initialData = {} }) {
                ...
                const validate = () => {
                    ...
                    if (!reason) return "Выберите причину тренинга.";
                ...
                const handleSave = async () => {
                    ...
                    setIsLoading(true);
                    try {
                    // onSave может быть sync или async — ждём результат
                    await onSave({
                        ...initialData,
                        date,
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                        ...
                        <div className="px-6 py-5">
                            ...
                            <input
                                id="count-in-hours"
                                type="checkbox"
                                checked={countInHours}
                                onChange={(e) => setCountInHours(e.target.checked)}
                                disabled={isLoading}
                                className="w-4 h-4"
                            />
            ...
            if (hours <= 3) return "bg-green-100 text-green-800";
            if (hours <= 5) return "bg-green-300 text-green-900";
            if (hours <= 7) return "bg-green-500 text-white";
            ...
            const WorkHoursCalendar = ({ hoursData, selectedMonth, user, trainings = [] }) => {
                ...
                const safeMonth = selectedMonth || fallbackMonth;
                ...
                const dailyMap = React.useMemo(() => {
                    ...
                    // 1) если пришла структура operator.daily (мапа)
                    if (hoursData.daily && typeof hoursData.daily === "object" && !Array.isArray(hoursData.daily)) {
                    return hoursData.daily;
                    }

                    // 2) старый формат: daily_hours — может быть массив чисел/объектов или объект
                    if (hoursData.daily_hours) {
                    const dh = hoursData.daily_hours;
                    // array -> map
                    if (Array.isArray(dh)) {
                        const map = {};
                        for (let i = 0; i < dh.length; i++) {
                        const val = dh[i];
                        // val может быть число (часы) или объект
                        if (val == null) continue;
                        const dayKey = String(i + 1);
                        if (typeof val === "number") {
                            map[dayKey] = { work_time: val };
                        } else if (typeof val === "object") {
                            map[dayKey] = val;
                        } else {
                            // fallback
                            map[dayKey] = { work_time: Number(val) || 0 };
                        }
                        }
                        return map;
                    }

                    // если это объект уже, вернём как есть
                    if (typeof dh === "object") return dh;
                    }

                    // 3) если в hoursData непосредственно лежит мапа по дням (редко), пробуем вернуть её
                    if (typeof hoursData === "object" && !Array.isArray(hoursData)) {
                    // попробуем найти ключи '1','2',...
                    const keys = Object.keys(hoursData);
                ...
                for (let day = 1; day <= endDate.getDate(); day++) {
                    ...
                    // получаем запись за день (object) или undefined
                    const entry = dailyMap[dayKey];

                    // entry может быть: undefined | { work_time, break_time, talk_time, calls, efficiency, ... } | { hours: number } | number
                    let hours = 0;
                    let dayData = null;
                    ...
                    // поддерживаем разные имена полей: work_time, hours, time
                    hours = Number(entry.work_time ?? entry.hours ?? entry.time ?? 0);
                    dayData = entry;
                    } else {
                    hours = 0;
                    dayData = {};
                ...
                const handleDayClick = (cell) => {
                    if (cell) {
                    // сохраняем полный объект дня (dayData + метаданные) — чтобы в модале были все поля
                    setSelectedDay(cell);
                    setShowModal(true);
                    setRequestMessage('');
                    }
                ...
                const handleSendRequest = async () => {
                    ...
                    showToast('Запрос успешно отправлен супервайзеру', 'success');
                    setShowModal(false);
                    setSelectedDay(null);
                    } catch (error) {
                ...
                return (
                    <div className="workhours-calendar bg-white p-6 rounded-lg shadow-lg">
                    <h2 className="text-2xl font-bold mb-4 text-center">Календарь работ</h2>

                    <div className="flex items-center justify-center gap-2 mb-2">
                        <div className="text-center text-sm text-gray-600">
                        {`Месяц: ${monthLabel} ${year}`}
                        </div>

                        {/* i-кнопка с тултипом (hover / focus) */}
                        <div className="relative inline-block group" aria-live="polite">
                        <button
                            className="w-5 h-5 flex items-center justify-center rounded-full border border-gray-400 text-xs font-bold text-gray-600 bg-white hover:bg-gray-100 focus:bg-gray-100 focus:outline-none"
                            aria-describedby="how-to-request-tooltip"
                            aria-label="Как отправить запрос по дню"
                            type="button"
                        >
                            i
                        </button>
                        ...
                        <div
                            id="how-to-request-tooltip"
                            role="tooltip"
                            className="pointer-events-none absolute top-full left-1/2 -translate-x-1/2 mt-2 w-72 z-50 opacity-0 group-hover:opacity-100 group-focus:opacity-100 transition-opacity duration-150"
                        >
                            <div className="bg-white border rounded shadow p-3 text-sm text-gray-700">
                            <div className="font-medium mb-1">Как отправить запрос</div>
                            <ol className="list-decimal list-inside text-sm text-gray-600 space-y-1">
                                <li>Кликните по ячейке дня (тот, где нужно отправить запрос).</li>
                                <li>В появившемся окне введите текст запроса в поле.</li>
                                <li>Нажмите кнопку <b>«Отправить запрос»</b>.</li>
                            </ol>
                            <div className="text-xs text-gray-400 mt-2">Можно также отменить — кнопка «Отмена».</div>
                            </div>
                        </div>
                        </div>
                    </div>
                    ...
                    {/* Сетка календаря */}
                    <div className="workhours-grid grid grid-cols-7 gap-1 overflow-visible">
                        ...
                        cell ? (
                            ...
                            tabIndex={0}
                            className={`group relative w-10 h-10 rounded flex items-center justify-center font-semibold transition-all duration-150 ${getCellColor(
                                cell.hours,
                            ...
                            <div className="pointer-events-none absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-3 py-1 bg-gray-900 text-white text-xs rounded-lg shadow-lg opacity-0 group-hover:opacity-100 group-focus:opacity-100 z-50 transition-opacity">
                                {`${cell.day} ${monthLabel}${cell.isToday ? " (Сегодня)" : ""}`}
                    ...
                    {/* Легенда — теперь всегда отображается */}
                    <div className="workhours-legend mt-4 text-sm bg-gray-50 p-2 sm:p-3 rounded-lg border border-gray-200 shadow-sm">
                        <p className="flex items-center gap-2 sm:gap-3 flex-wrap text-xs sm:text-sm">
                        <span className="inline-flex items-center gap-1">
                            <span className="inline-block w-4 h-4 border-2 border-blue-600 rounded" /> Сегодня
                        </span>
                        <span className="inline-flex items-center gap-1">
                            <span className="inline-block w-4 h-4 bg-green-100 rounded" /> 1–3 ч
                        </span>
                        <span className="inline-flex items-center gap-1">
                            <span className="inline-block w-4 h-4 bg-green-300 rounded" /> 4–5 ч
                        </span>
                        <span className="inline-flex items-center gap-1">
                            <span className="inline-block w-4 h-4 bg-green-500 rounded" /> 6–7 ч
                        </span>
                        <span className="inline-flex items-center gap-1">
                            <span className="inline-block w-4 h-4 bg-green-700 rounded" /> 8+ ч
                        </span>
                        {/* 🔶 Маркер тренинга */}
                        <span className="inline-flex items-center gap-1">
                    ...
                    {/* Модальное окно для запроса (показываем все данные дня) */}
                    {showModal && selectedDay && (
                        ...
                        >
                            ...
                            {/* --- Верхняя инфо-панель (дата, часы, ключевые метрики с иконками) --- */}
                            <div className="workhours-info-panel mb-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
                                ...
                                {/* -- Метрики в виде сетки с иконками -- */}
                                {(() => {
                                // утилиты форматирования значений
                                const d = selectedDay.dayData || {};
                                const normalize = (key) => {
                                    // ищем в нескольких местах: dayData, root поля selectedDay
                                    return d[key] ?? selectedDay[key] ?? null;
                                };
                                ...
                                const fmtTimeLike = (v) => {
                                    if (v === null || v === undefined || v === "") return "—";
                                    // если строка вида "HH:MM" — отдаем как есть
                                    if (typeof v === "string" && /^\d{1,2}:\d{2}$/.test(v)) return v;
                                    // если число — считаем минуты -> HH:MM
                                    const n = Number(v);
                                    if (Number.isFinite(n)) {
                                    // предполагаем минуты, если > 12, иначе часы
                                    if (n > 12) {
                            ...
                            {/* --- Тренинги (оставил как был, с теми же утилитами) --- */}
                            {(() => {
                                ...
                                const computeTrainingDuration = (t) => {
                                let hours = 0;
                                let source = "fallback";
                                ...
                                return { hours: Number(hours), source };
                                };
                                ...
                                <div className="mb-4">
                                    <div className="flex items-center justify-between mb-2">
                                    <h4 className="text-md font-semibold text-gray-800">Тренинги</h4>
                                    <div className="text-sm text-gray-600">
                                        Засчитывается в часы: <span className="font-medium">{countedTrainingSummary.toFixed(2)} ч</span>
                                    </div>
                                    </div>

                                    <div className="space-y-2 max-h-56 overflow-y-auto pr-2">
                                    {trainingsOnDay.map((t) => {
                                        const { hours, source } = computeTrainingDuration(t);
                                        return (
                                        <div key={t.id} className="p-3 bg-yellow-50 border-l-4 border-yellow-400 rounded-md">
                                            <div className="flex items-start justify-between">
                                            <div className="flex-1 min-w-0">
                                                <p className="text-sm text-gray-800">
                                                <span className="font-semibold">Время:</span> {t.start_time || "—"} — {t.end_time || "—"}
                                                </p>
                                                <p className="text-sm text-gray-800">
                                                <span className="font-semibold">Причина:</span> {t.reason || "—"}
                                                </p>
                                                {t.comment && <p className="text-sm text-gray-600 italic">"{t.comment}"</p>}
                                                <p className="text-xs text-gray-500 mt-1">Создал: {t.created_by_name} ({t.created_at})</p>
                                            </div>
                            ...
                            {/* Штрафы (если есть) */}
                            {(() => {
                                const finesOnDay = Array.isArray(selectedDay.dayData?.fines) ? selectedDay.dayData.fines : [];
                                if (finesOnDay.length === 0) return null;
                                const total = finesOnDay.reduce((s, f) => s + (Number(f.amount || f.fine_amount || 0) || 0), 0);
                                return (
                                <div className="mb-4">
                                    <div className="flex items-center justify-between mb-2">
                                    <h4 className="text-md font-semibold text-gray-800">Штрафы</h4>
                                    <div className="text-sm text-gray-600">Сумма: <span className="font-medium">{total}</span></div>
                                    </div>

                                    <div className="space-y-2">
                                    {finesOnDay.map((f, idx) => {
                                        const amt = Number(f.amount || f.fine_amount || 0) || 0;
                                        const minutes = (String(f.reason) === 'Опоздание') ? Math.round(amt / 50) : null;
                                        return (
                                        <div key={idx} className="p-3 bg-red-50 border-l-4 border-red-400 rounded-md">
                                            <div className="flex items-start justify-between">
                                            <div className="flex-1 min-w-0">
                                                <p className="text-sm text-gray-800"><span className="font-semibold">Причина:</span> {f.reason || f.fine_reason || '—'}</p>
                            ...
                            <div className="flex-shrink-0 p-4 sm:p-6 pt-4 border-t border-gray-100 bg-gray-50 rounded-b-2xl">
                                <div className="flex flex-col-reverse sm:flex-row justify-end gap-2">
                                    <button
                                        ...
                                        className="w-full sm:w-auto px-4 py-2.5 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition font-medium text-sm"
                                        disabled={isSending}
                                    >
                                        Отмена
                                    </button>
                                    <button
                                        ...
                                        className={`w-full sm:w-auto px-5 py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition font-medium text-sm flex items-center justify-center gap-2 ${isSending ? "opacity-50 cursor-not-allowed" : ""}`}
                                        disabled={isSending}
                                    >
                                        {isSending ? (
                    ...
                    return runSingleFlight(requestKey, async () => {
                        ...
                        try {
                            ...
                            if (data.status === 'success' && isMounted.current) {
                                ...
                                showToast(data.error || 'Failed to fetch directions', 'error');
                            }
                        } catch (err) {
                            console.error('Fetch directions error:', err);
                            showToast(err.response?.data?.error || 'Failed to fetch directions', 'error');
                        } finally {
            ...
            const fetchUsers = async () => {
                const requestKey = `fetchUsers:${user?.id || 'anonymous'}`;
                return runSingleFlight(requestKey, async () => {
                    ...
                    try {
                        ...
                        if (isMounted.current) setIsAdminDataLoading(false);
                    }
                });
            };
            ...
            const handleRevokeAdminSession = async (session) => {
                ...
                try {
                    ...
                    const data = response.data || {};
                    if (data.status === 'success') {
                        showToast('Сессия прервана', 'success');
            ...
            const saveUserChanges = async (editedUser) => {
                ...
                try {
                    if (!editedUser.id) {
                        ...
                        const response = await axios.post(
                            `${API_BASE_URL}/api/admin/add_user`, // <-- проверь реальный endpoint на сервере (возможно другое имя)
                            payload,
                            {
                            headers: {
                                'Content-Type': 'application/json',
                    ...
                    if (editedUser.new_password && editedUser.new_password.trim() !== '') {
                        await axios.post(`${API_BASE_URL}/api/user/change_password`, {
                            user_id: editedUser.id,
            ...
            const runSingleFlight = useCallback((key, task) => {
                ...
                const promise = Promise.resolve()
                    .then(task)
                    .finally(() => {
                        if (inFlightRequests.current[key] === promise) {
            ...
            const summarizeScores = (scores = []) => {
                const summary = { Correct: 0, Incorrect: 0, 'N/A': 0, other: 0 };
                scores.forEach(s => {
                    if (s === 'Correct') summary.Correct++;
                    else if (s === 'Incorrect') summary.Incorrect++;
                    else if (s === 'N/A') summary['N/A']++;
                    else summary.other++;
                });
                const parts = [];
                if (summary.Correct) parts.push(`✔ ${summary.Correct}`);
                if (summary.Incorrect) parts.push(`✖ ${summary.Incorrect}`);
                if (summary['N/A']) parts.push(`N/A ${summary['N/A']}`);
            ...
            const handleSubmitDispute = async () => {
                ...
                try {
                    const response = await axios.post(
                        `${API_BASE_URL}/api/call_evaluation/dispute`,
                        {
                            operator_id: user.id,
            ...
            const calculateSalary = () => {
                ...
                // Бонусы
                let bonuses = 0;
                if (salaryData.bonusTraining) bonuses += 6000;
                if (salaryData.bonusRefer) {
                    const quantity = parseInt(salaryData.bonusReferQuantity) || 0;
                    bonuses += 5000 * quantity;
                }
                if (salaryData.bonusFilming) {
            ...
            // Add this function to generate points table
            const generatePointsTable = () => {
                const { experience, quality, callsPerHour } = salaryData;
            
                const experienceRanges = [
                    { range: '0-3 месяцев', points: 15, selected: experience === '0-3' },
                    { range: '4-9 месяцев', points: 25, selected: experience === '4-9' },
                    { range: '10-15 месяцев', points: 35, selected: experience === '10-15' },
                    { range: '≥16 месяцев', points: 50, selected: experience === '16+' }
                ];
            
                const qualityRanges = [
                    { range: '85-89%', points: 20, selected: quality >= 85 && quality < 90 },
                    { range: '90-94%', points: 25, selected: quality >= 90 && quality < 95 },
                    { range: '95-98%', points: 30, selected: quality >= 95 && quality < 99 },
                    { range: '99-100%', points: 50, selected: quality >= 99 && quality <= 100 }
                ];
            
                const callsPerHourRanges = [
                    { range: '≥5', points: 20, selected: callsPerHour >= 5 && callsPerHour < 10 },
                    { range: '≥10', points: 25, selected: callsPerHour >= 10 && callsPerHour < 15 },
                    { range: '≥15', points: 30, selected: callsPerHour >= 15 && callsPerHour < 20 },
                    { range: '≥20', points: 50, selected: callsPerHour >= 20 }
                ];
                ...
                return (
                    <table className="min-w-full bg-white border-collapse">
                        ...
                        <tbody>
                            ...
                            {experienceRanges.map((row, index) => (
                                <tr key={`exp-${index}`} className={row.selected ? 'bg-blue-100' : ''}>
                                    <td className="p-4 border-b"></td>
                                    <td className="p-4 border-b">{row.range}</td>
                            ...
                            {qualityRanges.map((row, index) => (
                                <tr key={`qual-${index}`} className={row.selected ? 'bg-blue-100' : ''}>
                                    <td className="p-4 border-b"></td>
                                    <td className="p-4 border-b">{row.range}</td>
                                    <td className="p-4 border-b">{row.points}</td>
                                </tr>
                            ...
                            <tr className="bg-gray-100 font-semibold">
                                <td className="p-4 border-b" colSpan="3">Кол-во звонков в час</td>
                            </tr>
                            {callsPerHourRanges.map((row, index) => (
                                <tr key={`cph-${index}`} className={row.selected ? 'bg-blue-100' : ''}>
                                    <td className="p-4 border-b"></td>
                                    <td className="p-4 border-b">{row.range}</td>
                                    <td className="p-4 border-b">{row.points}</td>
                                </tr>
            ...
            // Add this function to clear the salary form
            ...
            const getAudioUrlWithAuth = async (evaluationId, apiKey, userId) => {
                ...
                try {
                    ...
                    const data = await response.json();
                    if (data && data.url) {
                        audioUrlCache[evaluationId] = data.url;
                        return data.url;
                    } else {
                        console.error("No signed URL returned in response:", data);
                        return null;
                    }
                } catch (error) {
            ...
            const handleEvaluationClick = async (evaluation) => {
                if (expandedEvaluation?.id === evaluation.id) {
                    setExpandedEvaluation(null);
                    setAudioUrl(null);
                    return;
                }

                setExpandedEvaluation(evaluation);
                const canLoadAudio = !!operatorData?.sensitive_access?.granted;
                if (!canLoadAudio) {
                    setAudioUrl(null);
            ...
            const getCallCountColor = (count, expected) => {
                if (count >= expected) return 'text-green-600';
                if (count >= expected * 0.5) return 'text-yellow-500';
                return 'text-red-600';
            };

            const getHoursPerformanceColor = (hours, norm) => {
                if (hours >= norm * 0.9) return 'text-green-600';
                if (hours >= norm * 0.5) return 'text-yellow-600';
                return 'text-red-600';
            };

            const getCallsPerHourColor = (CPH) => {
                if (CPH >= 20) return 'text-green-600';
                if (CPH >= 10) return 'text-yellow-600';
                return 'text-red-600';
            };
            ...
            const getCurrentWeekOfMonth = () => {
                ...
                const diff = today.getDate() + firstDay.getDay() - 1;
                return Math.ceil(diff / 7);
            };

            const getExpectedCalls = (weekNumber) => {
                return weekNumber * 5;
            };

            const handleChangeLogin = async () => {
                if (!loginData.new_login || !loginData.confirm_login) {
                    showToast('Please fill in both login fields', 'error');
                    return;
                }
                if (loginData.new_login !== loginData.confirm_login) {
                    showToast('Logins do not match', 'error');
                    return;
                }
                if (loginData.new_login.length < 4) {
                    showToast('Login must be at least 4 characters long', 'error');
                    return;
                }
                setIsLoading(true);
                ...
                try {
                    const response = await axios.post(
                        `${API_BASE_URL}/api/user/change_login`,
                        {
                            user_id: user.id,
                            new_login: loginData.new_login
                        },
                        {
                            headers: {
                                'Content-Type': 'application/json',
                    ...
                    const data = response.data;
                    if (data.status === 'success' && isMounted.current) {
                        showToast(data.message || 'Login updated successfully', 'success');
                        setLoginData({
                            new_login: '',
                            confirm_login: ''
                        });
                    ...
                    console.error('Change login error:', err);
                    showToast(err.response?.data?.error || 'Failed to update login', 'error');
                } finally {
                    if (isMounted.current) setIsLoading(false);
                }
            };
            ...
            const handleChangePassword = async () => {
                ...
                try {
                    const response = await axios.post(
                        `${API_BASE_URL}/api/user/change_password`,
                        {
                            user_id: passwordData.user_id || user.id,
            ...
            const handleLogin = async () => {
                ...
                try {
                    ...
                    setErrorMessage(err.response?.data?.error || 'Ошибка сети');
                    setTimeout(() => setErrorMessage(''), 5000);
                } finally {
                    if (isMounted.current) setIsLoading(false);
                }
            ...
            const confirmLogout = async () => {
                ...
                if (isMounted.current) {
                    ...
                    // Добавьте очистку salary данных
                    setSalaryData({
                        experience: '',
                        quality: '',
                        callsPerHour: '',
                    ...
                    localStorage.removeItem('selectedReportMonth');
                    localStorage.removeItem('calculatorType');
                }
            };

            const fetchSvList = async () => {
                const requestKey = `fetchSvList:${user?.id || 'anonymous'}`;
                return runSingleFlight(requestKey, async () => {
                    setIsAdminDataLoading(true);
                    ...
                    try {
                        ...
                        if (data.status === 'success' && isMounted.current) {
                            ...
                            showToast(data.error || 'Failed to fetch supervisor list', 'error');
                            console.error(data.error || 'Failed to fetch supervisor list', 'error');
                        }
                    } catch (err) {
                        console.error('Fetch SV list error:', err);
                        showToast(err.response?.data?.error || 'Failed to fetch supervisor list', 'error');
                        console.error(err.response?.data?.error || 'Failed to fetch supervisor list', 'error');
                    } finally {
                        if (isMounted.current) setIsAdminDataLoading(false);
                    }
                });
            ...
            const svDataUrl = (id, month) => {
                const base = `${API_BASE_URL}/api/sv/data?id=${encodeURIComponent(id)}`;
                return month ? `${base}&month=${encodeURIComponent(month)}` : base;
                };

                // --- Обновлённый fetchSvData ---
                // Замените вашу текущую функцию fetchSvData на эту версию.
                // Использует selectedReportMonth если он задан, иначе selectedMonth.
            const fetchSvData = async (month = (selectedReportMonth || selectedMonth)) => {
                const requestKey = `fetchSvData:${user?.id || 'anonymous'}:${month || ''}`;
                return runSingleFlight(requestKey, async () => {
                    setIsLoading(true);
                    try {
                        ...
                        showToast(data.error || 'Failed to fetch supervisor data', 'error');
                        }
                    } catch (err) {
                        console.error('Fetch SV data error:', err);
                        showToast(err.response?.data?.error || 'Failed to fetch supervisor data', 'error');
                    } finally {
                        if (isMounted.current) setIsLoading(false);
                    }
                });
            ...
            const fetchOperatorData = async () => {
                const requestKey = `fetchOperatorData:${user?.id || 'anonymous'}:${selectedMonth || ''}`;
                return runSingleFlight(requestKey, async () => {
                    setIsLoading(true);
                    ...
                    try {
                        ...
                        if (data.status === 'success' && isMounted.current) {
                            ...
                            showToast(data.error || 'Failed to fetch operator data');
                        }
                    } catch (err) {
                        console.error('Fetch operator data error:', err);
                        showToast(err.response?.data?.error || 'Failed to fetch operator data');
                    } finally {
                        if (isMounted.current) setIsLoading(false);
                    }
                });
            ...
            const fetchProfileData = async () => {
                ...
                try {
                    ...
                    if (data.status === 'success' && isMounted.current) {
                        ...
                        showToast(data.error || 'Failed to fetch profile data', 'error');
                    }
                } catch (err) {
                    console.error('Fetch profile data error:', err);
                    showToast(err.response?.data?.error || 'Failed to fetch profile data', 'error');
                } finally {
                    if (isMounted.current) setIsLoading(false);
                }
            };
            ...
            const fetchTrainings = async () =>{
                const requestKey = `fetchTrainings:${user?.id || 'anonymous'}:${selectedMonth || ''}`;
                return runSingleFlight(requestKey, async () => {
                    try{
                        ...
                        const data = response.data;
                        if (data.status === 'success' ){
                            setOperatorTrainings(data.trainings);
                        } 
                    }
                    catch (err) {
                        console.error('Fetch training data error:',err);
                        showToast('Fetch training data error','error');
                    }
                });
            };

            const fetchHoursData = async () => {
            const requestKey = `fetchHoursData:${user?.id || 'anonymous'}:${selectedMonth || ''}`;
            return runSingleFlight(requestKey, async () => {
                setIsLoading(true);
                ...
                try {
                    ...
                    // Если сервер уже вернул корректную обёртку с operators — используем её целиком
                    if (Array.isArray(data.operators)) {
                        // Если это список для SV — найдём нашего оператора и гарантируем, что operators содержит хотя бы его
                        const operatorObj = data.operators.find(op => Number(op.operator_id) === Number(user.id)) || data.operators[0];

                        if (operatorObj) {
                        // Сохраняем весь ответ (он уже в нужном формате)
                        setHoursData({
                            status: data.status,
                            month: data.month ?? selectedMonth,
                            days_in_month: data.days_in_month ?? (data.operators[0]?.days_in_month ?? undefined),
                            operators: [operatorObj],
                        });
                        } else {
                        // operators — пуст или не содержит нужного оператора
                        setHoursData({ status: data.status, month: data.month ?? selectedMonth, days_in_month: data.days_in_month, operators: [] });
                        showToast('No daily hours available for this user/month', 'warning');
                        }
                    } else if (data.operator && typeof data.operator === 'object') {
                        // Иногда бэкенд может вернуть { operator: {...}, month:..., days_in_month:... }
                        setHoursData({
                        status: data.status,
                        month: data.month ?? selectedMonth,
                    ...
                    } else if (data.operator === null || (Array.isArray(data.operators) && data.operators.length === 0)) {
                        // Явный случай «нет данных»
                        setHoursData({ status: data.status, month: data.month ?? selectedMonth, days_in_month: data.days_in_month, operators: [] });
                        showToast('No daily hours available for this user/month', 'warning');
                    } else {
                        // Защитный fallback — если прилетел только объект daily map (старый/ошибочный формат),
                        // оборачиваем его в ожидаемый формат: оператор без meta (operator_id не известен)
                        // Но лучше, если бек вернёт нормальный объект — оставляем предупреждение.
                        if (typeof data === 'object' && Object.keys(data).some(k => /^\d+$/.test(k))) {
                        setHoursData({
                            status: 'success',
                            month: selectedMonth,
                            days_in_month: Object.keys(data).length,
                            operators: [{
                            operator_id: user.id,
                            name: null,
                            rate: 0,
                        ...
                        showToast('Warning: received legacy daily map — wrapped into operator object', 'warning');
                        } else {
                        // Нечто совсем неожиданное — сохраним как есть и покажем ошибку
                        setHoursData(data);
                        showToast('Unexpected response format from server', 'error');
                        console.warn('Unexpected /api/sv/daily_hours response:', data);
                        }
                    }
                    } else {
                    showToast(data?.error || 'Failed to fetch daily hours', 'error');
                    // очистим старые данные, чтобы UI не показывал некорректную информацию
                    setHoursData({ status: data?.status ?? 'error', month: selectedMonth, days_in_month: undefined, operators: [] });
                    }
                } catch (err) {
                    console.error('Fetch daily hours error:', err);
                    showToast(err.response?.data?.error || 'Failed to fetch daily hours', 'error');
                    if (isMounted.current) {
                    setHoursData({ status: 'error', month: selectedMonth, days_in_month: undefined, operators: [] });
                    }
                } finally {
                    if (isMounted.current) setIsLoading(false);
                }
            ...
            const fetchSelectedSvData = async (svId, month = (selectedReportMonth || selectedMonth)) => {
                if (!svId) {
                    if (isMounted.current) setSelectedSvData(null);
                    return;
                }
                setIsAdminDataLoading(true);
                try {
                    ...
                    const data = response.data;
                    if (data.status === 'success' && isMounted.current) {
                    setSelectedSvData(data);
                    } else {
                    showToast(data.error || 'Failed to fetch supervisor data', 'error');
                    }
                } catch (err) {
                    console.error('Fetch selected SV data error:', err);
                    showToast(err.response?.data?.error || 'Failed to fetch supervisor data', 'error');
                } finally {
                    if (isMounted.current) setIsAdminDataLoading(false);
                }
            ...
            const addSv = async () => {
                    ...
                    try {
                        ...
                        if (data.status === 'success' && isMounted.current) {
                            ...
                            setNewCredentials({ login: data.login, password: data.password });
                            fetchSvList();
                            setNewSvName('');
                        } else {
                            showToast(data.error || 'Failed to add supervisor', 'error');
                        }
                    } catch (err) {
                        console.error('Add SV error:', err);
                        showToast(err.response?.data?.error || 'Failed to add supervisor', 'error');
                    } finally {
                        if (isMounted.current) setIsLoading(false);
                    }
                };
            ...
            const removeSv = async (svId) => {
                ...
                try {
                    ...
                    const data = response.data;
                    if (data.status === 'success' && isMounted.current) {
                        showToast('Supervisor removed successfully', 'success');
                        fetchSvList();
                    } else {
                        showToast(data.error || 'Failed to remove supervisor', 'error');
                    }
                } catch (err) {
                    console.error('Remove SV error:', err);
                    showToast(err.response?.data?.error || 'Failed to remove supervisor', 'error');
                } finally {
                    if (isMounted.current) setIsLoading(false);
                }
            };
            ...
            const changeSvTable = async (svId, url) => {
                ...
                try {
                    const response = await axios.post(`${API_BASE_URL}/api/admin/change_sv_table`, {
                        telegram_id: svId,
                        table_url: url
                    }, {
                        headers: { 'X-API-Key': user.apiKey }
                    ...
                    if (data.status === 'success' && isMounted.current) {
                        ...
                        setNewTableUrl('');
                        setShowChangeTableForm(false);
                        setCurrentSvForTableChange(null);
                        fetchSvList();
                    } else {
                        showToast(data.error || 'Failed to update table', 'error');
                    }
                } catch (err) {
                    console.error('Change SV table error:', err);
            ...
            const generateMonthlyReport = async () => {
                ...
                try {
                    ...
                    if (err.response) {
                        ...
                        showToast(
                            `Failed to generate report: ${err.response.data?.error || err.message} (HTTP ${err.response.status})`,
                            'error'
                        );
                    } else {
            ...
            // --- Функция для получения AI-отзыва за месяц ---
            const openAiMonthlyFeedback = async (operatorId, operatorName, month) => {
                if (!operatorId) {
                ...
                setAiMonthlyFeedbackTitle(`${operatorName || 'Оператор'} · ${month}`);
                setAiMonthlyFeedbackError('');
                setAiMonthlyFeedbackResult(null);
                setAiMonthlyFeedbackLoading(true);
                setShowAiMonthlyFeedbackModal(true);

                try {
                    const resp = await axios.post(`${API_BASE_URL}/api/ai/monthly_feedback`, {
                        operator_id: operatorId,
                        month
                    }, {
                        headers: {
                    ...
                    const data = resp?.data;
                    if (data?.status === 'success') {
                        setAiMonthlyFeedbackResult(data.result || null);
                    } else {
                        setAiMonthlyFeedbackError(typeof data?.error === 'string' ? data.error : JSON.stringify(data?.error || data || 'Ошибка', null, 2));
                    }
                } catch (err) {
                    const msg = err?.response?.data?.error || err?.message || 'Ошибка запроса';
                    setAiMonthlyFeedbackError(typeof msg === 'string' ? msg : JSON.stringify(msg, null, 2));
                } finally {
                    setAiMonthlyFeedbackLoading(false);
                }
            };

            const closeAiMonthlyFeedbackModal = () => {
                setShowAiMonthlyFeedbackModal(false);
                setAiMonthlyFeedbackLoading(false);
                setAiMonthlyFeedbackError('');
            };
            ...
            const notifySupervisor = async (svId, operatorName) => {
                ...
                try {
                    ...
                    console.error('Notify SV error:', err);
            ...
            const clearSensitiveQrPolling = useCallback(() => {
                if (sensitiveQrPollRef.current) {
                    clearInterval(sensitiveQrPollRef.current);
                    sensitiveQrPollRef.current = null;
                }
            }, []);
            ...
            const closeSensitiveQrModal = useCallback(() => {
                clearSensitiveQrPolling();
                setShowSensitiveQrModal(false);
                setSensitiveQrUrl('');
                setSensitiveQrImage('');
            ...
            const requestSensitiveQrAccess = async () => {
                ...
                try {
                    ...
                    clearSensitiveQrPolling();
                    sensitiveQrPollRef.current = setInterval(() => {
                        fetchSensitiveAccessStatus();
                    }, 2500);
                } catch (err) {
                    const msg = err.response?.data?.error || err.message || 'Не удалось сформировать QR';
                    setSensitiveQrError(msg);
                }
            };
                    ...
                    <div className={`sidebar fixed top-0 left-0 h-full bg-gradient-to-b from-blue-800 to-blue-900 text-white shadow-2xl z-50 overflow-visible ${sidebarCollapsed ? 'collapsed' : ''} ${mobileMenuOpen ? 'mobile-open' : ''}`}>
                        <div className="p-4">
                            ...
                            <ul className="space-y-2">
                                ...
                                {/* Убераю письмо от модера
                                <li>
                                        <button onClick={() => { console.log('DevLetter button clicked'); showToast?.('С наступающим вас!'); setShowDevLetterModal(true); try { startConfetti(); } catch(e){}; setMobileMenuOpen(false); }} className={`w-full text-left py-3 px-4 rounded-lg hover:bg-blue-700 transition-all duration-200 flex items-center gap-3`}>
                                        <i className="fas fa-envelope"></i> <span className="sidebar-text">Письмо от модера</span>
                                    </button>
                                </li>
                                */}
                        ...
                        <>
                                ...
                                <div className="bg-white rounded-2xl shadow-sm mb-6 border border-gray-100 overflow-hidden">
                                    ...
                                    <div className="p-5 space-y-6">
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            {/* ✅ Кнопка подтверждения после сканирования */}
                                            {qrApproveInput && !qrApproveResult && (
                                                <div className="mt-1 animate-in fade-in slide-in-from-bottom-2 duration-300">
                                                    <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 space-y-3">
                                                        <div className="flex items-center gap-2 text-blue-800 text-sm font-medium">
                                                            <i className="fa-solid fa-qrcode text-blue-500 text-base"></i>
                                                            QR успешно распознан
                                                        </div>
                                                        <p className="text-xs text-blue-600 font-mono bg-blue-100/60 rounded-lg px-3 py-2 break-all line-clamp-2">
                                                            {qrApproveInput}
                                                        </p>
                                                        <button
                                                            onClick={approveSensitiveQrAccess}
                                                            disabled={qrApproveLoading}
                                                            className={`w-full py-3 rounded-xl text-sm font-semibold text-white transition active:scale-95 shadow-md shadow-blue-200 ${
                                                                qrApproveLoading
                                                                    ? 'bg-blue-400 cursor-not-allowed'
                                                                    : 'bg-blue-600 hover:bg-blue-700'
                                                            }`}
                            ...
                            <div className="bg-white p-8 rounded-xl shadow-md mb-8 border border-gray-200 transition-all duration-300 hover:shadow-lg">
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <button
                                    onClick={() => setShowAddSvModal(true)}
                                    className="bg-green-600 text-white px-6 py-3 rounded-lg hover:bg-green-700 transition-all duration-200 min-w-[180px] text-center"
                                    >
                                    + Добавить СВ
                                    </button>
                                </div>
                                ...
                                <div className="flex items-center space-x-3 mb-4">
                                    {(() => {
                                        const all = svList ?? [];
                                        const activeCount = all.filter(sv => sv.status === 'working' || sv.status === 'unpaid_leave' || !sv.status).length;
                                        const firedCount = all.filter(sv => sv.status === 'fired').length;
                                        return (
                                        <>
                                            <button
                                            onClick={() => setActiveSvTab('active')}
                                            className={`px-4 py-2 rounded-lg font-medium text-sm focus:outline-none ${activeSvTab === 'active' ? 'bg-green-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}
                                            >
                                            ...
                                            <button
                                            onClick={() => setActiveSvTab('fired')}
                                            className={`px-4 py-2 rounded-lg font-medium text-sm focus:outline-none ${activeSvTab === 'fired' ? 'bg-red-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}
                                            >
                                ...
                                <p className="text-center text-gray-600">Нету супервайзеров.</p>
                                ) : (
                                <div className="overflow-visible rounded-lg border border-gray-200">
                                    ...
                                    <colgroup>
                                        ...
                                        <col style={{ width: "30%" }} /> {/* Hire Date */}
                                        <col style={{ width: "10%" }} />  {/* Menu */}
                                    </colgroup>
                                    ...
                                    <thead className="bg-gray-50">
                                        ...
                                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Дата принятия</th>
                                    ...
                                    <tbody className="bg-white divide-y divide-gray-200">
                                        {(() => {
                                        const all = svList ?? [];
                                        const filtered = activeSvTab === 'fired'
                                            ? all.filter(sv => sv.status === 'fired')
                                            : all.filter(sv => sv.status === 'working' || sv.status === 'unpaid_leave' || !sv.status);

                                        if (filtered.length === 0) {
                                            return (
                                                <tr>
                                                    <td colSpan={5} className="px-6 py-4 text-center text-gray-600">Соответствующие супервайзеры не найдены.</td>
                                                </tr>
                                            );
                                        }

                                        return filtered.map((sv, idx) => (
                                        <tr key={sv.id} className="hover:bg-gray-50 transition-colors duration-200 group">
                                            {/* Number */}
                                            ...
                                            {sv.status === "working" || !sv.status ? (
                                                ...
                                                <span className="border border-gray-500 text-gray-700 px-2 py-1 rounded text-sm">{sv.status}</span>
                                            )}
                                            </td>
                                            ...
                                            {sv.hire_date
                                                ? (() => {
                                                    const [day, month, year] = sv.hire_date.split("-");
                                            ...
                                            {/* Menu */}
                                            <td
                                            className={`px-2 py-4 text-right transition-opacity duration-200 relative overflow-visible ${
                                                openMenuId === sv.id ? "opacity-100" : "opacity-0 group-hover:opacity-100"
                                            }`}
                                            >
                                            <div className="relative inline-block text-left">
                                                <button
                                                onClick={() =>
                                                    setOpenMenuId(openMenuId === sv.id ? null : sv.id)
                                                }
                                                ...
                                                <div className="absolute right-0 mt-2 w-40 bg-white border rounded-lg shadow-lg z-50">
                                                    ...
                                                    <button
                                                    onClick={() => {
                                                        setSelectedUserForHistory(sv);
                                                        fetchUserHistory(sv.id);
                                                        setOpenMenuId(null);
                                                    }}
                                                    className="block w-full text-left px-4 py-2 hover:bg-gray-100"
                                                    >
                                ...
                                <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
                                    ...
                                    <input
                                        type="text"
                                        placeholder="Имя супервайзера"
                                        value={newSvName}
                                        onChange={(e) => setNewSvName(e.target.value)}
                                        className="w-full p-3 mb-6 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all duration-200"
                                        disabled={isLoading}
                                    />
                                    <button
                                        onClick={addSv}
                                        className={`w-full bg-blue-600 text-white p-3 rounded-lg hover:bg-blue-700 transition-all duration-200 ${isLoading ? 'cursor-not-allowed opacity-50' : ''}`}
                                        disabled={isLoading}
                                    >
                                        {isLoading ? 'Добавление...' : 'Добавить'}
                                    </button>

                                    {/* Показать новые креды */}
                                    {newCredentials && (
                                        <div className="mt-4 p-4 bg-green-100 rounded-lg">
                                        <p className="font-bold mb-2">Учетные данные нового СВ:</p>
                                        <p className="mb-1">
                                            Логин: <span className="font-mono bg-gray-200 px-2 py-1 rounded">{newCredentials.login}</span>
                                        </p>
                                        <p>
                                            Пароль: <span className="font-mono bg-gray-200 px-2 py-1 rounded">{newCredentials.password}</span>
                                        </p>
                                        <p className="text-sm text-gray-600 mt-2 italic">
                                            Примечание: эти учётные данные отображаются только один раз. Пожалуйста, запишите их в безопасное место.
                                        </p>
                                        </div>
                                    )}
                                ...
                                {view === 'view_scores' && (
                                    <div className="bg-white p-6 rounded-xl shadow-md mb-8 border border-gray-200 transition-all duration-300 hover:shadow-lg">
                                        ...
                                        (() => {
                                            ...
                                            // Группируем по направлению (показываем заголовки групп, но колонку направления не выводим)
                                            ...
                                            // Сортировка групп: если выбран field = 'direction' — сортируем группы по имени направления,
                                            // иначе внутри каждой группы сортируем по выбранному полю (combined: primary + name)
                                            let sortedDirectionNames = Object.keys(grouped);
                                            if (viewScoresSortField === 'direction') {
                                            sortedDirectionNames = sortedDirectionNames.sort((a, b) => {
                                                const res = a.localeCompare(b, 'ru', { sensitivity: 'base' });
                                                return viewScoresSortDirection === 'asc' ? res : -res;
                                            });
                                            // внутри группы — сортировка по имени оператора
                                            sortedDirectionNames.forEach(dir => {
                                                grouped[dir].sort((x, y) => (x.name || '').localeCompare(y.name || '', 'ru', { sensitivity: 'base' }) * (viewScoresSortDirection === 'asc' ? 1 : -1));
                                            });
                                            } else {
                                            // сортируем внутри групп по выбранному полю
                                            sortedDirectionNames.forEach(dir => {
                                                grouped[dir].sort((x, y) => compareViewScoresByField(x, y, viewScoresSortField));
                                            });
                                            // стабильный порядок групп по имени направления
                                            sortedDirectionNames = sortedDirectionNames.sort((a, b) => a.localeCompare(b, 'ru', { sensitivity: 'base' }));
                                            }
                            ...
                            <>
                                ...
                                {view === 'manage_operators' && (
                                    <div className="bg-white p-8 rounded-xl shadow-md mb-8 border border-gray-200 transition-all duration-300 hover:shadow-lg">
                                        ...
                                        (() => {
                                            ...
                                            // Сортируем группы: если сортируем по 'direction' — сортируем имена групп по направлению + вторично по ФИО
                                            let sortedDirectionNames = Object.keys(grouped);
                                            ...
                                            // внутри групп — комбинированная сортировка (direction primary, name secondary)
                                            sortedDirectionNames.forEach((dir) => {
                                                grouped[dir].sort((x, y) => {
                                                // primary same for all in group, so compare by name (respect sortDirection for combined behavior)
                                                return (x.name || '').localeCompare(y.name || '', 'ru', { sensitivity: 'base' }) * (sortDirection === 'asc' ? 1 : -1);
                                                });
                                            });
                                            } else {
                                            // для всех остальных полей — внутри каждой группы сортируем по (primary field, затем name)
                                            sortedDirectionNames.forEach((dir) => {
                                                grouped[dir].sort((x, y) => compareByField(x, y, sortField));
                                            });
                                            // сортируем список групп по имени направления для стабильного порядка
                                            sortedDirectionNames = sortedDirectionNames.sort((a, b) => a.localeCompare(b, 'ru', { sensitivity: 'base' }));
                                            }
                                ...
                                <div className="bg-white p-8 rounded-xl shadow-md mb-8 border border-gray-200 transition-all duration-300 hover:shadow-lg">
                                    ...
                                    <div className="flex items-end gap-3 flex-wrap">
                                        ...
                                        className="p-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                                        disabled={isAdminDataLoading}
                                        aria-label="Выберите месяц для отчета"
                                        >
                                        {getMonthOptions()}
                                        </select>
                                    ...
                                    {(() => {
                                        ...
                                        <>
                                            ...
                                            className={`px-4 py-2 rounded-lg font-medium text-sm focus:outline-none ${activeOperatorsTab === 'active' ? 'bg-green-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}
                                            role="tab"
                                            aria-selected={activeOperatorsTab === 'active'}
                                            aria-controls="operators-active"
                                            >
                                            Активные <span className="ml-2 inline-flex items-center justify-center px-2 py-0.5 text-xs rounded bg-white text-gray-700 font-medium">{activeCount}</span>
                                            </button>
                                            ...
                                            className={`px-4 py-2 rounded-lg font-medium text-sm focus:outline-none ${activeOperatorsTab === 'fired' ? 'bg-red-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}
                                            role="tab"
                                            aria-selected={activeOperatorsTab === 'fired'}
                                            aria-controls="operators-fired"
                                            >
                                            Уволенные <span className="ml-2 inline-flex items-center justify-center px-2 py-0.5 text-xs rounded bg-white text-gray-700 font-medium">{firedCount}</span>
                                            </button>
                                    ...
                                    (() => {
                                        ...
                                        // Сортируем группы: если сортируем по direction, сортируем сами группы по имени направления,
                                        // иначе сортируем внутри каждой группы по opsSortField (комбинированно: primary + name)
                                        let sortedDirectionNames = Object.keys(grouped);
                                        if (opsSortField === 'direction') {
                                        sortedDirectionNames = sortedDirectionNames.sort((a, b) => {
                                            const res = a.localeCompare(b, 'ru', { sensitivity: 'base' });
                                            return opsSortDirection === 'asc' ? res : -res;
                                        });
                                        // внутри группы — сортировка по имени (в направлении учитываем opsSortDirection)
                                        sortedDirectionNames.forEach(dir => {
                                            grouped[dir].sort((x, y) => (x.name || '').localeCompare(y.name || '', 'ru', { sensitivity: 'base' }) * (opsSortDirection === 'asc' ? 1 : -1));
                                        });
                                        } else {
                                        // сортируем внутри групп по выбранному полю (compareOpsByField делает secondary sort по name)
                                        sortedDirectionNames.forEach(dir => {
                                            grouped[dir].sort((x, y) => compareOpsByField(x, y, opsSortField));
                                        });
                                        // стабильный порядок групп по имени направления
                                        sortedDirectionNames = sortedDirectionNames.sort((a, b) => a.localeCompare(b, 'ru', { sensitivity: 'base' }));
                                        }
                                        ...
                                        <div className="overflow-x-auto rounded-lg border border-gray-200">
                                            ...
                                            <thead className="bg-gray-50">
                                                <tr>
                                                <th
                                                    onClick={() => handleOpsSort('name')}
                                                    className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer select-none"
                                                >
                                                    Имя {getOpsSortIcon('name')}
                                                </th>
                                                <th
                                    ...
                                    <p className="text-center text-gray-600 py-8">Операторы не найдены.</p>
                                    )}
                                </div>
                                )} 
                                {( view === "trainings" && (<TrainingsView user={user} operators={users} />))}
                                ...
                                {view === 'hours' && (
                                    <div className="workhours-section workhours-container bg-white p-4 sm:p-8 rounded-xl shadow-md mb-8 border border-gray-200 transition-all duration-300 hover:shadow-lg">
                                        ...
                                        (() => {
                                            ...
                                            // --- собираем релевантные тренинги для оператора в выбранном месяце ---
                                            const trainingsForOp = Array.isArray(operatorTrainings)
                                            ? operatorTrainings.filter(t => {
                                                if (!t) return false;
                                                // фильтрация по оператору (если указан)
                                                if (t.operator_id && Number(t.operator_id) !== Number(op.operator_id)) return false;
                                                // фильтрация по месяцу если есть поле date (формат YYYY-MM-DD)
                                                if (t.date) {
                                                    try {
                                                    if (String(t.date).slice(0, 7) !== selectedMonth) return false;
                                                    } catch (e) { /* ignore */ }
                                                }
                                                return true;
                                                })
                                            ...
                                            <div className="workhours-main-grid grid grid-cols-1 md:grid-cols-3 gap-6">
                                                ...
                                                {/* Правая часть – календарь (передаём весь оператор-объект) */}
                                                <div className="workhours-right col-span-1 md:col-span-1">
                                                <WorkHoursCalendar
                                                    hoursData={op}
                                        ...
                                        return isLoading ? (
                                            ...
                                            <div className="block lg:hidden space-y-3">
                                                {allEvals.map((ev, index) => {
                                                    ...
                                                    const handleCardClick = () => {
                                                        ...
                                                        setExpandedEvalId(isExpanded ? null : id);
                                                        handleEvaluationClick(ev);
                                                    };

                                                    return (
                                                        <div key={id} className={`bg-white rounded-xl border ${isExpanded ? 'border-blue-300 shadow-md' : 'border-gray-200'} ${isImported ? 'opacity-60' : ''} transition-all duration-200`}>
                                                            <div 
                                                                className={`p-4 ${isImported ? 'cursor-not-allowed' : 'cursor-pointer active:bg-gray-50'}`}
                                                                onClick={handleCardClick}
                                                            >
                                                                {/* Header row: score badge + phone */}
                                                                <div className="flex items-center justify-between mb-3">
                                                                    <div className="flex items-center gap-2">
                                                                ...
                                                                {Array.isArray(ev.criterion_comments) && ev.criterion_comments.filter(Boolean).length > 0 ? (
                                                                    <div className="text-xs text-gray-500 line-clamp-2">
                                                                        {ev.criterion_comments.filter(Boolean).slice(0,1).map((c,i)=>(
                                                                            <span key={i}>{c}</span>
                                                                        ))}
                                                                        {ev.criterion_comments.filter(Boolean).length > 1 && <span className="text-gray-400"> и ещё {ev.criterion_comments.filter(Boolean).length - 1}...</span>}
                                                                    </div>
                                                                ) : ev.comment ? (
                                                            ...
                                                            {isExpanded && (
                                                                <div className="border-t border-gray-200 p-4 bg-gray-50 space-y-4">
                                                                    <div>
                                                                        <h4 className="font-semibold text-gray-900 mb-2 text-sm">Общий комментарий</h4>
                                                                        {ev.comment ? (
                                                                            String(ev.comment).split(/\r?\n/).map((line, i) => (
                                                                    ...
                                                                    <div>
                                                                        ...
                                                                        <div className="space-y-2">
                                                                            {Array.isArray(ev.direction?.criteria) && ev.direction.criteria.length > 0 ? (
                                                                                ev.direction.criteria.map((crit, cidx) => {
                                                                                    const status = Array.isArray(ev.scores) ? ev.scores[cidx] : null;
                                                                                    const critComment = Array.isArray(ev.criterion_comments) ? ev.criterion_comments[cidx] : '';
                                                                                    return (
                                                                                        <div key={cidx} className="bg-white rounded-lg p-3 border border-gray-200">
                                                                                            <div className="flex items-start justify-between gap-2 mb-1">
                                                                                                <span className="text-sm font-medium text-gray-800">{cidx + 1}. {crit.name}</span>
                                                                                                <span className={`shrink-0 text-xs px-2 py-0.5 rounded-full font-medium ${
                                                                                                    status === 'Correct' ? 'bg-green-100 text-green-700' :
                                                                                                    status === 'Incorrect' ? 'bg-red-100 text-red-700' :
                                                                                                    status === 'N/A' ? 'bg-gray-100 text-gray-700' : 'bg-yellow-100 text-yellow-700'
                                                                                                }`}>{status || '-'}</span>
                                                                                            </div>
                                                                                            <div className="flex gap-3 text-xs text-gray-500 mb-1">
                                                                                                <span>Вес: {crit.weight ?? '-'}</span>
                                                                                                <span>Критич.: {crit.isCritical ? 'Да' : 'Нет'}</span>
                                                                                            </div>
                                                                                            {critComment && <div className="text-xs text-gray-600 mt-1">{critComment}</div>}
                                                                                        </div>
                                                                                    );
                                                                                })
                                                                            ) : Array.isArray(ev.scores) && ev.scores.length > 0 ? (
                                                                                ev.scores.map((status, cidx) => {
                                                                                    const critComment = Array.isArray(ev.criterion_comments) ? ev.criterion_comments[cidx] : '';
                                                                                    const name = (ev.criterion_names && ev.criterion_names[cidx]) || `Критерий ${cidx+1}`;
                                                                                    return (
                                                                                        <div key={cidx} className="bg-white rounded-lg p-3 border border-gray-200">
                                                                                            <div className="flex items-start justify-between gap-2 mb-1">
                                                                                                <span className="text-sm font-medium text-gray-800">{cidx + 1}. {name}</span>
                                                                                                <span className={`shrink-0 text-xs px-2 py-0.5 rounded-full font-medium ${
                                                                                                    status === 'Correct' ? 'bg-green-100 text-green-700' :
                                                                                                    status === 'Incorrect' ? 'bg-red-100 text-red-700' :
                                                                                                    status === 'N/A' ? 'bg-gray-100 text-gray-700' : 'bg-yellow-100 text-yellow-700'
                                                                                ...
                                                                                <div className="text-sm text-gray-500">Критерии не найдены.</div>
                                                                            )}
                                                                        </div>
                                                                    </div>
                                                                    ...
                                                                    <div className="text-xs text-gray-500 pt-2 border-t border-gray-200">
                                                                        ...
                                                                        <div>Дата создания: {formatDate(ev.created_at)}</div>
                                                                    </div>
                                                                </div>
                                                            )}
                                                        </div>
                                            ...
                                            <div className="hidden lg:block overflow-x-auto rounded-xl shadow border border-gray-200">
                                                ...
                                                <thead className="bg-blue-50">
                                                    <tr>
                                                    {['No.', 'Номер телефона', 'Дата обращения', 'Балл', 'Краткая сводка', 'Месяц', 'Дата оценки', 'Действия'].map(
                                                        (head, idx) => (
                                                        <th
                                                            key={idx}
                                                ...
                                                <tbody className="bg-white divide-y divide-gray-100">
                                                    ...
                                                    return (
                                                        ...
                                                        <tr
                                                            className={`transition-colors duration-200 ${isImported ? 'opacity-60 cursor-not-allowed' : 'hover:bg-gray-50 cursor-pointer'} ${isExpanded ? 'bg-blue-50' : ''}`}
                                                        ...
                                                        >
                                                            ...
                                                            <td className="px-6 py-4 whitespace-nowrap">{formatDate(ev.appeal_date || ev.created_at)}</td>
                                                            <td className={`px-6 py-4 whitespace-nowrap font-semibold ${getScoreColor?.(ev.score) || ''}`}>
                                                            {ev.score != null ? Number(ev.score).toFixed(0) : '-'}
                                                            </td>
                                                            ...
                                                            {Array.isArray(ev.criterion_comments) && ev.criterion_comments.filter(Boolean).length > 0 ? (
                                                                ...
                                                                {ev.criterion_comments.filter(Boolean).length > 2 && <div className="text-xs text-gray-400">и ещё {ev.criterion_comments.filter(Boolean).length - 2}...</div>}
                                                                </div>
                                                            ) : ev.comment ? (
                                                        ...
                                                        {isExpanded && (
                                                            <tr>
                                                            <td colSpan={8} className="p-4 bg-gray-50 border-t text-sm">
                                                                <div className="space-y-4">
                                                                <div>
                                                                    <h4 className="font-semibold text-gray-900 mb-2">Общий комментарий</h4>
                                                                    {ev.comment ? (
                                                                    String(ev.comment).split(/\r?\n/).map((line, i) => (
                                                                        <p key={i} className="text-sm text-gray-700 whitespace-pre-wrap">{line}</p>
                                                                    ))
                                                                ...
                                                                <div>
                                                                    ...
                                                                    <table className="min-w-full text-sm">
                                                                        ...
                                                                        <tr>
                                                                            ...
                                                                            <th className="p-3 text-left text-xs font-semibold text-gray-600">Критерий</th>
                                                                            <th className="p-3 text-left text-xs font-semibold text-gray-600">Вес</th>
                                                                            <th className="p-3 text-left text-xs font-semibold text-gray-600">Критичность</th>
                                                                            <th className="p-3 text-left text-xs font-semibold text-gray-600">Статус</th>
                                                                            <th className="p-3 text-left text-xs font-semibold text-gray-600">Комментарий</th>
                                                                        </tr>
                                                                        </thead>
                                                                        <tbody>
                                                                        {Array.isArray(ev.direction?.criteria) && ev.direction.criteria.length > 0 ? (
                                                                            ...
                                                                            const status = Array.isArray(ev.scores) ? ev.scores[cidx] : null;
                                                                            const critComment = Array.isArray(ev.criterion_comments) ? ev.criterion_comments[cidx] : '';
                                                                            const shortValue = crit?.value ? String(crit.value).split('\n')[0].slice(0,120) : '';
                                                                            return (
                                                                                <tr key={cidx} className="even:bg-gray-50">
                                                                                <td className="p-3 align-top">{cidx + 1}</td>
                                                                                <td className="p-3 align-top">
                                                                                    <div className="font-medium text-gray-800">{crit.name}</div>
                                                                                    {shortValue && <div className="text-xs text-gray-500 mt-1">{shortValue}</div>}
                                                                                </td>
                                                                                <td className="p-3 align-top">{crit.weight ?? '-'}</td>
                                                                                <td className="p-3 align-top">{crit.isCritical ? 'Да' : 'Нет'}</td>
                                                                                <td className="p-3 align-top">
                                                                                ...
                                                                                <td className="p-3 align-top text-gray-700">{critComment || '-'}</td>
                                                                                </tr>
                                                                            );
                                                                            })
                                                                        ) : Array.isArray(ev.scores) && ev.scores.length > 0 ? (
                                                                            ...
                                                                            return (
                                                                                <tr key={cidx} className="even:bg-gray-50">
                                                                                <td className="p-3 align-top">{cidx + 1}</td>
                                                                                <td className="p-3 align-top"><div className="font-medium text-gray-800">{name}</div></td>
                                                                            ...
                                                                            <tr>
                                                                            <td colSpan={6} className="p-3 text-gray-500">Критерии не найдены.</td>
                                                                            </tr>
                                                                        )}
                                                                        </tbody>
                                                                    </table>
                                            ...
                                            <div className="bg-white p-8 rounded-xl shadow-md mb-8 border border-gray-200 transition-all duration-300 hover:shadow-lg">
                                                ...
                                                {calculatorType === 'call' ? (
                                                        ...
                                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                                            <div className="p-6 bg-gray-50 rounded-xl shadow-sm hover:shadow-md transition mb-4">
                                                                ...
                                                                >
                                                                    ...
                                                                    <option value="0-3">0-3 месяцев</option>
                                                            ...
                                                            <div className="p-6 bg-gray-50 rounded-xl shadow-sm hover:shadow-md transition mb-4">
                                                                ...
                                                                <input
                                                                    type="number"
                                                                    value={salaryData.quality}
                                                                    onChange={(e) => setSalaryData({ ...salaryData, quality: e.target.value })}
                                                                    min="0"
                                                                    max="100"
                                                                    step="0.01"
                                                                    className="w-full p-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                                                            ...
                                                            <div className="p-6 bg-gray-50 rounded-xl shadow-sm hover:shadow-md transition mb-4">
                                                                ...
                                                                <input
                                                                    type="number"
                                                                    value={salaryData.callsPerHour}
                                                                    onChange={(e) => setSalaryData({ ...salaryData, callsPerHour: e.target.value })}
                                                                    min="0"
                                                                    max="100"
                                                                    step="0.01"
                                                                    className="w-full p-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                                                            ...
                                                            <div className="p-6 bg-gray-50 rounded-xl shadow-sm hover:shadow-md transition mb-4">
                                                                ...
                                                                <input
                                                                    type="number"
                                                                    value={salaryData.hoursNorm}
                                                                    onChange={(e) => setSalaryData({ ...salaryData, hoursNorm: e.target.value })}
                                                                    min="0"
                                                                    max="744"
                                                                    step="0.01"
                                                                    className="w-full p-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                                                            ...
                                                            <div className="p-6 bg-gray-50 rounded-xl shadow-sm hover:shadow-md transition mb-4">
                                                                ...
                                                                <input
                                                                    type="number"
                                                                    value={salaryData.totalHours}
                                                                    onChange={(e) => setSalaryData({ ...salaryData, totalHours: e.target.value })}
                                                                    min="0"
                                                                    max="744"
                                                                    step="0.01"
                                                                    className="w-full p-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                                                                ...
                                                                <div className="space-y-4 mt-2">
                                                                    ...
                                                                    <div className="flex items-center gap-4">
                                                                        ...
                                                                        {salaryData.bonusRefer && (
                                                                            <input
                                                                                type="number"
                                                                                value={salaryData.bonusReferQuantity}
                                                                                onChange={(e) => setSalaryData({ ...salaryData, bonusReferQuantity: e.target.value })}
                                                                                min="1"
                                                                                step="1"
                                                                                placeholder="Кол-во"
                                                                    ...
                                                                    <div className="flex items-center gap-4">
                                                                        ...
                                                                        {salaryData.bonusFilming && (
                                                                            <input
                                                                                type="number"
                                                                                value={salaryData.bonusFilmingQuantity}
                                                                                onChange={(e) => setSalaryData({ ...salaryData, bonusFilmingQuantity: e.target.value })}
                                                                                min="1"
                                                                                step="1"
                                                                                placeholder="Кол-во"
                                                                                className="w-24 p-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                                                        ...
                                                        <div className="flex justify-center gap-4 mt-8">
                                                            <button
                                                                ...
                                                                className={`px-8 py-4 rounded-xl font-bold text-lg bg-green-500 text-white hover:bg-green-600 shadow transition-all duration-200 ${
                                                                    ...
                                                                    !salaryData.hoursNorm ||
                                                                    !salaryData.totalHours ||
                                                                    (salaryData.bonusRefer && !salaryData.bonusReferQuantity) ||
                                                    ...
                                                    <div className="grid grid-cols-1 gap-6">
                                                            ...
                                                            <div className="form-group">
                                                              <label className="block mb-2 font-semibold text-gray-700">KPI (макс 2.5):</label>
                                                              <input
                                                                type="number"
                                                                value={converterData.kpi}
                                                                onChange={(e) => {
                                                                  let val = parseFloat(e.target.value) || "";
                                                                  if (val > 2.5) val = 2.5;
                                                                  setConverterData({ ...converterData, kpi: val });
                                                                }}
                                                                min="0"
                                                                max="2.5"
                                                                step="0.01"
                                                                ...
                                                                <table className="w-full border border-gray-300 rounded-lg overflow-hidden">
                                                                  <thead className="bg-gray-100">
                                                                    <tr>
                                                                      ...
                                                                      <th className="p-3 text-left border-b">Часы</th>
                                                                    </tr>
                                                                  </thead>
                                                                  <tbody>
                                                                    {[2.5, 1.9, 1.5].map((k) => (
                                                                      <tr key={k} className="odd:bg-white even:bg-gray-50">
                                                                        <td className="p-3 border-b">{k}</td>
                                                                        <td className="p-3 border-b">
                                                                          {(converterData.amount / (k * 700)).toFixed(2)} ч.
                                                                        </td>
                                                                      </tr>
                                                                    ))}
                                                                  </tbody>
                            ...
                            <Suspense fallback={null}>
                                <HistoryModal
                                    ...
                                    subjectName={
                                        ...
                                        ? (selectedUserForHistory.name || selectedUserForHistory.login || selectedUserForHistory.id)
                                            : ""
                                    }
                                    />
                            </Suspense>
                            ...
                            {showChangeLoginForm && (
                                ...
                                <div className="px-6 py-5">
                                    ...
                                    onSubmit={(e) => {
                                        ...
                                        if (newLogin.length < 3) { setModalError("Логин должен быть минимум 3 символа."); return; }
                                    ...
                                    <div className="relative">
                                        ...
                                        placeholder="например: ivan1987"
                                        value={loginData.new_login}
                                        onChange={(e) =>
                                            setLoginData({ ...loginData, new_login: e.target.value })
                                        }
                                    ...
                                    <div className="relative">
                                        ...
                                        onChange={(e) =>
                                            setLoginData({ ...loginData, confirm_login: e.target.value })
                                        }
                                        className="w-full pl-10 p-3 border border-gray-300 rounded-lg 
                                                    focus:outline-none focus:ring-2 focus:ring-blue-500 
                                                    transition-all duration-150 bg-white/90 dark:bg-slate-800 
                                    ...
                                    <button
                                        type="submit"
                                        className={`inline-flex items-center justify-center gap-2 px-6 py-3 rounded-xl text-white font-medium shadow-md transition 
                    ...
                    {showSensitiveQrModal && (
                        ...
                        >
                            ...
                            >
                                ...
                                <div className="mb-3">
                                    <p className="text-xs text-gray-500 mb-1">Резервный код:</p>
                                    <div className="flex items-center gap-2">
                                        <input
                                            type="text"
                                            readOnly
                                            value={sensitiveQrUrl}
                                            className="flex-1 p-2 border border-gray-300 rounded-lg text-xs text-gray-700 bg-gray-50"
                                        />
                                        <button
                                            onClick={() => navigator.clipboard?.writeText(sensitiveQrUrl || '')}
                                            className="px-3 py-2 rounded-lg bg-gray-200 text-gray-800 hover:bg-gray-300 text-xs"
                                        >
                                            Копировать
                                        </button>
                                    </div>
                                </div>
                                {sensitiveQrExpiresAt && (
                                    <p className="text-xs text-gray-500 mb-3">
                                        Действителен до: {formatDate(sensitiveQrExpiresAt)}
                                    </p>
                                )}
                                <div className="flex items-center gap-2">
                                    <button
                                        onClick={requestSensitiveQrAccess}
                                        className="flex-1 px-3 py-2 rounded-lg bg-blue-600 text-white hover:bg-blue-700 text-sm"
                                    >
                                        Обновить QR
                                    </button>
                                    <button
                                        onClick={fetchSensitiveAccessStatus}
                                        className="px-3 py-2 rounded-lg bg-gray-200 text-gray-800 hover:bg-gray-300 text-sm"
                                    >
                                        Проверить
                                    </button>
                                </div>
                                ...
                                <div className="p-4 sm:p-6 md:p-10">
                                    ...
                                    <div className="mb-10">
                                        <div className="mb-3">
                                            ...
                                            <div className="mt-3 h-[1px] w-[21.2rem] bg-blue-600"></div>
                                        </div>
                                    ...
                                    {/* Content */}
                                        ...
                                        {/* Carousel: photos with caption overlay (wider + horizontal scroll for thumbnails) */}
                                        <div className="mt-6">
                                            <div className="relative bg-gray-100 rounded-2xl overflow-hidden aspect-[16/9]">
                                                <img src={devLetterImages[devLetterIndex].src} alt={devLetterImages[devLetterIndex].caption} onMouseEnter={() => { devAutoplayPausedRef.current = true; }} onMouseLeave={() => { devAutoplayPausedRef.current = false; }} onTouchStart={() => { devAutoplayPausedRef.current = true; }} onTouchEnd={() => { devAutoplayPausedRef.current = false; }} className="absolute inset-0 w-full h-full object-cover" />
                                                <div className="absolute inset-0 bg-gradient-to-t from-black/40 via-transparent to-transparent"></div>
                                                <div className="absolute left-4 sm:left-6 bottom-4 sm:bottom-6">
                                                    <div className="bg-black/60 text-white px-3 sm:px-4 py-1.5 sm:py-2 rounded-md text-sm sm:text-sm max-w-[86%]">{devLetterImages[devLetterIndex].caption}</div>
                                                </div>

                                                <button onClick={() => setDevLetterIndex((devLetterIndex - 1 + devLetterImages.length) % devLetterImages.length)} className="absolute left-2 sm:left-4 top-1/2 -translate-y-1/2 bg-white/90 hover:bg-white p-2 sm:p-3 rounded-full shadow-md">
                                                    <i className="fas fa-chevron-left"></i>
                                                </button>
                                                <button onClick={() => setDevLetterIndex((devLetterIndex + 1) % devLetterImages.length)} className="absolute right-2 sm:right-4 top-1/2 -translate-y-1/2 bg-white/90 hover:bg-white p-2 sm:p-3 rounded-full shadow-md">
                                            ...
                                            <div className="mt-3">
                                                <div ref={thumbsContainerRef} className="flex gap-3 dev-carousel-thumbs overflow-x-auto py-2 px-1" style={{ WebkitOverflowScrolling: 'touch' }}>
                                                    {devLetterImages.map((img, i) => (
                                                        <button ref={el => thumbItemsRef.current[i] = el} key={i} onClick={() => setDevLetterIndex(i)} className={`flex-shrink-0 w-24 sm:w-28 md:w-36 h-16 sm:h-20 md:h-24 rounded-lg overflow-hidden border-2 ${i === devLetterIndex ? 'border-blue-500' : 'border-transparent'} shadow-sm` }>
                                    ...
                                    {/* CTA: send your photos to Telegram */}
