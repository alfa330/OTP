...
- Опирайся на CATALOG: category и subcategory ВСЕГДА бери строго из него (точные строки).
- ВАЖНО — автокоррекция темы: если выбранные CATEGORY/SUBCATEGORY не соответствуют сути DESCRIPTION
  (и ответам), САМ выбери правильные category и subcategory из CATALOG. Если изменил хотя бы одно —
  поставь "category_adjusted": true и кратко объясни в "category_adjustment_note" (что и почему изменено,
  например: «Тип изменён на „Не поступают звонки в Oktell“ — в описании речь о входящих звонках»).
  Если всё совпадает — "category_adjusted": false и пустой note.
