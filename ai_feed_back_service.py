...
«Создать внутренний номер сотруднику Омарова Ару, ОП, очереди …, тг @…».
...
2. Сформируй ОДИН набор полей form.fields — всё, что нужно заполнить под эту задачу (это и есть «уточнения»,
   ...
   (text|textarea|select|date|time|number), required (bool), placeholder, options (для select), hint, value.
   Обязательно включи поля под тип задачи: для «починить» — «Где (отдел + № РМ / ориентир)» и «Что случилось»;
   для «сделать» — «Что сделать» и «Для кого (отдел + РМ / ФИО)». Плюс при уместности: когда началось,
   частота, массовость (сколько человек), что уже пробовали, текст ошибки, ссылка/скрин.
   Обычно 3–6 полей. required=true только для критичных (Где, Что). Предзаполняй value из DESCRIPTION.
3. priority: low|medium|high|critical — по влиянию (массовость, простой, блокировка).
...
2. При status="ready" составь ticket:
   ...
   - markdown: ГОТОВЫЙ КОРОТКИЙ текст для Telegram, читается за 5 секунд. Правила:
     • Объём: 1–6 строк, чем короче — тем лучше (можно как в примерах — одной плотной строкой).
     • Только HTML-теги Telegram: <b>…</b>, <i>…</i>, <code>…</code>. Без *, #, markdown, <br>, <ul>, <p>.
     • Переносы — обычной новой строкой. Текст ошибки/код — в <code>…</code>.
     • Включай ТОЛЬКО строки с конкретикой; пустое/«ничего»/«все» — пропускай (не пиши «не указано»).
     • НЕ дублируй категорию, приоритет и автора — они добавляются отдельно.
     • Каркас — бери ТОЛЬКО нужные строки (заголовки <b>):
       для «починить»: <b>Что:</b> …   <b>Где:</b> отдел + № РМ / ориентир   <b>Когда/частота:</b> …   <b>Уже пробовали:</b> …
       для «сделать»:  <b>Сделать:</b> …   <b>Кому:</b> отдел + РМ / ФИО   <b>Детали:</b> очереди/ссылки/…
