    ...
    try:
        ...
        for idx, row in enumerate(operators):
            try:
                ...
                # accept bonuses array (new format): list of {type, quantity, amount, friend_names, video_links}
                bonuses_arr = row.get('bonuses') if ('bonuses' in row and isinstance(row.get('bonuses'), list)) else None
...
def _lms_build_bold_split_certificate_pdf(certificate_number, learner_name, course_title, issued_at):
    ...
    sig_name_1 = str(os.getenv("LMS_CERTIFICATE_SIGNER_1_NAME") or "Мулдир Тестова").strip()
    sig_role_1 = str(os.getenv("LMS_CERTIFICATE_SIGNER_1_ROLE") or "РУКОВОДИТЕЛЬ ПРОГРАММ").strip()
    sig_name_2 = str(os.getenv("LMS_CERTIFICATE_SIGNER_2_NAME") or "Рамазан Тестбай").strip()
