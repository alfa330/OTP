...
def message_time(message):
    ...
    OLX отдаёт ISO-8601 с зоной. Приводим к тому же виду, в котором в базе лежит
    всё остальное время проекта, иначе SLA считался бы с пятичасовой ошибкой.
    """
    ...
    if stamp.tzinfo is None:
        # Зоны нет — считаем, что это уже местное время, как и всё в базе.
        return stamp
    utc = stamp - stamp.utcoffset()
    ...
    return utc.replace(tzinfo=None) + timedelta(hours=5)
