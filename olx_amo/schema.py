...
_STATEMENTS = [
    ...
    CREATE TABLE IF NOT EXISTS olx_threads (
        ...
        -- Сколько непрочитанного было в чате, когда мы его последний раз
        -- разбирали. Это НЕ статистика, а пропуск лишних запросов: если счётчик
        -- не изменился, нового сообщения не появилось, и лезть за списком
        -- сообщений незачем. Без этого чаты, отсечённые горизонтом на первом
        -- запуске, навсегда занимали бы весь лимит цикла и вытесняли свежие.
...
_MIGRATIONS = [
    # Пока пусто: схема заводится первой версией. Дальнейшие правки — только
    # ALTER'ами с IF NOT EXISTS, чтобы разворот оставался идемпотентным.
]
