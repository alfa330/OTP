...
def can_delete(ctx):
    """Удалить карточку целиком — только глобальный админ.

    Ошибочно заведённую посылку менеджер не удаляет, а исправляет: удаление
    уносит с собой историю, а по ней потом отвечают на вопрос «куда делась
    коробка». То же правило, что у обращений.
    """
