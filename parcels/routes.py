...
def _export_period(args):
    ...
    if days > report.EXPORT_MAX_DAYS:
        return None, (jsonify({
            ...
            # число фиксированное, и «суток» после него стоит правильно. Писать
            # «%d суток» с подставляемым числом нельзя: «31 суток» — не
            # по-русски, а формы единственного числа у «суток» нет вовсе.
