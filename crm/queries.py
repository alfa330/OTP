...
# ─────────────────────────────────────────────────────────────────────────────
# Граница пространства вики
#
# Справочники парков и офисов принадлежат ПРОСТРАНСТВУ вики с 24.08.2026
# (wiki.schema._scope_directories_to_space). Раздел «Обращения» читает их
# прямым SQL, мимо wiki/parks.py и wiki/offices.py, где space_id обязателен, —
# и до 31.08.2026 читал целиком: в мастере, на шаге «Адрес офиса» в городах
# Атырау, Кокшетау и Туркестане рядом с нашим офисом стоял офис «Tez Taxi» из
# пространства «Тез».
#
# Своего понятия пространства у раздела нет и заводить его нечем: переключателя
# вики здесь не будет, а выводить пространство из отдела ТОГО, КТО СМОТРИТ,
# нельзя — глобального админа раздел пускает, а отдела у него нет вовсе, и
# справочник схлопнулся бы то в чужой, то во все сразу. Пространство у раздела
# одно и то же для всех, кто в него допущен, потому что и вопрос у всех один:
# «работает ли сегодня НАШ офис». «Наш» = отдела, которому выдан раздел.
# ─────────────────────────────────────────────────────────────────────────────

def section_space_ids(cursor):
    """Пространства вики, справочниками которых пользуется раздел.

    Отдел раздела — один и жёстко задан (access.SECTION_DEPARTMENT_CODE), так что
    и список пространств не зависит от пользователя. Запрос дешёвый (две строки в
    wiki_space_departments), и держать его результат в кеше процесса нельзя:
    границу правит конструктор пространств, а раздел обязан узнать об этом
    сразу, а не после перезапуска.
    """
    from wiki import structure as wiki_structure

    return wiki_structure.space_ids_for_departments(cursor, [access.SECTION_DEPARTMENT_CODE])


def taxi_parks(cursor, *, space_ids):
    ...
    space_ids именованным и БЕЗ значения по умолчанию — по образцу wiki/parks.py:
    забытый аргумент обязан уронить вызов, а не тихо вернуть чужой справочник.
    Пустой список означает «отделу раздела не выдано ни одного пространства», и
    честный ответ на него — пустой список парков: подставлять вместо границы
    «все» значит вернуть ту же дыру под другим именем.
    """
...
def parcel_matches(cursor, *, phone=None, licence=None, name=None,
    ...
    Город, который оператор указывает с 27.08.2026, в ключи НЕ добавлен: посылку
    в реестр заносит офис, а не звонящий, и запись по тому же водителю могла лечь
    в соседний город. Сузив выборку городом, мы спрятали бы от оператора ровно то
    совпадение, из-за которого обращение не нужно отправлять. Город каждой
    найденной записи в строке и так стоит — сравнить их человеку есть с чем.
...
def city_offices(cursor, city, day, *, space_ids):
    ...
    «Наши» — это ещё и НАШЕГО ПРОСТРАНСТВА (space_ids, см. section_space_ids).
    Раньше здесь стояло «справочник офисов у компании один, читаем целиком», и
    это перестало быть правдой 24.08.2026, когда справочник стал принадлежать
    пространству. Цена ошибки не в лишней строке списка: в Атырау, Кокшетау и
    Туркестане у «Тез» заведены свои точки «Tez Taxi», и три из четырёх — с
    признаком «офиса нет». Оператор мог выбрать чужую и получить от
    scenarios.office_verdict готовое «обращение не требуется, ответьте по
    таблице» — то есть отказать водителю парка по статусу чужого офиса.
    """
    ...
    cursor.execute(
        ...
        """,
        {'city': str(city or ''), 'day': day, 'spaces': space_ids},
    )
    items = []
    for row in cursor.fetchall():
