...
# Офис таксопарка в выбранном городе. Варианты не лежат ни в options, ни в
...
# ─────────────────────────────────────────────────────────────────────────────
# РЕГИОНЫ: вопросы офис-менеджерам городов (ТЗ задачи #201, Кастек Гаухар)
#
# Принимающая сторона здесь другая — не специалисты Sapar и не представители
# Яндекс Доставки, а офис-менеджеры по городам. Отсюда и общее у обеих тематик:
# обязательная проверка идёт не по словам водителя, а по ТАБЛИЦЕ, которую ведут
# сами получатели обращения.
#
# Обе таблицы к этому времени уже переехали в портал: реестр невостребованных
# посылок — в раздел «Посылки» (задача #240), статусы офисов — на вкладку
# «Офисы» в вики (задача #188). ТЗ описывает их как листы Google Doc, потому что
# писалось до переезда; сама проверка от этого не изменилась — изменилось лишь
# то, что в таблицу смотрит система, а не оператор глазами. Ровно этого просил
# владелец в уточнении к #240: «не заставлять каждый раз брать информацию,
# которая уже есть».
#
# Что именно смотрит система, объявлено данными: LOOKUP_INPUTS отвечает «по
# каким ответам спрашивать», LOOKUP_ON_ANSWER — «когда». У офисов ответ нужен,
# чтобы нарисовать САМ вопрос (список офисов города), поэтому он спрашивается
# сразу по городу; у реестра посылок ответ нужен только чтобы решить, идти ли
# дальше, — он спрашивается по кнопке «Далее».
# ─────────────────────────────────────────────────────────────────────────────

LOOKUP_PARCELS = 'parcels'   # реестр невостребованных посылок (раздел «Посылки»)
LOOKUP_OFFICES = 'offices'   # статусы офисов на сегодня (вкладка «Офисы» в вики)

# Города «Уточнения посылки» в ключах поиска по реестру нет намеренно — почему,
# объяснено у самого поиска (crm.queries.parcel_matches).
LOOKUP_INPUTS = {
    LOOKUP_PARCELS: ('driver_name', 'driver_licence', 'contact_number'),
    LOOKUP_OFFICES: ('office_city',),
}

LOOKUP_ON_ANSWER = frozenset({LOOKUP_OFFICES})

# Снимок справочника кладётся в ответы служебным ключом — так же, как снимок
# Sapar: это не вопрос оператору, но в обращении он обязан остаться, иначе через
# месяц не понять, на каком основании обращение вообще отправили.
OFFICES_ANSWER_KEY = '__offices__'


PARCEL_LOCATION = {
    'key': 'parcel_location',
    'queue_code': 'regions',
    'title': 'Уточнение посылки',
    'group_title': 'Просьба уточнить, где посылка водителя',
    'when_to_use': 'Курьер не смог доставить посылку получателю или забрать её у '
                   'отправителя и оставил в офисе таксопарка, а Яндекс уточняет, '
                   'там ли она и где именно.',
    'attachment': ATTACH_NONE,
    'lookup': LOOKUP_PARCELS,
    # §2.2 ТЗ: проверка по реестру обязательна, и отмечает её оператор. Система
    # ищет по реестру сама и показывает найденное рядом, но подтверждение с него
    # не снимает: реестр молодой, менеджеры могли ещё не начать его вести, и
    # пустой ответ поиска сегодня не доказывает, что посылки в офисе нет.
    'checks': ['В реестре невостребованных посылок (раздел «Посылки») записи '
               'по этому водителю нет'],
    # Тема без ИИН: у этой тематики его больше не спрашивают, а «Уточнение
    # посылки» одинаковое у всех обращений — в списке их было бы не различить.
    # Город стоит в теме по той же причине, что у соседней тематики: обращение
    # адресуется офис-менеджеру города, и в списке это первое, по чему его
    # разбирают.
    'subject_template': 'Уточнение посылки · {parcel_city} · {driver_name}',
    'steps': [
        # Пять полей §2.3, в порядке ТЗ, и шестым — город. Первые три и город —
        # из диспетчерской, они же уезжают в подпись к карточке, чтобы их можно
        # было скопировать.
        step('driver_name', 'Фамилия, имя, отчество водителя', TEXT,
             placeholder='Как в диспетчерской, без сокращений'),
        step('driver_licence', 'Номер ВУ', TEXT, half=True,
             placeholder='Например: 123456789'),
        step('contact_number', 'Номер телефона аккаунта', TEXT, half=True,
             hint='Тот номер, на который зарегистрирован аккаунт водителя'),
        # Города в §2.3 нет: он добавлен по возврату постановщика 27.08.2026
        # («рядом с фио, номера ВУ и телефона добавить поле для выбора города»).
        # Причина видна из §1 — принимающая сторона это офис-менеджеры ВСЕХ
        # городов в одной группе, а из ФИО, ВУ и телефона город не следует.
        # Без него первым ответом группы был вопрос «а какой город?», и
        # распределение обращений по городам (§4) строить было бы не на чем.
        #
        # Выбор из справочника, а не свободная строка: постановщик просила
        # «поле для выбора», и это же спасает от «Актау» рядом с «г. Актау» —
        # получателю обращения город служит адресом, а не описанием.
        step('parcel_city', 'Город', CITY,
             hint='Город, в офисе которого оставили посылку — со слов Яндекса'),
        step('delivery_date', 'Дата, когда посылку должны были доставить', DATETIME,
             date_only=True,
             hint='Со слов Яндекса или по данным заказа'),
        step('parcel_description', 'Краткое описание посылки', LONGTEXT,
             placeholder='Что это, из какого магазина или откуда, как выглядит'),
    ],
    'rules': [],
}


# Со слов водителя — вторая половина расхождения, ради которого обращение и
# заводится. В группе она обязана стоять: иначе вопрос «работает офис или нет»
# при статусе «Открыт» выглядит бессмыслицей.
#
# Сначала это была подставленная строка — условие применения тематики (§3.1) и
# так утверждает ровно это. Но подставленная фраза одинакова у всех обращений, а
# водитель называет подробности, по которым регион и разбирается: во сколько
# приехал, что увидел, кто что ответил. Поэтому спрашиваем, а фразу оставляем
# ответом по умолчанию — пустым обращение всё равно не уйдёт непонятным.
OFFICE_DRIVER_CLAIM = 'офис не работает'

OFFICE_STATUS = {
    'key': 'office_status',
    'queue_code': 'regions',
    'title': 'Статус работы офиса',
    # Фиксированная формулировка вопроса из §3.3. Её место — заголовок
    # сообщения, а не поле, которое оператор «заполняет»: ответ на такое поле
    # всегда один и тот же, и спрашивать его значит просить нажать «Далее» ни за
    # чем. В группе он читается первой строкой, как и требует ТЗ.
    'group_title': 'Уточнение — работает офис или нет?',
    'when_to_use': 'Водитель утверждает, что приехал в офис, а офис не работает. '
                   'Сверим со статусом офиса на сегодня и спросим регион только '
                   'тогда, когда таблица говорит, что офис открыт.',
    'attachment': ATTACH_NONE,
    'lookup': LOOKUP_OFFICES,
    'subject_template': 'Статус работы офиса · {office_city}',
    'steps': [
        step('office_city', 'Город', CITY,
             hint='Город, по которому уточняется статус офиса'),
        # Варианты приезжают с проверкой: это офисы ЭТОГО города со статусом на
        # сегодня. Оператор выбирает тот, куда приехал водитель, — и статус
        # именно этого офиса решает, отправлять обращение или нет.
        step('office', 'Адрес офиса', OFFICE),
        # Просьба владельца 28.08.2026: «надо добавить поле Номер
        # водителя/курьера, чтобы эта инфа тоже отправлялась в телеграм». Личный
        # номер, а не номер аккаунта: обращение заводится ради расхождения, и
        # закрыть его регион может только одним способом — позвонив тому, кто
        # стоит у офиса. Раньше в сообщении не было ни одного способа связи, и
        # менеджеру приходилось идти обратно через оператора.
        #
        # Стоит ПОСЛЕ офиса: при закрытом офисе или его отсутствии обращение не
        # отправляется вовсе, и спрашивать номер до этого значило бы просить
        # данные для сообщения, которого не будет.
        step('driver_phone', 'Номер водителя/курьера', TEXT,
             hint='Личный номер телефона — по нему регион свяжется с водителем'),
        # Необязательный: не оставить оператора без слов водителя нельзя, но и
        # держать его на поле, когда добавить нечего, незачем — тогда в группу
        # уходит фраза по умолчанию.
        step('driver_claim', 'Со слов водителя', LONGTEXT, optional=True,
             placeholder='Например: приехал к 10:00, дверь закрыта, на звонки не отвечают',
             hint='Что именно сказал водитель. Оставите пустым — в группу уйдёт '
                  '«офис не работает»'),
    ],
    'rules': [],
}
...
STEP_GROUPS = {
    ...
    # Кто спрашивает — у тематик регионов свой экран: там нет ни периода, ни
    # ИИН, а «Водитель и период» на экране с одним ФИО читается как чужая
    # подпись.
    'driver_name': 'Водитель',
    'driver_licence': 'Водитель',
    'contact_number': 'Водитель',
    # Город «Уточнения посылки» стоит здесь дословно по возврату постановщика:
    # «рядом с фио, номера ВУ и телефона». Своего экрана он не заводит — на нём
    # был бы один вопрос, то есть лишнее нажатие «Далее» посреди разговора.
    'parcel_city': 'Водитель',

    # Где именно — город и офис спрашиваются вместе: список офисов существует
    # только для выбранного города, и разводить их по двум экранам значило бы
    # показать пустой второй.
    'office_city': 'Офис',
    'office': 'Офис',
    # Номер водителя — про водителя, а не про офис, но экран у этой тематики
    # один: отдельным экраном он стоил бы лишнего нажатия «Далее» посреди
    # разговора, а порядок вопросов на экране и так идёт от офиса к водителю.
    'driver_phone': 'Офис',
    'driver_claim': 'Офис',

    # Про саму посылку
    'delivery_date': 'Посылка',
    'parcel_description': 'Посылка',
...
#
# У тематик регионов сюда же уходят ФИО, номер ВУ, телефон и город: специалист
# в группе копирует их так же, как ИИН у Sapar, — с картинки этого не сделать.
# Город тут ещё и адрес: обращение получают офис-менеджеры всех городов сразу.
BODY_DATA = ('iin', 'driver_name', 'driver_licence', 'contact_number',
             'park', 'city', 'parcel_city', 'office_city', 'office',
             'driver_phone', 'period')
BODY_DATA_LABELS = {
    'iin': 'ИИН',
    'driver_name': 'ФИО водителя',
    'driver_licence': 'Номер ВУ',
    ...
    'office_city': 'Город',
    'office': 'Адрес офиса',
    # Тематика спрашивает «Номер водителя/курьера», а в сообщении важно, что это
    # телефон: рядом стоят город и адрес, и «номер» читался бы как номер офиса.
    'driver_phone': 'Телефон водителя/курьера',
    'period': 'Отчётный период',
}

# Что в перечень ответов отдельной строкой не идёт. Данные водителя — потому
# что они уже в блоке данных; «со слов водителя» — потому что он показывается в
# паре со статусом таблицы, и в общем перечне стоял бы отдельно от того самого
# утверждения, которому противоречит.
BODY_SKIP = BODY_DATA + ('driver_claim',)
...
#
# Карточку-картинку (crm/card.py) завели под ТЗ #206 — макет СЗоВ с плашками и
# галочками в кружках, которых в разметке Telegram нет. Оправдана она там, где
# текста МНОГО: у вопросов Sapar в сообщении до восемнадцати строк — данные,
# проверенные пункты, что оператор уже сделал.
#
# Остальным тематикам она мешает. В «Уточнении посылки», «Статусе работы офиса»
# и термокоробе сообщение — от одной до шести строк: картинка ради них заставляет
# открывать вложение, чтобы прочитать то, что уместилось бы в самом сообщении, а
# поиску по чату остаётся только подпись — самого текста обращения в нём нет.
# Решение владельца 28.08.2026: картинка остаётся только у вопросов Sapar.
#
# Признак задан на ОЧЕРЕДЬ, а не на каждую тематику: появится шестая категория
# Sapar — она поедет карточкой сама, и вспоминать про неё не придётся.
# ─────────────────────────────────────────────────────────────────────────────

CARD_QUEUES = frozenset({'itaxi_sapar'})


def sends_card(scenario_key):
    """Уходит ли обращение картинкой. Нет — уходит обычным текстом."""
    return (get(scenario_key) or {}).get('queue_code') in CARD_QUEUES


# ─────────────────────────────────────────────────────────────────────────────
# Кого отмечать в группе (задача #280, постановщик Сабыр Азана)
#
# В группе «iTaxi Вопросы/ответы» сидят офис-менеджеры ВСЕХ городов сразу, и до
# сих пор обращение висело там ничьим: пока кто-нибудь не прочитает всю ленту,
# свой город никто не узнавал. Тег в сообщении и есть адресация — Telegram
# доводит упоминание уведомлением, даже если группа приглушена.
#
# Правило владельца (07.09.2026) ровно одно, и оно повторяет то, что оператор
# уже ответил: выбран конкретный офис — отмечаем ник ЭТОГО офиса; выбран только
# город — отмечаем ники всех офисов города. Второе не «на всякий случай»: в
# «Уточнении посылки» офис не спрашивают вовсе (Яндекс называет город), а в
# Алматы у нас четыре офиса парка, и угадывать за оператора нечего.
#
# Признак задан на ОЧЕРЕДЬ, как и картинка. Иначе тег расползся бы: город
# спрашивают и вопросы Sapar (STEP_CITY на входе в тематику), но их читают
# специалисты Sapar в своей группе, и офис-менеджеру Тараза там делать нечего.
#
# Ключи ответов не перечислены списком, а выведены из ТИПОВ шагов: OFFICE и CITY
# уже отвечают на вопрос «где спрашивать офис и где город», и второй список тех
# же ключей разошёлся бы с первым при первом же переименовании шага.
# ─────────────────────────────────────────────────────────────────────────────

MENTION_QUEUES = frozenset({'regions'})


def mention_target(scenario_key, answers):
    """Кого искать в справочнике офисов: {'office_id': …} либо {'city': …}.

    None — тематика вообще не тегает (чужая очередь или ответов нет). Офис
    старше города: он и есть точный адрес, и тегать рядом с ним соседние офисы
    того же города значит будить людей по чужому вопросу.
    """
    scenario = get(scenario_key) or {}
    if scenario.get('queue_code') not in MENTION_QUEUES:
        return None

    answers = answers or {}
    office_id = city = None
    for item in scenario.get('steps') or []:
        value = _value(answers, item.get('key'))
        text = str(value).strip() if value is not None else ''
        if not text:
            continue
        if item.get('kind') == OFFICE and office_id is None:
            office_id = text
        elif item.get('kind') == CITY and city is None:
            city = text
    if office_id:
...
# ─────────────────────────────────────────────────────────────────────────────
# Проверки по справочникам компании (ТЗ #201)
#
# Устроены как предпроверка Sapar и по той же причине: снимок приносит слой базы
# (crm.queries), а решение принимает чистая функция, которую можно прогнать
# тестом без единого запроса. Модуль по-прежнему не знает ни базы, ни сети — он
# знает только, как выглядит снимок.
# ─────────────────────────────────────────────────────────────────────────────
...
PARCEL_KIND_LABELS = {'parcel': 'посылка', 'document': 'документ', 'other': 'другое'}

# Статусы офиса, при которых спрашивать регион не о чем: таблица уже ответила.
# «Нет графика» сюда НЕ входит — это не ответ, а его отсутствие, и запрещать по
# нему обращение значило бы выдать незнание за знание. §3.2 требует обратного:
# спрашиваем, когда таблица говорит «Открыт», а водитель — противоположное.
OFFICE_CLOSED_STATES = ('closed', 'absent')


def lookup_kind(scenario_key):
    """По какому справочнику проверяется тематика. None — проверки нет."""
    return (get(scenario_key) or {}).get('lookup')


def lookup_inputs(kind):
    """Ответы, по которым спрашивают справочник."""
    return list(LOOKUP_INPUTS.get(str(kind or ''), ()))


def _day_text(value):
    """'2026-08-12' -> '12.08.2026'. Не дата — как есть."""
    found = re.match(r'^(\d{4})-(\d{2})-(\d{2})', str(value or '').strip())
    if not found:
        return str(value or '')
    return '%s.%s.%s' % (found.group(3), found.group(2), found.group(1))


# ─── Реестр невостребованных посылок ────────────────────────────────────────

def parcel_match_text(item):
    """Одна запись реестра одной строкой — как её прочитает оператор вслух."""
    item = item or {}
    where = ITEM_SEP.join(x for x in (item.get('city'), item.get('office_name')) if x)
    parts = [x for x in (
        _day_text(item.get('received_on')),
        where,
        PARCEL_KIND_LABELS.get(item.get('kind'), item.get('kind')),
        PARCEL_STATUS_LABELS.get(item.get('status'), item.get('status')),
    ) if x]
    text = ITEM_SEP.join(parts)
    description = ' '.join(str(item.get('description') or '').split())
    return '%s — %s' % (text, description) if description else text


def parcel_registry_verdict(snapshot):
    """Что реестр невостребованных посылок говорит про этого водителя.

    BLOCKED, а не CLOSE, и это главное решение здесь. §2.2 ТЗ отменяет
    обращение, если в реестре есть запись, «совпадающая по водителю/посылке», —
    а совпадает ли она, решает человек: у водителя может лежать вторая посылка,
    а по ФИО в реестр попадают полные тёзки. Поэтому найденное показывается
    рядом с вопросом и мешает идти дальше, но последнее слово за оператором.

    Пустой список — PASS: в НАШЕМ реестре записи нет. Доказательством, что её
    нет вообще, это не считается, поэтому пункт проверки оператор отмечает сам.
    """
    snapshot = snapshot or {}
    items = snapshot.get('items') or []
    if not snapshot.get('available') or not items:
        return {'outcome': PASS, 'message': None, 'items': []}
    return {
        'outcome': BLOCKED,
        'message': ('В реестре невостребованных посылок есть %s по этому водителю. '
                    'Если это та самая посылка — ответ берите из реестра, обращение '
                    'в группу не нужно. Если посылка другая — продолжайте.'
                    % ('запись' if len(items) == 1 else 'записи (%d)' % len(items))),
        'items': [parcel_match_text(item) for item in items],
    }
...
def office_label(office):
    """Как офис называется в обращении: название и адрес из справочника вики."""
    office = office or {}
    parts = [x for x in (office.get('name'), office.get('address')) if x]
    return ITEM_SEP.join(parts) if parts else 'офис без адреса'


def office_by_id(snapshot, office_id):
    """Офис снимка по идентификатору.

    Сравниваем строками: с клиента ответ приезжает строкой, из базы — числом, и
    одно из двух сравнений всегда оказывалось бы ложным.
    """
    wanted = str(office_id or '').strip()
    if not wanted:
        return None
    for office in ((snapshot or {}).get('offices') or []):
        if str(office.get('id')) == wanted:
            return office
    return None


def _office_state_text(office):
    """«Закрыт · ремонт · до 31.08» — статус вместе с причиной, если она есть."""
    office = office or {}
    parts = [office.get('label') or office.get('state') or 'статус неизвестен']
    note = ' '.join(str(office.get('note') or '').split())
    if note:
        parts.append(note)
    until = office.get('closed_until')
    if office.get('state') == 'closed' and until:
        parts.append('до %s' % _day_text(until))
    return ITEM_SEP.join(parts)


def office_verdict(snapshot, office_id=None):
    """Можно ли спрашивать регион про этот офис (§3.2 ТЗ).

    Отправляем только при расхождении: таблица говорит «Открыт», а водитель —
    что офис не работает. Сказала таблица «Закрыт» или «Офиса в городе нет» —
    ответ у оператора уже есть, и группу этим не занимают.

    Справочник не ответил — PASS: считать молчание за «офис закрыт» нельзя, по
    такому выводу оператор отказал бы водителю в ответе, которого никто не
    давал. Ровно так же ведёт себя предпроверка Sapar.
    """
    snapshot = snapshot or {}
    if not snapshot.get('available'):
        return {'outcome': PASS, 'message': None}

    city = snapshot.get('city') or 'этом городе'
    offices = snapshot.get('offices') or []
    if not offices:
        return {
            'outcome': CLOSE,
            'message': ('В таблице статусов офиса в городе «%s» нет. '
                        'Обращение не требуется — ответьте водителю по таблице.' % city),
        }

    chosen = office_by_id(snapshot, office_id)
    if chosen is not None:
        if chosen.get('state') in OFFICE_CLOSED_STATES:
            return {
                'outcome': CLOSE,
                'message': ('По таблице статусов офис «%s» сегодня: %s. '
                            'Обращение не требуется — ответьте водителю по таблице.'
                            % (office_label(chosen), _office_state_text(chosen))),
            }
        return {'outcome': PASS, 'message': None}

    # Офис ещё не выбран. Если в городе не осталось ни одного, про который есть
    # смысл спрашивать, — не заставляем выбирать из закрытых.
    if all(office.get('state') in OFFICE_CLOSED_STATES for office in offices):
        # Офисы — отдельными строками, а не одной фразой: внутри строки уже
        # стоят разделители («Закрыт · ремонт · до 31.08»), и вторым уровнем тех
        # же точек не видно, где кончается один офис и начинается другой.
        return {
            'outcome': CLOSE,
            'message': ('По таблице статусов офисы города «%s» сегодня не работают. '
                        'Обращение не требуется — ответьте водителю по таблице.' % city),
            'items': ['%s — %s' % (office_label(item), _office_state_text(item))
                      for item in offices],
        }
    return {'outcome': PASS, 'message': None}
...
def validate_step(item, answers):
    ...
    # список офисов приезжает не из справочника фронта, а тем же снимком, что
    # лежит в ответах, — значит он есть и здесь, и проверять его есть чем.
    if kind == OFFICE and office_by_id(answers.get(OFFICES_ANSWER_KEY), value) is None:
        return 'Выберите офис из списка'
...
def evaluate(scenario_key, answers, *, has_attachment=False, checks_confirmed=False,
    ...
    # 1а. Справочник компании. Такое же правило, только его условие лежит не в
    # ответах оператора, а в снимке, который положил сюда сервер: «офис по
    # таблице закрыт» — это не то, что оператор сказал, а то, что он проверил.
    if scenario.get('lookup') == LOOKUP_OFFICES:
        hit = office_verdict(answers.get(OFFICES_ANSWER_KEY), _value(answers, 'office'))
        if hit['outcome'] != PASS:
            return {'outcome': hit['outcome'], 'message': hit['message'],
                    'step': 'office', 'missing': {}}
...
def format_answer(item, answers):
    ...
    if item['kind'] == OFFICE:
        # Снимок лежит в самих ответах, поэтому подпись собирается и через
        # месяц: обращение читается тем же кодом, что его создавал.
        found = office_by_id(answers.get(OFFICES_ANSWER_KEY), value)
        return office_label(found) if found else str(value).strip()
    if item['kind'] == PERIOD:
...
BLOCK_TABLE = 'table'        # что говорит справочник компании (статусы офисов)
...
def _office_rows(snapshot, office_id, claim=None):
    """Что таблица статусов офисов говорит о выбранном офисе. Пусто — если её не спрашивали.

    Две строки, и вторая не менее важна первой: без «со слов водителя» вопрос
    «работает офис или нет» при статусе «Открыт» выглядит в группе бессмыслицей,
    а с ней читается как расхождение, ради которого обращение и заводят.

    Вторую строку пишет оператор. Не написал — остаётся фраза из §3.1: она и
    была здесь раньше, и без неё сообщение теряет смысл.
    """
    office = office_by_id(snapshot, office_id)
    if not office:
        return []
    claim = ' '.join(str(claim or '').split()) or OFFICE_DRIVER_CLAIM
    return [
        {'label': 'Статус на сегодня', 'value': _office_state_text(office)},
        {'label': 'Со слов водителя', 'value': claim},
    ]
...
def body_blocks(scenario_key, answers, *, flags=()):
    ...
    rows = _office_rows(answers.get(OFFICES_ANSWER_KEY), _value(answers, 'office'),
                        _value(answers, 'driver_claim'))
    if rows:
        # Таблицу статусов ведут сами получатели обращения. Показываем, что мы в
        # неё смотрели и что там стояло, — иначе первый их вопрос будет «а вы
        # таблицу открывали?».
        blocks.append({'kind': BLOCK_TABLE, 'title': 'По таблице статусов офисов',
                       'rows': rows})
...
def render_body(scenario_key, answers, *, flags=()):
    ...
    for block in body_blocks(scenario_key, answers, flags=flags):
        ...
        elif kind in (BLOCK_DATA, BLOCK_LIST, BLOCK_SAPAR, BLOCK_TABLE):
...
def public_catalog():
    ...
    return [{
        ...
        'title': item['title'],
        # Первая строка сообщения в группе. Мастеру она нужна в предпросмотре:
        # без неё оператор подтверждает не то, что увидят коллеги, — а у
        # «Статуса работы офиса» именно в ней стоит сам вопрос из ТЗ.
        ...
        # Проверка по справочнику компании: по какому, по каким ответам и в
        # какой момент спрашивать. Мастер не знает названий тематик — он читает
        # это, поэтому третья такая проверка не потребует правки интерфейса.
        'lookup': item.get('lookup'),
        'lookup_inputs': lookup_inputs(item.get('lookup')),
        'lookup_on_answer': item.get('lookup') in LOOKUP_ON_ANSWER,
        ...
        # Уходит ли отсюда вообще что-нибудь в рабочую группу. У «Документы не
        # поступили» заполненные ответы — набор данных для супервайзера, и
        # адреса у такой темы нет: назначать ей группу нечего и незачем.
        'sends_to_group': not item.get('final_outcome'),
        'steps': item['steps'],
