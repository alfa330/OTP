...
# заводится. Оператора об этом не спрашивают: он выбрал тематику, а её условие
# применения (§3.1) и есть это утверждение. Но в группе оно обязано стоять —
# иначе вопрос «работает офис или нет» при статусе «Открыт» выглядит бессмыслицей.
...
def _office_rows(snapshot, office_id):
    ...
    return [
        ...
        {'label': 'Со слов водителя', 'value': OFFICE_DRIVER_CLAIM},
    ]
