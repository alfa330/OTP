...
Модель (по ТЗ задач #133 и #29):
    ...
    crm_topic_routes    темы, уведённые в чужую группу (адрес перебивает queue_code)
...
_STATEMENTS = [
    ...
    #
    # Тема (сценарий в crm/scenarios.py) по умолчанию уходит в группу своей
    # тематики — это её queue_code. Маршрут перебивает адрес у ОДНОЙ темы, не
    # трогая остальные: «Ошибку в работе Sapar» можно увести к техподдержке,
    # оставив всё остальное в группе Sapar.
    #
    ...
    # темы означал бы два получателя одного обращения, а обращение уходит в
    # одну группу: ответ из неё возвращается в нить, и второй нити нет.
    #
    # ON DELETE CASCADE: удалить очередь можно только пока по ней нет обращений
    # (delete_queue это стережёт), и вместе с ней исчезает указывающий на неё
    # маршрут — тема возвращается в группу своей тематики.
    # ──────────────────────────────────────────────────────────────────────
    """
    ...
    """ % {'now': _NOW},
    # «Что ещё приходит в эту группу» — вопрос настройщика при каждом открытии
    # вкладки очередей, и он спрашивается по очереди, а не по теме.
    """
    CREATE INDEX IF NOT EXISTS idx_crm_topic_routes_queue
        ON crm_topic_routes(queue_id)
    """,
