...
Модель (по ТЗ задач #133 и #29):
    ...
    crm_topic_routes    темы, уведённые в другой Telegram-чат (адрес поверх queue_code)
...
_STATEMENTS = [
    ...
    #
    # Тема (сценарий в crm/scenarios.py) по умолчанию уходит в чат своей
    # тематики — очереди с кодом queue_code. Маршрут перебивает адрес у ОДНОЙ
    # темы, не трогая остальные: «Отображается оплата» можно увести в чат
    # «Sapar/Kaspi — отмена», оставив всё остальное в чате iTaxi Sapar.
    #
    # **Адрес — chat_id, а не очередь.** Это главное решение таблицы. Через
    # очередь адресом могли быть только СОСЕДНИЕ тематики, и чтобы отправить
    # тему в произвольный рабочий чат, пришлось бы сначала заводить на этот чат
    # тематику — то есть выдумывать сущность ради адреса. Бот и так знает все
    # чаты, где состоит (it_ticket_channels), и выбирают ровно из них.
    #
    # FK на it_ticket_channels нет НАМЕРЕННО: тот реестр наполняет обработчик
    # my_chat_member, и строка оттуда может уехать (бота выгнали из чата). Тема
    # при этом не должна ни падать, ни молча менять адрес — она становится
    # недоступной, и настройщик видит почему. Проверка живости чата — в
    # queries.resolve_route, а не в базе.
    #
    # Почему отдельная таблица, а не колонка в crm_topics. crm_topics — это
    # свободный список названий, который никогда не был связан со сценариями
    # (на проде он пуст, а topic_id у всех обращений NULL). Тема же существует
    # в коде, у неё есть ключ, и адресовать надо именно её.
    #
    # Почему ключ, а не id. Темы объявлены данными в Python и id у них нет;
    # scenario_key — то же самое, чем тема помечена в самом обращении
    # (crm_tickets.scenario_key), и по нему строится отчёт по тематикам.
    #
    # PRIMARY KEY на ключе = у темы ровно один адрес. Второй маршрут той же
    # темы означал бы двух получателей одного обращения, а ответ возвращается
    # в одну нить — второй нити нет. Обратное не запрещено: РАЗНЫЕ темы в один
    # чат уводят сколько угодно, поэтому уникальности по chat_id тут нет.
    #
    # chat_title — снимок названия на момент настройки. Живое название берётся
    # из реестра чатов; снимок нужен, чтобы объяснить адрес, когда чата в
    # реестре уже нет («бот больше не в этой группе»).
    # ──────────────────────────────────────────────────────────────────────
    """
    CREATE TABLE IF NOT EXISTS crm_topic_routes (
...
_MIGRATIONS = [
    ...
    # Куда обращение ушло НА САМОМ ДЕЛЕ. Раньше адрес однозначно вычислялся из
    # очереди, теперь у темы может быть свой чат — и в списке нужно показывать
    # тот, куда сообщение легло. Снимок, а не вычисление на лету: маршрут
    # завтра поменяют, а нить старого обращения останется в прежнем чате.
    "ALTER TABLE crm_tickets ADD COLUMN IF NOT EXISTS tg_chat_title VARCHAR(255)",

    # Первая версия маршрутов адресовала ОЧЕРЕДЬ (crm_topic_routes.queue_id).
    # Это оказалось не тем: адресом могли быть только соседние тематики. Ниже —
    # перевод на chat_id. Порядок важен: сначала колонки, потом перенос, потом
    # снятие старой. На проде строк не было ни одной, но база разработчика или
    # стенд могли успеть их завести — поэтому перенос, а не DROP TABLE.
    "ALTER TABLE crm_topic_routes ADD COLUMN IF NOT EXISTS chat_id BIGINT",
    "ALTER TABLE crm_topic_routes ADD COLUMN IF NOT EXISTS chat_title VARCHAR(255)",
    """
    DO $$
    BEGIN
        IF EXISTS (SELECT 1 FROM information_schema.columns
                    WHERE table_name = 'crm_topic_routes' AND column_name = 'queue_id') THEN
            EXECUTE 'ALTER TABLE crm_topic_routes ALTER COLUMN queue_id DROP NOT NULL';
            EXECUTE 'UPDATE crm_topic_routes r
                        SET chat_id = q.chat_id, chat_title = q.chat_title
                       FROM crm_queues q
                      WHERE q.id = r.queue_id AND r.chat_id IS NULL';
            EXECUTE 'ALTER TABLE crm_topic_routes DROP COLUMN queue_id';
        END IF;
    END $$
    """,
    # Маршрут без адреса — это не маршрут: очередь могла стоять без чата.
    "DELETE FROM crm_topic_routes WHERE chat_id IS NULL",
    "ALTER TABLE crm_topic_routes ALTER COLUMN chat_id SET NOT NULL",
]
...
_SEED_QUEUES = [
    ...
    # Четвёртая группа ТЗ #133 — «iTaxi Вопросы-ответы с регионами». Принимающая
    # сторона тут не служба, а офис-менеджеры по городам, и вопросов к ним по
    # ТЗ #201 два: посылка, оставленная курьером в офисе, и статус работы офиса.
    ('regions', 'Регионы',
     'Вопросы офис-менеджерам городов: посылка, оставленная в офисе, '
     'и статус работы офиса.', 40),
]
