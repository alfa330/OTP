...
def build_oktell_guard_blueprint(*, db, require_api_key, build_cors_preflight_response,
    ...
    def oktell_guard_download(requester_id, requester):
        ...
        только тем, кто видит раздел: с 07.09 по 22.09.2026 пункт «Скачать
        Oktell» стоял у них в меню, как «Скачать iCore Phone» у ОП и Тез КЦ.
        Пункт убран — агент ставится MSI через групповую политику, — сюда
        теперь ходит только кнопка «Скачать агента» в самом разделе. Круг не
        сужали: секретного тут ничего нет, сам файл и так отдаёт публичная
        /version, иначе не работало бы автообновление ни на одной машине.
        """
        with db._get_cursor() as cursor:
            release = queries.current_release(cursor)
            if not release:
