...
def can_download_agent(user) -> bool:
    ...
    Шире, чем раздел: пункт «Скачать Oktell» стоит в меню у КАЖДОГО оператора
    СЗоВ, а не только у тех, кто видит настройки. Так и правильнее по существу —
    личный токен, уезжающий в имени файла, оказывается на самом человеке, а не
    на супервайзере, который раздавал установщик за него. До этого присланные
    факты подписывались чужим именем, и в отчёте приходилось делать пометку
    «агент принадлежит такому-то, факт про номер такой-то».

    Ничего секретного этим не открывается: сам файл и так отдаёт публичная
    /version, она нужна автообновлению на каждой машине.
    """
